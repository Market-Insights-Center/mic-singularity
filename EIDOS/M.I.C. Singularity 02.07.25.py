...
async def handle_spear_command(args: List[str], ai_params: Optional[Dict] = None, is_called_by_ai: bool = False):
    """Handles the new /spear command for CLI and AI."""
    if not is_called_by_ai:
        print("\n--- /spear Command ---")
    ...
    # Corrected line: 'lastPrice' instead of 'last_price'
    actual_price = tckr.fast_info.get('lastPrice')
    if not actual_price:
    ...
    prediction_data = calculate_spear_prediction(
        ...
        params['market_trend'], params['market_reversal_likely'], params['is_meme_stock'], 1, actual_price
    )
    ...
    if -0.025 <= prediction <= 0.025:
        ...
        trade_recommendation = f"Buy Calls (${strike_price} Strike Price) Or Place An Order For A Long Position. Prepare With Proper Risk Management Such As Taking A Smaller Position."
    elif -0.05 <= prediction < -0.025:
        trade_recommendation = "Do Not Trade Options. Place A Buy Stop Order For A Short Position At -2%."
    elif prediction < -0.05:
        strike_price = round(d1, -1) if actual_price > 100 else round(d1, 0) if actual_price > 10 else round(d1, 1)
        trade_recommendation = f"Buy Puts (${strike_price} Strike Price) Or Place An Order For A Short Position. Prepare With Proper Risk Management Such As Taking A Smaller Position."
...
get_comparison_custom_portfolio_tool = FunctionDeclaration(
    ...
    description=(
        "Performs a full comparison cycle for a custom portfolio. It FIRST loads the previously saved run output (if any) "
        "FROM ITS CSV FILE. THEN, it runs the portfolio analysis fresh using specified or default parameters "
        "(e.g., 'value_for_assessment', 'use_fractional_shares_override'). It returns a detailed comparison string "
        "between this 'saved/on-disk' version and the 'freshly run' version. FINALLY, IT AUTOMATICALLY SAVES THE 'FRESHLY RUN' "
        "OUTPUT TO THE PORTFOLIO'S CSV FILE, OVERWRITING THE PREVIOUS DATA. This tool effectively updates the baseline while providing a comparison to the state just before the update. "
        "If the user asks to 'run portfolio X and compare to its previous state', this single tool can handle the entire process."
    ),
    parameters={"type":"object", "properties":{
        "portfolio_code":{"type":"string","description":"The code/name of the custom portfolio to compare and then update."},
        "value_for_assessment":{"type":"number","description":"Optional. The monetary value to use for the 'fresh run' part of the comparison if tailoring is desired. If not provided, fresh run might be untailored or use portfolio's default config."},
        "use_fractional_shares_override":{"type":"boolean","description":"Optional. For the 'fresh run' part: true/false to override the portfolio's default fractional share setting for the comparison. If null/not provided, respects portfolio's saved config."}
...
AVAILABLE_PYTHON_FUNCTIONS: dict[str, callable] = {
    ...
    "save_breakout_stock_data": handle_breakout_command,
    ...
    "handle_spear_command": handle_spear_command, # <--- ADD THIS
}
...
async def handle_ai_prompt(user_new_message: str, is_new_session: bool = False, original_session_request: Optional[str] = None):
    ...
    if not gemini_model:
        ...
        # For an actual application, you might return a user-facing error or raise an exception
    ...
    # Define all_gemini_tools inside the function using globally defined FunctionDeclarations
    try:
        all_gemini_tools = Tool(function_declarations=[
            ...
            get_comparison_for_custom_portfolio,
            spear_analysis_tool # <--- ADD THIS
        ])
    except NameError as e:
        # This means one of the FunctionDeclaration objects (e.g., run_breakout_tool) isn't defined globally
    ...
    if is_new_session:
        ...
        AI_INTERNAL_STEP_COUNT = 0 # Reset for a new logical session
        # Store the very first request of this new session
        CURRENT_AI_SESSION_ORIGINAL_REQUEST = original_session_request if original_session_request else user_new_message
        
        base_system_prompt_template = load_system_prompt() # Assumes load_system_prompt is defined
        current_date_val = datetime.now().strftime('%B %d, %Y') # Standard datetime
        current_date_mmddyyyy_val = datetime.now().strftime('%m/%d/%Y') # Standard datetime
        try:
            # Ensure your system_prompt.txt uses these exact placeholder names if you customize it
            ...
            print(f"KeyError during system prompt formatting: {e_format_sys}. Check system_prompt.txt or default prompt string.")
            # Fallback or error
    ...
    max_internal_turns = 10 # Max tool calls + final summary attempt
    executed_tool_calls_in_current_interaction = set() # To detect immediate repetitions for the current user message
    functions_were_called_in_this_interaction = False # Track if any tool was successfully invoked for this user message
    ...
    try: 
        for turn_num in range(max_internal_turns):
            AI_INTERNAL_STEP_COUNT = turn_num # Update global step count (optional, if used elsewhere)
            try:
                gemini_response = await asyncio.to_thread(
                    gemini_model.generate_content,
                    contents=AI_CONVERSATION_HISTORY,
                    tools=[all_gemini_tools], 
                    tool_config={"function_calling_config": {"mode": "ANY"}} # ANY allows model to call function or reply with text
                )
                ...
                # Consider logging this error server-side if applicable
            ...
            model_turn_history_parts_to_add = [] # Parts for the current model turn to append to history
            ...
            if gemini_candidate.content and gemini_candidate.content.parts:
                for part_object in gemini_candidate.content.parts:
                    if part_object.function_call and part_object.function_call.name:
                        ...
                        # Ensure args are serializable (they should be from Gemini)
            ...
            # Append what the model decided (either call a function or say something)
            if model_turn_history_parts_to_add:
                AI_CONVERSATION_HISTORY.append({"role": "model", "parts": model_turn_history_parts_to_add})
            elif text_from_model_in_turn.strip(): # If only text was generated
                 AI_CONVERSATION_HISTORY.append({"role": "model", "parts": [{"text": text_from_model_in_turn.strip()}]})
            # else:
                # This case (no function call, no text) should ideally lead to summarization or error if it's unexpected.
                # If functions_were_called_in_this_interaction is false, it's an empty first response.
            ...
            if function_call_to_run_this_turn:
                ...
                tool_args_dict_current = dict(function_call_to_run_this_turn.args) # Use the already serialized args
                ...
                if current_call_signature_tuple in executed_tool_calls_in_current_interaction:
                    ...
                    break # Exit loop, will go to summarization
                ...
                if tool_name_current in AVAILABLE_PYTHON_FUNCTIONS:
                    ...
                    tool_result_from_execution = f"Error: Tool '{tool_name_current}' did not return a recognizable value." # Default
                    try:
                    ...
                    # Prepare payload for history (must be JSON serializable)
                    ...
                    try: 
                        MAX_TOOL_RESPONSE_SIZE_BYTES = 30000 # Gemini API recommendation is often around 32KB
                        ...
                        if len(json_str_payload.encode('utf-8')) > MAX_TOOL_RESPONSE_SIZE_BYTES:
                            # Intelligent truncation if possible
                            ...
                            if found_summary_for_trunc:
                                 response_payload_for_history = {"status": "truncated_due_to_size", "summary_from_tool": found_summary_for_trunc[:1500]+"..."} # Truncate summary
                            else: # Generic truncation
                    ...
                    final_text_response_from_ai = "AI proposed an unknown tool. Cannot proceed with this path." # User-facing error
                    break # Exit loop
            elif text_from_model_in_turn.strip(): 
                final_text_response_from_ai = text_from_model_in_turn.strip()
                break # Model provided a direct text response, end loop
            else: 
                # No function call, no text from model in this turn.
                # If functions were called previously in this interaction, proceed to summarization.
                # If no functions were called at all, it's an empty/failed response from the start.
                ...
                break # Exit loop, rely on summarization logic if functions were called

        else: # Loop completed max_internal_turns (else block of for loop)
            if not final_text_response_from_ai.strip(): # Only if no text response was set during the loop
                # This means loop finished by max_turns without a text response or breaking function call error
...
async def main_singularity():
    ...
    while True:
        try:
            ...
            elif command == "/spear": await handle_spear_command(args, ai_params=None, is_called_by_ai=False) # <--- ADD THIS
