...
async def handle_macd_forecast_command(args: List[str], ai_params: Optional[Dict] = None, is_called_by_ai: bool = False):
    ...
    if ai_params:
        ...
        # Improved sanitization to handle spaces, commas, and quotes
        tickers = [t.strip().upper().replace('"', '') for t in tickers_str.replace(',', ' ').split() if t.strip()]
    elif args:
        tickers = [t.strip().upper().replace('"', '') for t in args]
    else:
        tickers_input = input("Enter a list of stock tickers separated by spaces or commas: ")
        tickers = [ticker.strip().upper().replace('"', '') for ticker in tickers_input.replace(',', ' ').split() if ticker.strip()]
