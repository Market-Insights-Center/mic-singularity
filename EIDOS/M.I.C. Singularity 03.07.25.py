...
async def handle_breakout_command(args: List[str], ai_params: Optional[Dict] = None, is_called_by_ai: bool = False):
    ...
    if action_to_perform == "run":
        # For AI, we need to return the data from the analysis
        breakout_list = await run_breakout_analysis_singularity(is_called_by_ai=is_called_by_ai)
        if is_called_by_ai:
            if not breakout_list:
                return "Breakout analysis ran successfully but found no stocks matching the required criteria."
            # The tool function returns a list of dictionaries, which is a good format for the AI history.
            return breakout_list
        else:
            return None # CLI version prints its own output

    elif action_to_perform == "save":
        if date_for_save:
            return await save_breakout_data_singularity(date_for_save, is_called_by_ai=is_called_by_ai)
        else:
            # This case should not be reached due to prior checks, but as a safeguard:
...
# MODIFICATION: New unified breakout tool
breakout_command_tool = FunctionDeclaration(
    name="handle_breakout_command",
    description="Handles breakout stock analysis. Action 'run' identifies and returns a list of current breakout stocks. Action 'save' saves the most recent breakout data to a historical database for a given date. To save the latest data, ensure 'run' is called first.",
    parameters={"type": "object", "properties": {
        "action": {"type": "string", "description": "Choose 'run' to find new breakout stocks or 'save' to archive the last run's data.", "enum": ["run", "save"]},
...
async def handle_ai_prompt(user_new_message: str, is_new_session: bool = False, original_session_request: Optional[str] = None):
    ...
    try:
        all_gemini_tools = Tool(function_declarations=[
            breakout_command_tool,  # Use the new unified tool
            quickscore_tool,
            market_command_tool,
