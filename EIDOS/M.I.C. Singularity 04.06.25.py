...
async def _continuous_spinner_animation(stop_event: asyncio.Event, message_prefix: str = "AI is processing..."):
    ...
    animation_chars = ["|", "/", "-", "\\"] # Welcome message style spinner
    idx = 0
    try:
        ...
        print(f"\r{' ' * (len(message_prefix) + 10)}\r", end="", flush=True) # Clear the spinner line

def _get_custom_portfolio_output_filepath(portfolio_code: str) -> str:
    """Generates the filepath for a custom portfolio's run output."""
    return os.path.join(PORTFOLIO_OUTPUT_DIR, f"custom_run_output_{portfolio_code.lower().replace(' ','_')}.json")

async def _save_custom_portfolio_run_output(portfolio_code: str, tailored_holdings: List[Dict], final_cash: float, is_called_by_ai: bool = False):
    """Saves the detailed tailored output of a custom portfolio run."""
    filepath = _get_custom_portfolio_output_filepath(portfolio_code)
    output_data = {
        "portfolio_code": portfolio_code,
        "timestamp_utc": datetime.now(pytz.UTC).isoformat(), # MODIFIED HERE
        "tailored_holdings": tailored_holdings,
        "final_cash_value": final_cash
    }
    try:
        with open(filepath, 'w', encoding='utf-8') as f:
            json.dump(output_data, f, indent=4)
        if not is_called_by_ai:
            print(f"Custom portfolio run output for '{portfolio_code}' saved to: {filepath}")
    except IOError as e:
        if not is_called_by_ai:
            print(f"Error saving custom portfolio run output for '{portfolio_code}' to {filepath}: {e}")
    except Exception as e_gen:
        if not is_called_by_ai:
             print(f"Unexpected error saving custom portfolio output for '{portfolio_code}': {e_gen}")

async def _load_custom_portfolio_run_output(portfolio_code: str, is_called_by_ai: bool = False) -> Optional[Dict]:
    """Loads the last saved detailed run output of a custom portfolio."""
    filepath = _get_custom_portfolio_output_filepath(portfolio_code)
    if not os.path.exists(filepath):
        if not is_called_by_ai:
            print(f"No saved run output found for portfolio '{portfolio_code}' at {filepath}.")
        return None
    try:
        ...
        if not is_called_by_ai:
            print(f"Error loading saved run output for '{portfolio_code}' from {filepath}: {e}")
        return None
...
async def process_custom_portfolio(
    ...
    Returns:
        ...
        - combined_result_intermediate_calc (List[Dict]): Intermediate combined data before final tailoring.
    ...
    # Assuming get_allocation_score, calculate_ema_invest, calculate_one_year_invest, plot_ticker_graph,
    # safe_score, generate_portfolio_pie_chart, and tabulate are defined elsewhere and function correctly.
    # Also assuming global constants like PORTFOLIO_DB_FILE etc. are defined.
    ...
    for i in range(num_portfolios):
        ...
        for ticker in tickers:
            try:
                ...
                if live_price is None and ema_invest is None: # Critical data missing
                ...
                live_price_val = 0.0 if live_price is None else live_price # Ensure live_price_val is float
                ...
                entry_data = {
                    ...
                    'amplified_score_original': amplified_score_original_clamped, # Store this for sell-to-cash comparison
                    'portfolio_weight': weight, 'score_was_adjusted': score_was_adjusted_flag,
                    'portfolio_allocation_percent_adjusted': None, # Will be calculated
                    'portfolio_allocation_percent_original': None, # Will be calculated
                    'combined_percent_allocation_adjusted': None, # Will be calculated
                    'combined_percent_allocation_original': None, # Will be calculated
                }
                current_portfolio_list_calc.append(entry_data)
                if live_price_val > 0: # Only plot if price is valid
                    all_entries_for_graphs_plotting.append({'ticker': ticker, 'ema_sensitivity': ema_sensitivity})
            except Exception as e_ticker_loop:
                # Log or handle ticker-specific error more gracefully if needed
    ...
    if not suppress_prints:
        for graph_entry in all_entries_for_graphs_plotting:
            ...
            if not ticker_key_graph or ticker_key_graph in sent_graphs: continue # Avoid duplicate graphs
            if 'error' not in graph_entry: # Don't try to plot if there was an error fetching data for it
    ...
    for portfolio_list_item in portfolio_results_list:
        ...
        for entry in portfolio_list_item:
            ...
            else: entry['portfolio_allocation_percent_original'] = None # Error case
    
    if not suppress_prints:
        print("\n--- Sub-Portfolio Details ---")
        for i, portfolio_list_print in enumerate(portfolio_results_list, 1):
            # Sort by the adjusted allocation for display
            ...
            for entry_print in portfolio_list_print:
                if 'error' not in entry_print:
                    ...
                    invest_score_val_f = safe_score(entry_print.get('raw_invest_score', 0)) # Assuming raw_invest_score is available
                    ...
                    # Display original portfolio allocation for context if sell-to-cash is active
    ...
    for portfolio_list_intermediate in portfolio_results_list:
        for entry_intermediate in portfolio_list_intermediate:
            if 'error' not in entry_intermediate:
                ...
                # Calculate combined % using adjusted sub-portfolio allocation
                ...
                # Also calculate what combined % would have been using original sub-portfolio allocation
    ...
    # This variable will store the percentage of the total portfolio that should go to Cash
    # due to the sell-to-cash feature adjusting individual stock scores.
    cash_allocation_from_sell_to_cash_feature_pct = 0.0

    for entry_comb in combined_result_intermediate_calc:
        # Add stock/asset entries to final_combined_portfolio_data_calc
        final_combined_portfolio_data_calc.append({
            ...
            'combined_percent_allocation': entry_comb['combined_percent_allocation_adjusted'] # This is the allocation to use for the stock
        })
        # Calculate the cash portion arising from the sell-to-cash feature
        if sell_to_cash_active and entry_comb.get('score_was_adjusted', False):
            # If a score was adjusted (floored at 50), the difference between its *original* potential allocation
            # and its *adjusted* (likely higher due to 50 score) allocation represents a shift.
            # The cash_allocation_from_sell_to_cash_feature_pct should sum up the amounts by which
            # stocks *would have been lower* if not for the 50 score floor, effectively representing
            # money shifted from very low-scoring assets to a general cash pool.

            # This was the original logic for total_cash_diff_percent_calc.
            # It sums positive differences: if adjusted_combined > original_combined.
            # This typically happens if a stock's score was very low, making original_combined low,
            # and then it was floored at 50, making adjusted_combined higher (relative to its previous state,
            # but other stocks might still dominate its sub-portfolio).
            # This sum is the *increase* in allocation some stocks received due to the 50 floor.
            # This isn't directly the "cash freed up".
            # Instead, the normalization process later will effectively determine the cash.

            # Simpler: total_cash_diff_percent_calc was the sum of (adj - orig) for those where score_was_adjusted.
            # For now, let's rely on the normalization steps after this loop to correctly establish the cash component.
            # The total_cash_diff_percent_calc needs to be the target percentage for cash.
            # If sell_to_cash is active, we determine the target percentage for cash, then normalize stocks.

            # Let's refine the meaning of cash_allocation_from_sell_to_cash_feature_pct.
            # It should be the sum of original allocations of stocks that are now being "sold to cash" (score < 50).
            # No, that's not right. It's how much of the *total portfolio* becomes cash.
            # The avg_score < 50 implies a general move to cash.
            # The original logic for total_cash_diff_percent_calc was:
            ...
            # This still seems like the intended original mechanism for deriving the cash part.


    # If sell_to_cash_active, adjust stock allocations and add a Cash component
    if sell_to_cash_active and cash_allocation_from_sell_to_cash_feature_pct > 0:
        # Cap the cash allocation at 100%
        cash_allocation_from_sell_to_cash_feature_pct = min(cash_allocation_from_sell_to_cash_feature_pct, 100.0)

        # The total percentage allocated to stocks should now be (100 - cash_allocation_pct)
        ...
        if current_total_stock_allocation_pct > 1e-9: # Avoid division by zero if no stocks somehow
        ...
        elif target_total_stock_allocation_pct == 0.0: # All to cash
            for item in final_combined_portfolio_data_calc:
                if item['ticker'] != 'Cash':
                    item['combined_percent_allocation'] = 0.0
        
        # Add the Cash component
        final_combined_portfolio_data_calc.append({
    ...
    # Final normalization pass to ensure everything sums to 100%
    # This handles any floating point inaccuracies or if sell_to_cash_active was false.
    ...
    if not math.isclose(current_total_allocation_final_norm, 100.0, abs_tol=0.01) and current_total_allocation_final_norm > 1e-9:
        ...
        # Ensure cash is not disproportionately inflated if it exists, and sum of stocks is preserved relative to each other
        cash_item_final = next((item for item in final_combined_portfolio_data_calc if item['ticker'] == 'Cash'), None)
        current_stock_sum_before_final_norm = sum(item.get('combined_percent_allocation', 0.0) for item in final_combined_portfolio_data_calc if item['ticker'] != 'Cash')
        ...
        for item_norm in final_combined_portfolio_data_calc:
            ...
            # Cash will be adjusted last to make up the difference to 100%
        ...
        if cash_item_final:
            ...
            cash_item_final['combined_percent_allocation'] = min(cash_item_final['combined_percent_allocation'], 100.0) # Cap at 100
        elif (100.0 - current_stock_sum_after_final_norm) > 1e-4: # If no cash item yet, but there should be a remainder
             final_combined_portfolio_data_calc.append({
                'ticker': 'Cash', 'live_price': 1.0, 'raw_invest_score': None,
    ...
    final_combined_portfolio_data_calc.sort(
        key=lambda x: x.get('raw_invest_score', -float('inf')) if x.get('ticker') != 'Cash' else -float('inf')-1, # Cash sorts last
        reverse=True
    )
    
    if not suppress_prints:
        print("\n**--- Final Combined Portfolio (Sorted by Raw Score)---**")
        # Determine if cash allocation resulted from sell-to-cash feature for display note
        ...
        if sell_to_cash_active and cash_present_in_final:
            print("*(Sell-to-Cash Active, resulting in cash allocation)*")
        elif sell_to_cash_active: # Feature was active but might not have resulted in cash if all scores were high
            print("*(Sell-to-Cash Active, but might not have resulted in explicit cash if all scores were high)*")
    ...
    if tailor_portfolio_requested:
        # total_value_singularity can be None or <=0 if not tailoring or invalid input handled by caller
        if total_value_singularity is None: # Not tailoring to a specific value
             if not suppress_prints:
                print("Info: Tailoring not performed as total_value_singularity is not specified.")
             # Return combined percentages without actual share counts or dollar values
             return [], final_combined_portfolio_data_calc, 0.0, []


        total_value_float_for_tailor = safe_score(total_value_singularity)
        if total_value_float_for_tailor <= 0 and not is_called_by_ai: # For AI, let it proceed if it sent 0, it might be intentional for some edge case test
             if not suppress_prints:
        ...
        for entry_tailoring in final_combined_portfolio_data_calc:
            if entry_tailoring['ticker'] == 'Cash':
                continue
            ...
            if final_stock_alloc_pct_tailor > 1e-9 and live_price_for_tailor > 0:
                target_dollar_alloc_for_ticker = total_value_float_for_tailor * (final_stock_alloc_pct_tailor / 100.0)
                shares_to_buy_tailor = 0.0
                try:
                    exact_shares_calculated = target_dollar_alloc_for_ticker / live_price_for_tailor
                    if frac_shares_singularity:
                        shares_to_buy_tailor = round(exact_shares_calculated, 1)
                    else:
                        shares_to_buy_tailor = float(math.floor(exact_shares_calculated))
                except ZeroDivisionError:
                    shares_to_buy_tailor = 0.0
                shares_to_buy_tailor = max(0.0, shares_to_buy_tailor)

                actual_money_allocated_this_ticker = shares_to_buy_tailor * live_price_for_tailor
                
                min_share_purchase_threshold = 0.1 if frac_shares_singularity else 1.0
                
                if shares_to_buy_tailor >= min_share_purchase_threshold:
                    actual_percent_of_total_portfolio_value = (actual_money_allocated_this_ticker / total_value_float_for_tailor) * 100.0 if total_value_float_for_tailor != 0 else 0.0
        ...
        if not suppress_prints and not is_custom_command_simplified_output:
            ...
            for item_table in current_tailored_entries_for_calc:
                ...
                if not frac_shares_singularity and 0 < item_table['shares'] < 1: shares_display_table = "0"
            ...
            # Calculate final cash percentage based on the possibly non-zero total_value_float_for_tailor
            final_cash_percent_display_val = (final_cash_value_tailored / total_value_float_for_tailor) * 100.0 if total_value_float_for_tailor != 0 else 0.0
            # No longer cap final_cash_percent_display_val if cash can be negative or > 100% of initial for some reason
            ...
            elif not tailored_portfolio_table_data_display_full: # Should at least have cash row
                print("Error: No data for tailored portfolio display.")
            else:
        ...
        # Only include cash in pie chart if it's positive
...
async def handle_custom_command(args: List[str], ai_params: Optional[Dict] = None, is_called_by_ai: bool = False):
    ...
    if ai_params: # AI Call
        ...
        if action == "run_existing_portfolio":
            ...
            try:
                ...
                if frac_shares_override_ai is not None: 
                    ...
                    frac_shares_final_run_ai = csv_frac_shares_str in ['true', 'yes'] # Updated logic
                ...
                await _save_custom_portfolio_run_output(portfolio_code_input, tailored_data_run, final_cash_value_run, is_called_by_ai=True)
                summary_for_ai = f"Analysis for custom portfolio '{portfolio_code_input}' completed. Detailed run output saved/overwritten. "
                if tailor_run_ai:
        ...
        if not args:
            ...
            print("Note: Running a portfolio (e.g. /custom MYPORT) now automatically saves/overwrites its detailed run output.")
            return None
        ...
        if portfolio_config_from_db_cli:
            try:
                ...
                frac_shares_setting_from_config_cli = csv_frac_shares_str_cli in ['true', 'yes'] # Updated logic
                ...
                _, _, final_cash_cli_run, tailored_data_cli_run = await process_custom_portfolio(
                    ...
                    is_custom_command_simplified_output=tailor_this_run_cli, # True if tailoring for simplified output
                    is_called_by_ai=False
                )
                await _save_custom_portfolio_run_output(portfolio_code_cli, tailored_data_cli_run, final_cash_cli_run, is_called_by_ai=False)
                print(f"\nCLI: Custom portfolio analysis for `{portfolio_code_cli}` complete. Detailed run output saved/overwritten.")
...
async def get_comparison_for_custom_portfolio(
    ...
    """
    Loads saved output for a custom portfolio, runs it fresh, and returns comparison.
    Does NOT overwrite saved data.
    ai_params is expected to contain: "portfolio_code",
                                     "value_for_assessment" (optional for fresh run tailoring),
                                     "use_fractional_shares_override" (optional bool for fresh run)
    """
    if not ai_params:
        return {"error": "ai_params not provided to get_comparison_for_custom_portfolio"}
    ...
    if not portfolio_code:
        return {"error": "portfolio_code not provided in ai_params for comparison."}

    value_for_assessment: Optional[float] = None
    if value_for_assessment_raw is not None:
        try:
            value_for_assessment = float(value_for_assessment_raw)
            if value_for_assessment <= 0: # Allow zero or negative if that's a valid use case for your logic
                # For comparison, usually positive value is expected for tailoring.
                # If not tailoring, value_for_assessment can remain None.
                if not is_called_by_ai:
                    print(f"Warning: 'value_for_assessment' ({value_for_assessment_raw}) is not positive. Fresh run might be untailored or use default.")
                # value_for_assessment = None # Keep it if <=0 is meaningful, otherwise set to None to prevent tailoring
        except ValueError:
            if not is_called_by_ai:
                print(f"Warning: Non-numeric 'value_for_assessment' ('{value_for_assessment_raw}') for comparison. Proceeding untailored or with default.")
            value_for_assessment = None

    response = {
        "portfolio_code": portfolio_code,
        "saved_output_summary": "No previously saved run output was found or it was invalid.",
        "saved_run_data": None,
        "fresh_run_summary": "Fresh run could not be completed or yielded no data.",
        "fresh_run_data": None,
        "comparison_details": "Comparison could not be fully performed.",
        "parameters_used_for_fresh_run": {}
    }

    saved_output_data_full = await _load_custom_portfolio_run_output(portfolio_code, is_called_by_ai=True)
    if saved_output_data_full:
        saved_holdings = saved_output_data_full.get("tailored_holdings", [])
        saved_cash = saved_output_data_full.get("final_cash_value", 0.0)
        response["saved_output_summary"] = (
            f"Previously saved run (from {saved_output_data_full.get('timestamp_utc', 'N/A')}): "
            f"{len(saved_holdings)} holdings, final cash ${saved_cash:,.2f}."
        )
        response["saved_run_data"] = {
            "timestamp_utc": saved_output_data_full.get("timestamp_utc", "N/A"),
            "holdings": saved_holdings,
            "final_cash": saved_cash,
            "is_valid": True
        }
    else:
        response["saved_run_data"] = {
            ...
            "holdings": [],
            "final_cash": 0.0,
            "message": "No valid previously saved run output found for this portfolio."
        }

    portfolio_config_for_fresh_run = None
    if not os.path.exists(PORTFOLIO_DB_FILE): # PORTFOLIO_DB_FILE must be globally defined
        response["fresh_run_summary"] = f"Error: Portfolio configuration database '{PORTFOLIO_DB_FILE}' not found. Cannot perform fresh run."
        response["comparison_details"] = f"{response['saved_output_summary']} {response['fresh_run_summary']}"
        return response

    try:
        with open(PORTFOLIO_DB_FILE, 'r', encoding='utf-8', newline='') as f_db_fresh: # PORTFOLIO_DB_FILE
            ...
            for row_fresh in reader_fresh:
                if row_fresh.get('portfolio_code','').strip().lower() == portfolio_code.lower():
                    portfolio_config_for_fresh_run = row_fresh
                    break
        if not portfolio_config_for_fresh_run:
            response["fresh_run_summary"] = f"Error: Portfolio code '{portfolio_code}' not found in database. Cannot perform fresh run."
            response["comparison_details"] = f"{response['saved_output_summary']} {response['fresh_run_summary']}"
            return response
        ...
        # Tailor if value_for_assessment is a positive number
        tailor_fresh_run_flag = isinstance(value_for_assessment, (int, float)) and value_for_assessment > 0
        total_value_for_fresh_run = value_for_assessment if tailor_fresh_run_flag else None
        
        response["parameters_used_for_fresh_run"] = {
            "value": total_value_for_fresh_run,
            "frac_shares": frac_shares_for_fresh_run,
            "tailored": tailor_fresh_run_flag,
            "ema_sensitivity": portfolio_config_for_fresh_run.get('ema_sensitivity'),
            "amplification": portfolio_config_for_fresh_run.get('amplification')
        }

        _, _, final_cash_fresh_run, structured_data_fresh_run = await process_custom_portfolio(
            ...
            total_value_singularity=total_value_for_fresh_run,
            is_custom_command_simplified_output=True,
            is_called_by_ai=True
        )
        response["fresh_run_summary"] = (
            f"Fresh run (Value: {'${:,.2f}'.format(total_value_for_fresh_run) if total_value_for_fresh_run else 'Untailored'}, "
            f"FracShares: {frac_shares_for_fresh_run}): "
            f"{len(structured_data_fresh_run)} holdings, final cash ${final_cash_fresh_run:,.2f}."
        )
        response["fresh_run_data"] = {
            "timestamp_utc": datetime.now(pytz.UTC).isoformat(),
            "holdings": structured_data_fresh_run,
            "final_cash": final_cash_fresh_run,
            "parameters_used": response["parameters_used_for_fresh_run"],
            "is_valid": True
        }
    except Exception as e_fresh_run:
        response["fresh_run_summary"] = f"Error during fresh run of '{portfolio_code}' for comparison: {e_fresh_run}"
        response["fresh_run_data"] = {
            "is_valid": False,
            "holdings": [],
            "final_cash": 0.0,
            "error_message": str(e_fresh_run)
        }

    response["comparison_details"] = (
        f"Comparison for portfolio '{portfolio_code}'. "
        f"Saved Data Status: {response['saved_output_summary']}. "
        f"Fresh Run Status: {response['fresh_run_summary']}."
    )
    return response

# --- Breakout Command Functions --- (Assumed mostly unchanged, add is_called_by_ai flags)
async def run_breakout_analysis_singularity(is_called_by_ai: bool = False) -> list:
    """
    Performs breakout analysis, prints results to console (if not called by AI),
    saves current data to CSV, and returns a list of dictionaries of top breakout stocks.
    """
    if not is_called_by_ai:
    ...
    max_results_for_ai = 10 # Max detailed results to return to AI

    updated_data_for_return = [] # This will hold data for AI return or full CLI processing
    ...
    if os.path.exists(BREAKOUT_TICKERS_FILE):
        try:
            ...
            if not df_existing.empty:
                # Clean numeric columns (important for correct logic later)
                for col in ["Highest Invest Score", "Lowest Invest Score", "Live Price", "1Y% Change", "Invest Score"]:
                    if col in df_existing.columns:
                        if df_existing[col].dtype == 'object': # If read as string
    ...
    try:
        # Make sure Query and Column are imported from tradingview_screener
        query = Query().select('name', 'close', 'change', 'volume', 'market_cap_basic', 'change|1W', 'average_volume_90d_calc') \
            .where(
                Column('market_cap_basic') >= 1_000_000_000, # 1B Mkt Cap
                Column('volume') >= 1_000_000,             # 1M Volume
                Column('change|1W') >= 20,                 # 20% change in 1 week
                Column('close') >= 1,                      # Price >= $1
                Column('average_volume_90d_calc') >= 1_000_000 # Avg Vol 1M
            ).order_by('change', ascending=False).limit(100) # Top 100 by daily change

        # get_scanner_data is synchronous, run in thread for async context
        ...
        if scanner_results_tuple and isinstance(scanner_results_tuple, tuple) and len(scanner_results_tuple) > 0 and isinstance(scanner_results_tuple[1], pd.DataFrame):
            ...
            if 'name' in new_tickers_df.columns:
                # Extract ticker symbol (e.g., 'NASDAQ:AAPL' -> 'AAPL')
                new_tickers_from_screener = [str(t).split(':')[-1].replace('.', '-') for t in new_tickers_df['name'].tolist() if pd.notna(t)]
                new_tickers_from_screener = sorted(list(set(new_tickers_from_screener))) # Unique and sorted
                if not is_called_by_ai:
        ...
        if not is_called_by_ai:
            ...
            # traceback.print_exc() # Optionally print traceback for CLI
    ...
    temp_updated_data_for_df_build = [] # To build the new breakout_tickers.csv
    ...
    for ticker_b in all_tickers_to_process:
        ...
        try:
            # Use is_called_by_ai to suppress prints in these helper functions
            live_price_raw, current_invest_score_raw = await calculate_ema_invest(ticker_b, 2, is_called_by_ai=True) # Daily sensitivity
            ...
            # Ensure correct handling of potentially missing or non-numeric scores from existing file
            ...
            # Logic to remove ticker (from original script)
            remove_ticker = False
            if current_invest_score is None: # No current score, likely data issue
                remove_ticker = True
            elif current_highest_score > -float('inf') and current_highest_score > 0: # If there's a valid history
                if (current_invest_score > 600 or # Score too high (likely error or extreme outlier)
                    current_invest_score < invest_score_threshold or # Below minimum threshold
                    current_invest_score < fraction_threshold * current_highest_score): # Dropped too much from peak
                    remove_ticker = True
            elif current_invest_score < invest_score_threshold: # No history, and current score too low
                remove_ticker = True
            ...
            if not remove_ticker:
                ...
                stock_data_dict = {
                    ...
                    "Status": status,
                    "_sort_score_internal": current_invest_score if current_invest_score is not None else -float('inf') # For sorting
                }
            ...
            if not is_called_by_ai:
                ...
                # traceback.print_exc() # Optional for CLI debug

    # Sort data by current invest score (descending) before saving and for AI return
    temp_updated_data_for_df_build.sort(key=lambda x: x['_sort_score_internal'], reverse=True)

    # Prepare data for DataFrame (remove internal sort key) and for AI return
    df_data_for_csv_and_return = []
    for item_dict in temp_updated_data_for_df_build:
        # Create a new dict without the internal sort key for CSV and AI return
    ...
    if df_data_for_csv_and_return:
        ...
        if not is_called_by_ai:
            ...
            actual_display_cols = [col for col in display_cols_cli if col in final_df_to_save.columns]
            if actual_display_cols:
                 print(tabulate(final_df_to_save[actual_display_cols].head(15), headers="keys", tablefmt="pretty", showindex=False))
            else:
        ...
        if not is_called_by_ai:
            ...
            # Optionally, clear the BREAKOUT_TICKERS_FILE if no tickers qualify
            try:
                open(BREAKOUT_TICKERS_FILE, 'w').close() # Creates an empty file or truncates existing
    ...
    # For AI, return the top N results (structured data)
    # For CLI, the full data is saved, and top N printed. This function can return all for CLI if needed.
    if is_called_by_ai:
        if not df_data_for_csv_and_return:
            # print("Breakout analysis (AI call) did not yield any tickers.") # Already handled by AI handler
            return [] # Return empty list if no tickers
        return df_data_for_csv_and_return[:max_results_for_ai]
    else:
        return df_data_for_csv_and_return # CLI path can also return the full data if other parts of script use it
...
async def handle_breakout_command(args: List[str], ai_params: Optional[Dict] = None, is_called_by_ai: bool = False):
    if not is_called_by_ai: print("\n--- /breakout Command ---")
    if ai_params: # AI call, expected to be for saving
        date_to_save_ai = ai_params.get("date_to_save")
        if not date_to_save_ai: return "Error for AI (/breakout save): 'date_to_save' required."
        try: datetime.strptime(date_to_save_ai, '%m/%d/%Y')
        except ValueError: return f"AI Error: Invalid date '{date_to_save_ai}'. Use MM/DD/YYYY."
        return await save_breakout_data_singularity(date_to_save_ai, is_called_by_ai=True)
    else: # CLI Path
        if args and args[0] == "3725":
            date_cli = input("CLI: Enter date (MM/DD/YYYY) to save breakout data: ")
            try: datetime.strptime(date_cli, '%m/%d/%Y'); await save_breakout_data_singularity(date_cli, is_called_by_ai=False)
            except ValueError: print("CLI: Invalid date format.")
        else:
            await run_breakout_analysis_singularity(is_called_by_ai=False)
        return None # CLI prints directly or called functions do
...
async def calculate_percentage_above_ma_risk(symbols: List[str], ma_window: int, is_called_by_ai: bool = False) -> float:
    """Calculates percentage of symbols above MA for RISK module, processing symbols concurrently."""
    if not symbols: return 0.0
    above_ma_count = 0
    valid_stocks_count = 0

    # Create tasks for concurrent execution
    tasks = [asyncio.to_thread(calculate_ma_risk, symbol, ma_window, is_called_by_ai=True) for symbol in symbols]
    results = await asyncio.gather(*tasks) # gather will run them "concurrently" via threads

    for result in results:
        if result is not None:
            valid_stocks_count += 1
            if result: # True if above MA
                above_ma_count += 1

    if valid_stocks_count == 0:
        risk_logger.warning(f"MA({ma_window}): No valid stocks found to calculate percentage above MA.")
        return 0.0
...
def calculate_ema_score_risk(ticker:str ="SPY", is_called_by_ai: bool = False) -> Optional[float]:
    ...
    try:
        ...
        data['Close'] = pd.to_numeric(data['Close'], errors='coerce')
        if data['Close'].isnull().all(): return None
        data['Close'].ffill(inplace=True) # Fill NaNs before EMA calculation
...
market_command_tool = FunctionDeclaration(
    ...
    parameters={"type": "object", "properties": {
        ...
        "date_str": {"type": "string", "description": "Required for 'save' action: Date in MM/DD/YYYY format. If user says 'today', use current date."} # System prompt will fill current date
...
custom_command_tool = FunctionDeclaration(
    ...
    description="Manages custom portfolios. Action 'run_existing_portfolio' runs an existing saved portfolio configuration (can be tailored with 'total_value', 'use_fractional_shares') and automatically saves/overwrites its detailed run output. Action 'save_portfolio_data' saves *legacy combined percentage data* for an existing portfolio (requires 'date_to_save'). Requires 'portfolio_code' for both actions. Returns a summary.",
    parameters={"type": "object", "properties": {
        "action": {"type": "string", "description": "Specify 'run_existing_portfolio' to analyze it and save its run output, or 'save_portfolio_data' for legacy combined % save.", "enum": ["run_existing_portfolio", "save_portfolio_data"]},
        ...
        "use_fractional_shares": {"type": "boolean", "description": "For 'run_existing_portfolio' with tailoring: Optional. If true, fractional shares will be used if the portfolio configuration allows or this overrides it."},
        "date_to_save": {"type": "string", "description": "For 'save_portfolio_data' (legacy save): Date MM/DD/YYYY. If user says 'today', use current date."} # System prompt fills current date
    }, "required": ["action", "portfolio_code"]}
)

save_breakout_data_tool = FunctionDeclaration(
    name="save_breakout_stock_data", # This name must match a key in AVAILABLE_PYTHON_FUNCTIONS
    description="Saves the data from the most recent breakout analysis run (from breakout_tickers.csv) to a historical database for a specified date. Ensure breakout analysis (`run_breakout_analysis_singularity`) is run first if current data is needed for saving. Returns a success/failure message string.",
    parameters={"type": "object", "properties": {
        "date_to_save": {"type": "string", "description": "Date MM/DD/YYYY. If user says 'today', use current date."} # System prompt fills current date
...
cultivate_analysis_tool = FunctionDeclaration(
    ...
    parameters={"type": "object", "properties": {
        ...
        "date_to_save": {"type": "string", "description": "Optional. Date (MM/DD/YYYY) to save data for. Required if 'action' is 'save_data'. If 'today', use current date."} # System prompt fills
...
get_comparison_custom_portfolio_tool = FunctionDeclaration(
    name="get_comparison_for_custom_portfolio",
    description="Loads a previously saved run output for a custom portfolio, runs the portfolio analysis fresh (optionally for a new value or share preference), and returns a comparison of the saved vs. fresh results. This tool DOES NOT overwrite the saved run output data by itself. To overwrite, explicitly ask to run and save the portfolio again using 'handle_custom_command'.",
    parameters={"type":"object", "properties":{
        "portfolio_code":{"type":"string","description":"The code/name of the custom portfolio to compare."},
        "value_for_assessment":{"type":"number","description":"Optional. The monetary value to use for the fresh run of the portfolio if tailoring is desired for the comparison. If not provided, fresh run might be untailored or use a default/saved value."},
        "use_fractional_shares_override":{"type":"boolean","description":"Optional. For the fresh run: true/false to override the portfolio's default fractional share setting for the comparison."}
...
AVAILABLE_PYTHON_FUNCTIONS: dict[str, callable] = {
    ...
    "handle_history_command": handle_history_command, # For R.I.S.K. graphs
    "handle_custom_command": handle_custom_command,
    "save_breakout_stock_data": handle_breakout_command, # Note: Maps to handle_breakout_command for save action
...
async def handle_ai_prompt(user_new_message: str, is_new_session: bool = False, original_session_request: Optional[str] = None):
    ...
    if not gemini_model:
        # This print will go to console, but AI interaction won't proceed.
    ...
    all_gemini_tools = Tool(function_declarations=[ # Ensure this list is COMPLETE and matches AVAILABLE_PYTHON_FUNCTIONS keys
        run_breakout_tool, quickscore_tool, market_command_tool, risk_assessment_tool,
        generate_history_graphs_tool, custom_command_tool, save_breakout_data_tool,
        cultivate_analysis_tool, invest_analysis_tool, handle_assess_tool,
        get_comparison_custom_portfolio_tool # Ensure this and all others are defined FunctionDeclaration objects
    ])

    if is_new_session:
        AI_CONVERSATION_HISTORY.clear()
        AI_INTERNAL_STEP_COUNT = 0
        CURRENT_AI_SESSION_ORIGINAL_REQUEST = original_session_request
        base_system_prompt_template = load_system_prompt() # Ensure this function is defined
        current_date_val = datetime.now().strftime('%B %d, %Y')
        current_date_mmddyyyy_val = datetime.now().strftime('%m/%d/%Y')
        try:
            formatted_system_prompt_text = base_system_prompt_template.format(
                current_date_for_ai_prompt=current_date_val,
                current_date_mmddyyyy_for_ai_prompt=current_date_mmddyyyy_val
            )
        except KeyError as e_format_sys:
            print(f"KeyError during system prompt formatting: {e_format_sys}. Check system_prompt.txt or default.")
            # This error should ideally not happen if prompt is correct.
            # If it does, AI will not have its proper system instructions.
            return "Error: System prompt formatting failed due to missing keys."
        AI_CONVERSATION_HISTORY.append({"role": "user", "parts": [{"text": formatted_system_prompt_text}]})
        AI_CONVERSATION_HISTORY.append({"role": "model", "parts": [{"text": "Okay, I understand my role and guidelines. How can I assist you today?"}]})

    AI_CONVERSATION_HISTORY.append({"role": "user", "parts": [{"text": user_new_message}]})

    max_internal_turns = 7
    executed_tool_calls_in_current_interaction = set()
    functions_were_called_in_this_interaction = False
    final_text_response_from_ai = "" # Accumulates the AI's textual response if no tools are called or after summarization
    repeat_detected_in_loop = False
    tool_name_that_repeated = "" # Store name of the tool that AI tried to repeat

    # --- Continuous Spinner for the entire AI processing of this user message ---
    stop_main_spinner_event = asyncio.Event()
    # Use a generic message for the main spinner
    main_spinner_task = asyncio.create_task(_continuous_spinner_animation(stop_main_spinner_event, "AI is processing request..."))
    # ---

    try: # This try block covers the entire multi-turn interaction for the current user message
        for turn_num in range(max_internal_turns):
            # Spinner is already running in the background; AI_INTERNAL_STEP_COUNT is for logical steps, not animation timing.
            # AI_INTERNAL_STEP_COUNT = turn_num + 1 # This was for per-step animation, not needed with continuous spinner
            ...
            try:
                ...
                # This is an API call error, not a tool execution error.
                final_text_response_from_ai = f"Gemini API Error: {e_gen_content_api}"
                # No direct print to console here; will be part of final AI output.
                # Log this error if needed using risk_logger or similar.
                # risk_logger.error(f"Gemini API call failed: {e_gen_content_api}")
                break # Exit tool processing loop for this user message
                ...
                for part_object in gemini_candidate.content.parts:
                    if part_object.function_call and part_object.function_call.name:
                        ...
                        model_turn_history_parts_to_add.append({
                            "function_call": {"name": fc_obj.name, "args": dict(fc_obj.args)}
                        })
                        break # Process first function call found in parts
                    elif part_object.text:
                        text_from_model_in_turn += part_object.text
            if not function_call_to_run_this_turn and text_from_model_in_turn.strip():
                model_turn_history_parts_to_add = [{"text": text_from_model_in_turn.strip()}]

            if model_turn_history_parts_to_add:
                AI_CONVERSATION_HISTORY.append({"role": "model", "parts": model_turn_history_parts_to_add})
            else: # Model turn was empty or unhandled type
                final_text_response_from_ai = "AI model response part was empty or contained unhandled content."
                break # Stop processing if AI gives an unusable response
            ...
            if function_call_to_run_this_turn:
                ...
                # No print here: print(f"AI plans to use tool: {tool_name_current}")

                current_call_signature_tuple = (tool_name_current, frozenset(tool_args_dict_current.items()))
                if current_call_signature_tuple in executed_tool_calls_in_current_interaction:
                    tool_name_that_repeated = tool_name_current
                    error_msg_for_history = f"System detected an attempt by AI to re-run tool '{tool_name_current}' with identical parameters within the same user request processing. This specific tool call was aborted to prevent a loop. AI should now summarize previous successful actions or propose a different next step."
                    # No console print for this error; it's for AI context.
                    ...
                    final_text_response_from_ai = f"Processing halted: AI attempted a repetitive action for '{tool_name_current}'. Attempting to summarize progress." # Interim, might be overwritten
                ...
                if tool_name_current in AVAILABLE_PYTHON_FUNCTIONS:
                    ...
                    # No print here: print(f"Executing: {tool_name_current}...")
                    tool_result_from_execution = f"Error: Tool '{tool_name_current}' did not return a value."
                    try:
                        ...
                        tool_result_from_execution = f"Error during execution of tool '{tool_name_current}': {str(e_tool_execution_error)}"
                        traceback.print_exc() # Log full traceback for server-side debugging

                    # No print here: print(f"Execution of {tool_name_current} completed.")
                    response_payload_for_history = {}
                    if isinstance(tool_result_from_execution, dict): response_payload_for_history = tool_result_from_execution
                    elif isinstance(tool_result_from_execution, str): response_payload_for_history = {"summary": tool_result_from_execution}
                    elif tool_result_from_execution is None: response_payload_for_history = {"status": "success", "message": f"Tool '{tool_name_current}' executed and returned None (no explicit output)."}
                    else: response_payload_for_history = {"result": str(tool_result_from_execution)}
                    try: # Truncate if too long for history
                        if len(json.dumps(response_payload_for_history)) > 4000: # Adjust limit as needed
                            response_payload_for_history = {"summary_truncated": "Tool output was too long for history log.", "original_type": str(type(tool_result_from_execution))}
                    except TypeError: response_payload_for_history = {"summary": str(response_payload_for_history)[:1000] + "... (truncated non-JSON serializable data)"}
                ...
                else: # AI proposed a function not in our list
                    error_unknown_tool_msg = f"Error: AI proposed an unknown function '{tool_name_current}'."
                    ...
                    break # Stop if AI is making up tools
            elif text_from_model_in_turn.strip(): # AI provided a direct text response
                final_text_response_from_ai = text_from_model_in_turn.strip()
                break # End tool loop, AI is directly answering or asking for clarification
            else: # Should not happen: no function call, no text
                final_text_response_from_ai = "AI response was empty or format not recognized."
                break
        else: # Max internal turns reached
            if functions_were_called_in_this_interaction and not text_from_model_in_turn.strip():
                final_text_response_from_ai = f"Max processing turns ({max_internal_turns}) reached. Will attempt to summarize based on actions taken."
            elif not final_text_response_from_ai: # Loop ended, no tools, no text response yet
                final_text_response_from_ai = "Max processing turns reached without a conclusive AI response."
            # No console print for max turns here, spinner is active.

    finally: # Ensure spinner stops and clears its line
        stop_main_spinner_event.set()
        if 'main_spinner_task' in locals() and main_spinner_task:
            try: await main_spinner_task
            except asyncio.CancelledError: pass # Expected if task is cancelled
        # print(f"\r{' ' * 70}\r", end="", flush=True) # Clear line after spinner is done by spinner itself

    # --- Final Synthesis Step ---
    # Trigger if:
    # 1. Tools were called AND no direct final text response was given by AI during the tool loop.
    # OR
    # 2. Tools were called, but the loop was broken by a repeat detection (we still want to summarize successes).
    needs_explicit_summary_check = (functions_were_called_in_this_interaction and not final_text_response_from_ai) or \
                                   (functions_were_called_in_this_interaction and repeat_detected_in_loop)
    ...
    if needs_explicit_summary_check:
        ...
        if repeat_detected_in_loop:
            # The error about the repeat is already in the conversation history for AI to see.
            summarization_prompt_text_for_ai = (
                f"A repetitive tool call for '{tool_name_that_repeated}' was proposed by you and subsequently aborted by the system. "
                f"Please review our entire conversation history, focusing on my original request ('{true_original_request_for_summary_ctx}') "
                f"and any function calls that were successfully executed *before* this repetition error occurred for the user's last message ('{user_new_message}'). "
                f"Provide a concise, final, textual summary of what was achieved successfully based on those prior successes. "
                f"Do not propose new tools. Acknowledge the stoppage if relevant to the summary."
            )
            final_text_response_from_ai = "" # Clear interim error message from repeat detection
        else: # Standard summarization after a successful tool sequence
            summarization_prompt_text_for_ai = (
                f"The tool execution phase for my request ('{user_new_message}') appears complete. "
                f"Please review our conversation history, particularly my original request for this session ('{true_original_request_for_summary_ctx}'), "
                f"and all function call results. Provide a concise, final, textual answer based on the gathered information. Do not call any new functions."
            )
        AI_CONVERSATION_HISTORY.append({"role": "user", "parts": [{"text": summarization_prompt_text_for_ai}]})

        # Start spinner again for the final summarization API call
        ...
        try:
            summary_gemini_response_obj = await asyncio.to_thread(
                gemini_model.generate_content,
                contents=AI_CONVERSATION_HISTORY, tools=None, # No tools for summarization
            ...
            if summary_candidate_obj and summary_candidate_obj.content and summary_candidate_obj.content.parts:
                for summary_part_obj in summary_candidate_obj.content.parts:
                    if summary_part_obj.text: synthesized_summary_text_from_ai += summary_part_obj.text
            ...
            else: # AI gave no text for summary
                if not final_text_response_from_ai: # If no prior message (like repeat error already set)
                    final_text_response_from_ai = "AI completed processing but did not provide a final textual summary when explicitly asked."
                # If final_text_response_from_ai had the repeat error and summary is empty, it will retain the repeat error message.
                AI_CONVERSATION_HISTORY.append({"role": "model", "parts": [{"text": final_text_response_from_ai or "AI summary was empty."}]})
        except Exception as e_summary_api_call:
            summary_api_error_msg = f"An error occurred during the AI summarization API call: {e_summary_api_call}"
            if not final_text_response_from_ai or repeat_detected_in_loop: # If original error was repeat, or no text yet
                final_text_response_from_ai = summary_api_error_msg # Prioritize this new error
            AI_CONVERSATION_HISTORY.append({"role": "model", "parts": [{"text": final_text_response_from_ai}]})
        finally:
            ...
            # print(f"\r{' ' * 70}\r", end="", flush=True) # Spinner clears its own line

    # --- Display Final AI Response (printed by main_singularity loop after this function returns) ---
    # For console applications, the final response is usually printed by the caller.
    # This function will return the final_text_response_from_ai to be printed by main_singularity.
    # However, your original request was to print it here.
    print("\n--- AI's Final Synthesized Answer ---")
    print(final_text_response_from_ai if final_text_response_from_ai else "No final textual response was generated by the AI for this interaction.")
    print("-------------------------------------\n")

    return "AI processing complete." # Or return final_text_response_from_ai
...
async def main_singularity():
    ...
    if 'risk_logger' not in globals() or not isinstance(risk_logger, logging.Logger) or not risk_logger.hasHandlers():
        print("CRITICAL: risk_logger was not properly initialized at the global scope. Exiting.")
        # You might want to attempt a very basic emergency log here if possible, or just exit.
        # For example, to a simple file:
        # with open("emergency_log.txt", "a") as f:
        # f.write(f"{datetime.now()}: CRITICAL - risk_logger not initialized.\n")
        return # Exit if logger isn't ready

    risk_logger.info("Singularity Application Main Loop Started.")


    # --- Ensure portfolio_outputs directory exists ---
    # ensure_portfolio_output_dir() # Call this if defined, or implement os.makedirs here

    display_welcome_message() # Ensure this function is defined
    display_commands()    # Ensure this function is defined

    while True:
        try:
            # Determine prompt based on AI chat state
            ...
            # Command to end an active AI chat session without exiting the application
            ...
            if command == "/ai":
                ...
                elif not gemini_model: # Check if Gemini model is available
                    print("Gemini API model is not available. Cannot process AI request.")
                else:
                    is_new_ai_session = not AI_CONVERSATION_HISTORY # Start new if history is empty
                    if is_new_ai_session:
                        print("No active AI chat. Starting a new session with this /ai prompt...")
                        CURRENT_AI_SESSION_ORIGINAL_REQUEST = user_natural_prompt # Store as original request for this new session
                        AI_INTERNAL_STEP_COUNT = 0 # Reset step count for the new session's multi-tool operations
                    # Pass the stored original request for context if continuing a session,
                    # otherwise, the current prompt is the "original" for this new context.
            ...
            # Ensure these functions are defined to take `args` and `is_called_by_ai=False` for CLI calls.
            # The `ai_params=None` is for functions that can also be called by AI with specific parameters.
            ...
            elif command == "/history": await handle_history_command(args, ai_params=None, is_called_by_ai=False) # R.I.S.K. graphs
            # Utility Commands
                ...
                # --- Follow-up AI Chat (if input doesn't start with '/' and a session is active) ---
                if AI_CONVERSATION_HISTORY and not user_input_full_line.startswith("/"):
                    # This is a follow-up message in an active AI chat session
                    # print(f"Continuing AI chat with: '{user_input_full_line}'") # User sees their input already
                    await handle_ai_prompt(
                        user_input_full_line,
                        is_new_session=False, # Not a new session
                        original_session_request=CURRENT_AI_SESSION_ORIGINAL_REQUEST # Pass the original request of this session
                    )
                else: # Unknown command if not a follow-up to AI
            ...
            CURRENT_AI_SESSION_ORIGINAL_REQUEST = None # Clear on exit
            AI_INTERNAL_STEP_COUNT = 0
            break
        except Exception as e_main_loop:
            print(f"An unexpected error occurred in the main loop: {e_main_loop}")
            risk_logger.exception("Unexpected error in main_singularity loop:") # Logs exception info
            traceback.print_exc() # Print traceback to console for immediate debugging
            # Optionally, reset AI state on major errors if desired, or attempt to continue if safe
            # For now, we just log and the loop continues to the next input prompt.
            # If the error is critical, the user might need to restart or use /exit.
            # Consider if AI state should be cleared on unhandled main loop errors:
            # AI_CONVERSATION_HISTORY.clear()
            # CURRENT_AI_SESSION_ORIGINAL_REQUEST = None
            # AI_INTERNAL_STEP_COUNT = 0
