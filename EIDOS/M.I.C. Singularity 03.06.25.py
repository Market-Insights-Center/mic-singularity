...
gemini_model: Optional[genai.GenerativeModel] = None # Must be initialized
...
CURRENT_AI_SESSION_ORIGINAL_REQUEST = None # ADD THIS GLOBAL
...
def load_system_prompt(file_path="system_prompt.txt") -> str:
    ...
    # Ensure all literal curly braces in this default_prompt are doubled if they are part of the text,
    # e.g., "This is an example: {{{{some_example_key}}}}:{{{{some_example_value}}}}"
    # Placeholders for .format() should remain single, like {current_date_for_ai_prompt}.
...
"""
    try:
        ...
        try:
            ...
            return default_prompt # Return the raw default prompt
    except Exception as e:
...
async def handle_ai_prompt(user_new_message: str, is_new_session: bool = False, original_session_request: Optional[str] = None):
    ...
    Sends user messages, handles tool calls, prevents identical reruns within the same user message processing,
    and ensures a final summary is generated.
    """
    global AI_CONVERSATION_HISTORY, gemini_model, AVAILABLE_PYTHON_FUNCTIONS

    print(f"\nUser AI Message: '{user_new_message}'") # For logging user's current message

    if not gemini_model: # Basic checks
    ...
    # --- Define All Tools Gemini Can Use ---
    # These should match the keys in AVAILABLE_PYTHON_FUNCTIONS and your system prompt's tool list
    ...
    if is_new_session:
        ...
        # Ensure load_system_prompt() is defined and accessible
        ...
        try:
            ...
            print(f"KeyError during system prompt formatting: {e_format}. Check system_prompt.txt or default_prompt string.")
            traceback.print_exc()
            return "Error: System prompt formatting failed."

        AI_CONVERSATION_HISTORY.append({"role": "user", "parts": [{"text": formatted_system_prompt_text}]})
        AI_CONVERSATION_HISTORY.append({"role": "model", "parts": [{"text": "Okay, I understand my role and guidelines. How can I help you today?"}]})
        AI_CONVERSATION_HISTORY.append({"role": "user", "parts": [{"text": user_new_message}]})
    else:
        if not AI_CONVERSATION_HISTORY:
            print("Warning: Continuing AI chat, but history is empty. Forcing new session.")
            return await handle_ai_prompt(user_new_message, is_new_session=True, original_session_request=user_new_message)
        AI_CONVERSATION_HISTORY.append({"role": "user", "parts": [{"text": user_new_message}]})


    max_internal_turns = 7
    # last_proposed_function_call_details = None # No longer needed for immediate repeat check
    executed_tool_calls_in_current_interaction = set() # MODIFIED: Tracks all unique calls for this user_new_message
    functions_called_this_interaction = False
    text_response_from_gemini_in_loop = ""

    for turn in range(max_internal_turns):
        print(f"\n--- AI Internal Turn {turn + 1}/{max_internal_turns} for current user message ---")
        if not AI_CONVERSATION_HISTORY:
            print("Critical Error: AI Conversation history became empty mid-processing.")
            break
        ...
        try:
            print(f"Sending to Gemini (history items: {len(AI_CONVERSATION_HISTORY)})...")
            ...
            if not candidate:
                print("No candidate in Gemini's response. Ending turn.")
                break
            ...
            print(f"AI Finish Reason for this turn: {current_finish_reason}")

            if current_finish_reason == "MALFORMED_FUNCTION_CALL":
                malformed_call_info = "Malformed call details not available from candidate."
                if hasattr(candidate, 'content') and candidate.content and candidate.content.parts:
                     for p_proto_err in candidate.content.parts:
                        if hasattr(p_proto_err, 'function_call') and p_proto_err.function_call.name:
                            ...
                            break
                print(f"Error: Gemini attempted a function call but it was malformed. Details: {malformed_call_info}")
                AI_CONVERSATION_HISTORY.append({"role": "model", "parts": [{"text": f"My previous attempt to call a function resulted in a malformed structure. Details: {malformed_call_info}. I will re-evaluate."}]})
                break
            ...
            if model_response_content and model_response_content.parts:
                for p_proto in model_response_content.parts:
                    if hasattr(p_proto, 'function_call') and p_proto.function_call.name:
                        ...
                        break
                    elif hasattr(p_proto, 'text') and p_proto.text:
                        text_response_from_gemini_in_loop += p_proto.text

                if not function_call_to_execute and text_response_from_gemini_in_loop.strip():
                    model_turn_parts_for_history = [{"text": text_response_from_gemini_in_loop}]

            if model_turn_parts_for_history:
                AI_CONVERSATION_HISTORY.append({"role": "model", "parts": model_turn_parts_for_history})
            elif not function_call_to_execute and not text_response_from_gemini_in_loop.strip():
            ...
            if function_call_to_execute:
                ...
                print(f"Gemini proposes to call Function: {function_name}")
                print(f"With Arguments: {json.dumps(args_dict_from_ai, indent=2)}")

                # MODIFIED: Use a hashable representation for the call details
                # frozenset makes the dict items hashable for set storage
                current_proposed_call_signature = (function_name, frozenset(args_dict_from_ai.items()))

                if current_proposed_call_signature in executed_tool_calls_in_current_interaction:
                    repeated_call_error_msg = (
                        f"Error: I am attempting to re-run the function '{function_name}' "
                        f"with the exact same arguments ({args_dict_from_ai}) which has already been "
                        f"attempted or executed in the current interaction for this user message. "
                        "This indicates a potential loop or that the previous attempt was not satisfactory. "
                        "I will stop this specific action and await summarization or new instructions."
                    )
                    print(f"CRITICAL WARNING: AI attempting to re-run a previously executed/attempted call in this interaction: {function_name} with args {args_dict_from_ai}. Forcing break from tool loop.")
                    ...
                    text_response_from_gemini_in_loop = "Internal tool loop was stopped due to a detected re-run of a previously attempted function call for this user message."
                    functions_called_this_interaction = True # Mark that an interaction (even faulty proposal) happened
                    break # Break the `for turn in range(max_internal_turns)` loop IMMEDIATELY
                
                # If not a repeat for this interaction, add it to the set of attempted calls
                executed_tool_calls_in_current_interaction.add(current_proposed_call_signature)
                ...
                if function_name in AVAILABLE_PYTHON_FUNCTIONS:
                    ...
                    function_result_value = f"Error: Function '{function_name}' did not return an expected value."
                    ...
                    try:
                        ...
                        # --- Argument Preparation Dispatcher ---
                        if function_name == "handle_quickscore_command":
                            ...
                            else: raise ValueError("Ticker not provided for quickscore by AI.")
                        elif function_name == "handle_market_command":
                            action_arg = args_dict_from_ai.get("action")
                            s_arg = args_dict_from_ai.get("sensitivity")
                            d_arg = args_dict_from_ai.get("date_str")
                            if action_arg and s_arg is not None:
                                s_int = int(float(s_arg))
                                if s_int not in [1,2,3]: raise ValueError(f"Sensitivity '{s_int}' out of range [1,2,3] for market command.")
                                ...
                                if str(action_arg).lower() == "save":
                                    if d_arg:
                                        ...
                                        except ValueError: raise ValueError(f"Invalid date format '{d_arg}' from AI for market save. Expected MM/DD/YYYY.")
                                    else: raise ValueError("Date missing for market save action from AI.")
                                prepared_python_function_args = [c_args_list] # handle_market_command expects a list of args
                            else: raise ValueError("Action or sensitivity missing for market command from AI.")
                        elif function_name == "handle_risk_command":
                            a_type = args_dict_from_ai.get("assessment_type", "standard").lower()
                            ai_specific_arg = "standard_ai" if a_type == "standard" else "eod_ai"
                            prepared_python_function_args = [[ai_specific_arg]]
                            ...
                            raise NotImplementedError(f"Argument preparation logic for AI call to function '{function_name}' is not defined.")
                        ...
                        if asyncio.iscoroutinefunction(python_function):
                            ...
                            elif not prepared_python_function_args and hasattr(python_function, '__code__') and python_function.__code__.co_argcount == 0 : function_result_value = await python_function()
                            ...
                            elif not prepared_python_function_args and hasattr(python_function, '__code__') and python_function.__code__.co_argcount == 0 : function_result_value = await asyncio.to_thread(python_function)
                        ...
                        functions_called_this_interaction = True

                        tool_response_payload = {}
                        if isinstance(function_result_value, list): tool_response_payload = {"data_list": function_result_value[:30]}
                            ...
                            except TypeError: tool_response_payload = {"summary": str(function_result_value)[:2000]}
                        elif isinstance(function_result_value, str): tool_response_payload = {"summary": function_result_value[:3000]}
                        elif function_result_value is None: tool_response_payload = {"message": f"Function '{function_name}' executed successfully and returned None."}
                        else: tool_response_payload = {"content": str(function_result_value)[:2000]}
                        ...
                        error_message = f"Argument error preparing for {function_name} based on AI input: {str(e_val_dispatch)}"
                        print(error_message)
                        AI_CONVERSATION_HISTORY.append({"role": "tool", "parts": [{"function_response": {"name": function_name, "response": {"error": error_message}}}]})
                        # No need to reset last_proposed_function_call_details as it's not used for immediate repeat anymore
                        break
                    except Exception as e_exec:
                        error_message = f"Error during execution of {function_name}: {str(e_exec)}"
                        print(error_message); traceback.print_exc()
                        AI_CONVERSATION_HISTORY.append({"role": "tool", "parts": [{"function_response": {"name": function_name, "response": {"error": error_message}}}]})
                        break
                else:
                    error_message = f"Function '{function_name}' proposed by AI is not recognized or available."
                    print(error_message)
                    AI_CONVERSATION_HISTORY.append({"role": "tool", "parts": [{"function_response": {"name": function_name, "response": {"error": error_message}}}]})
                    break

            elif text_response_from_gemini_in_loop.strip():
                print("--- Gemini's Text Response (from tool loop) ---")
                print(text_response_from_gemini_in_loop.strip())
                print("-------------------------------------------")
                break
            else:
                print("Gemini did not propose a function call or provide text in this turn. Ending tool loop.")
                break

        except Exception as e_turn:
            print(f"An unexpected error occurred during AI interaction turn: {e_turn}")
            traceback.print_exc()
            if 'response' in locals() and hasattr(response, '_raw_response'): print(f"Raw response proto: {response._raw_response}")
            elif 'response' in locals(): print(f"Response object (no _raw_response): {response}")
            break

    if turn == max_internal_turns - 1 and function_call_to_execute :
        print(f"Max internal turns ({max_internal_turns}) reached. AI was proposing to call '{function_call_to_execute.name}'. Proceeding to summarization.")
        text_response_from_gemini_in_loop = "Max turns reached during tool execution. Attempting to summarize based on information gathered so far."


    needs_explicit_summary = False
    if functions_called_this_interaction:
        needs_explicit_summary = True
        print("Tools were used in this interaction. Preparing to request a final summary from AI.")
    elif text_response_from_gemini_in_loop.strip() and not any(q_phrase in text_response_from_gemini_in_loop.lower() for q_phrase in ["anything else?", "what next?", "can i help with another task?"]):
        print("No tools were called, AI provided a direct text response. This will be considered the final response for this message.")
    else:
        print("No tools were called and AI did not provide a clear text response or asked a question. No explicit summary will be forced unless history shows prior tool use for original request.")


    if needs_explicit_summary:
        true_original_request = original_session_request if original_session_request else user_new_message

        summarization_instruction = (
            f"The tool execution phase for my previous request ('{user_new_message}') is now complete. "
            f"Please review our entire conversation history, focusing on my ultimate original request for this session ('{true_original_request}') "
            f"and all the function call results you've received. "
            f"Based *only* on the information gathered so far throughout our conversation, provide a concise, final, textual answer directly addressing my original request ('{true_original_request}'). "
            f"Do not call any new functions or tools at this stage. Synthesize the answer from the existing data in our history."
        )
        print("\nRequesting final summary from AI based on executed functions and original request...")
        AI_CONVERSATION_HISTORY.append({"role": "user", "parts": [{"text": summarization_instruction}]})
        ...
        try:
            ...
            summary_candidate = summary_response.candidates[0] if summary_response.candidates else None
            final_text_summary_from_ai = ""

            if summary_candidate and summary_candidate.content and summary_candidate.content.parts:
                summary_model_turn_parts = []
                for p_proto_summary in summary_candidate.content.parts:
                    if hasattr(p_proto_summary, 'text') and p_proto_summary.text:
                        final_text_summary_from_ai += p_proto_summary.text

                if final_text_summary_from_ai.strip():
                    summary_model_turn_parts = [{"text": final_text_summary_from_ai}]
                    AI_CONVERSATION_HISTORY.append({"role": "model", "parts": summary_model_turn_parts})
                    print("\n--- AI's Final Synthesized Answer ---")
                    print(final_text_summary_from_ai.strip())
                    print("-------------------------------------")
                else:
                    fallback_msg = "AI did not provide a textual summary when explicitly asked and tools were disabled. The conversation might require further user guidance or the AI considers the task complete based on prior exchanges."
                    print(fallback_msg)
                    AI_CONVERSATION_HISTORY.append({"role": "model", "parts": [{"text": fallback_msg}]})
            else:
                no_candidate_msg = "No candidate or content in Gemini's summarization response. AI might have considered prior output final."
                print(no_candidate_msg)
                AI_CONVERSATION_HISTORY.append({"role": "model", "parts": [{"text": no_candidate_msg}]})

        except Exception as e_summary_turn:
            error_summary_msg = f"An error occurred during the AI summarization turn: {e_summary_turn}"
            print(error_summary_msg)
            traceback.print_exc()
            AI_CONVERSATION_HISTORY.append({"role": "model", "parts": [{"text": f"Error during summarization: {str(e_summary_turn)}"}]})

    elif not functions_called_this_interaction and text_response_from_gemini_in_loop.strip():
        pass # AI gave a direct text answer, no tools called, no further summary needed for this message.

    print("\n--- End of AI processing for this user message. Awaiting next input or command. ---")
    return "AI processing for current user message complete."
...
async def main_singularity():
    global AI_CONVERSATION_HISTORY, risk_persistent_signal, risk_signal_day, gemini_model, CURRENT_AI_SESSION_ORIGINAL_REQUEST # Added CURRENT_AI_SESSION_ORIGINAL_REQUEST

    # --- Initialize Loggers and Gemini API model (ensure this is done robustly) ---
    # (Your existing logger and Gemini API model initialization)
    # Example (ensure this happens ONCE at startup):
    if 'gemini_model' not in globals() or gemini_model is None:
        GEMINI_API_KEY = "AIzaSyDrtdvnjkMipwqOn-dboX2mpFJ_x05ulkI" # Replace or use os.getenv
        if not GEMINI_API_KEY or "YOUR_AI_API_KEY_HERE" == GEMINI_API_KEY:
            print("FATAL: Gemini API Key not set. Please set GEMINI_API_KEY variable.")
            return # Cannot run without API key
        try:
            genai.configure(api_key=GEMINI_API_KEY)
            gemini_model = genai.GenerativeModel('gemini-1.5-flash-latest') # Or your preferred model
            print("Gemini API configured successfully.")
        except Exception as e_cfg:
            print(f"FATAL: Error configuring Gemini API: {e_cfg}")
            gemini_model = None
            return # Cannot run
            
    if not risk_logger.hasHandlers():
        risk_file_handler_main = logging.FileHandler(RISK_LOG_FILE) # Ensure RISK_LOG_FILE is defined
    ...
    display_commands()    # Ensure this function is defined (the one with typing animation)
        ...
        try:
            ...
            if user_input_full_line.lower() == "end chat":
                if AI_CONVERSATION_HISTORY:
                    ...
                    AI_CONVERSATION_HISTORY.clear()
                    CURRENT_AI_SESSION_ORIGINAL_REQUEST = None # Clear original request
                else:
            ...
            if command == "/ai_start_chat": # This command effectively starts a new session
                ...
                if not user_natural_prompt:
                    ...
                    AI_CONVERSATION_HISTORY.clear()
                    CURRENT_AI_SESSION_ORIGINAL_REQUEST = user_natural_prompt # Store as original
                    await handle_ai_prompt(user_natural_prompt, is_new_session=True, original_session_request=CURRENT_AI_SESSION_ORIGINAL_REQUEST)

            elif command == "/ai_end_chat":
                if AI_CONVERSATION_HISTORY:
                ...
                if not user_natural_prompt:
                    ...
                    if is_new:
                        ...
                        CURRENT_AI_SESSION_ORIGINAL_REQUEST = user_natural_prompt # Store as original
                    # Pass the stored original request if available, otherwise the current prompt is the "original" for this new context
                    await handle_ai_prompt(user_natural_prompt, is_new_session=is_new, original_session_request=CURRENT_AI_SESSION_ORIGINAL_REQUEST if CURRENT_AI_SESSION_ORIGINAL_REQUEST else user_natural_prompt)
            
            # --- Standard CLI Command Handlers ---
            # Ensure these functions are defined to take `args` and, if also AI-callable, `ai_params=None`
            elif command == "/invest": await handle_invest_command(args, ai_params=None) 
            elif command == "/custom": await handle_custom_command(args, ai_params=None)
            elif command == "/breakout": await handle_breakout_command(args, ai_params=None)
            elif command == "/market": await handle_market_command(args, ai_params=None) # If refactored for ai_params
            elif command == "/cultivate": await handle_cultivate_command(args, ai_params=None)
            elif command == "/assess": await handle_assess_command(args, ai_params=None)
            elif command == "/quickscore": await handle_quickscore_command(args) # Typically just takes args
            elif command == "/risk": await handle_risk_command(args, ai_params=None) # If refactored for ai_params
            elif command == "/history": await handle_history_command(args) # Typically just takes args
                ...
                if AI_CONVERSATION_HISTORY and not user_input_full_line.startswith("/"):
                    ...
                    # This is a follow-up, so pass the existing original request
                    await handle_ai_prompt(user_input_full_line, is_new_session=False, original_session_request=CURRENT_AI_SESSION_ORIGINAL_REQUEST)
                else:
            ...
            CURRENT_AI_SESSION_ORIGINAL_REQUEST = None # Clear on error
...
if __name__ == "__main__":
    try:
        ...
        if 'risk_logger' in globals() and risk_logger: # Check if logger was initialized
            ...
            # Ensure RISK_LOG_FILE is defined if this fallback is hit early
            if 'RISK_LOG_FILE' not in globals(): RISK_LOG_FILE = 'risk_calculations.log' # Basic default
