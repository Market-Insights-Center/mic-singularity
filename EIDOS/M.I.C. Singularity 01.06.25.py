...
def display_commands():
    ...
    print("\nAI Command") # <--- NEW SECTION
    print("-------------------") # <--- NEW SECTION
    print("/ai - Interact with an AI to perform tasks using natural language.") # <--- NEW COMMAND
    print("  Usage: /ai <your natural language prompt>") # <--- NEW COMMAND
    print("  Example: /ai show me the breakout stocks then quickscore the top one") # <--- NEW COMMAND
...
async def run_breakout_analysis_singularity():
    """
    Performs breakout analysis, prints results, saves data, and
    returns a list of breakout ticker symbols.
    """
    print("\n--- Running Breakout Analysis (for AI & User) ---")
    invest_score_threshold = 100.0
    fraction_threshold = 3.0 / 4.0

    updated_data_breakout = []
    existing_tickers_data_breakout = {}
    breakout_tickers_list_for_ai = [] # For returning
    ...
    new_tickers_breakout_tv = [] # Renamed to avoid conflict with the return variable
    try:
    ...
    for ticker_b in all_tickers_to_process_breakout:
        ...
        try:
            live_price_b, current_invest_score_raw_b = await calculate_ema_invest(ticker_b, 2) # Daily sensitivity
    ...
    updated_data_breakout.sort(key=lambda x: x['_sort_score'], reverse=True)
    for item_b in updated_data_breakout: del item_b['_sort_score']
    ...
    try:
        ...
        if not final_df_breakout.empty:
            breakout_tickers_list_for_ai = final_df_breakout["Ticker"].tolist() # Get all tickers that made it
    except IOError as e_io_b:
        print(f"Error writing breakout data to '{BREAKOUT_TICKERS_FILE}': {e_io_b}")

    print("\n--- Breakout Analysis Results (for User) ---")
    if updated_data_breakout:
        result_table_data_breakout = [
            [
                ...
                row.get("Status", "N/A")
            ] for row in updated_data_breakout
        ]
    ...
    # Return the list of tickers for AI use
    if not breakout_tickers_list_for_ai:
        print("Breakout analysis did not yield any tickers to return to AI.")
    return breakout_tickers_list_for_ai
...
async def handle_ai_prompt(user_prompt: str):
    print(f"\nProcessing AI prompt: '{user_prompt}'...")

    if 'gemini_model' not in globals() or not gemini_model:
        print("Gemini model is not configured or available.")
        return

    # Define Tools Gemini can use
    run_breakout_tool = FunctionDeclaration(
        name="run_breakout_analysis_singularity",
        description="Scans the market for breakout stocks. Returns a list of ticker symbols.",
        parameters=None
    )

    quickscore_tool = FunctionDeclaration(
        name="handle_quickscore_command",
        description="Provides a quick score and graphs for a single stock ticker. Expects a specific ticker argument.",
        parameters={
            "type": "object",
            "properties": {"ticker": {"type": "string", "description": "The stock ticker, e.g., 'AAPL'."}},
            "required": ["ticker"]
        }
    )
    ...
    market_command_tool = FunctionDeclaration(
        ...
        description="Displays or saves S&P 500 market analysis scores. Scores include top 10, bottom 10, and SPY score based on EMA sensitivity (1:Weekly, 2:Daily, 3:Hourly).",
        parameters={
            "type": "object",
            "properties": {
                "action": {
                    "type": "string",
                    "description": "The action to perform: 'display' to show scores, or 'save' to save full market data.",
                    "enum": ["display", "save"]
                },
                "sensitivity": {
                    "type": "integer", # Important: AI should provide an integer based on this.
                    "description": "The EMA sensitivity to use: 1 for Weekly, 2 for Daily, 3 for Hourly. If user asks to display and is vague, default to 2 (Daily)."
                },
                "date_str": {
                    "type": "string",
                    "description": "The date for which to save data, in MM/DD/YYYY format. Only required if action is 'save'. If user says 'today' for saving, please determine the current date in MM/DD/YYYY format based on current date: " + datetime.now().strftime('%B %d, %Y') + "."
                }
            },
    ...
    conversation_history = [
        {
            "role": "user",
            "parts": [{"text": f"""You are an AI assistant for the 'Market Insights Center Singularity' script.
...
If you believe the user's request has been fully addressed based on the function results or your understanding, you can respond with a final summary message.
When providing arguments for functions, strictly adhere to the types and descriptions specified in the tool parameters. For sensitivity, provide an integer (1, 2, or 3).
For the 'handle_market_command' tool, if the action is 'save' and the user says 'today' for the date, use today's date: {datetime.now().strftime('%m/%d/%Y')}.
"""}]
        }
    ]
    ...
    for turn in range(max_turns):
        print(f"\n--- AI Interaction Turn {turn + 1}/{max_turns} ---")
        
        try:
            print(f"Sending to Gemini (history length: {len(conversation_history)})...")
            ...
            if not candidate:
                print("No candidate found in Gemini's response for this turn.")
                break 

            print(f"Candidate Finish Reason: {candidate.finish_reason.name if hasattr(candidate.finish_reason, 'name') else candidate.finish_reason}")

            if candidate.finish_reason.name == "MALFORMED_FUNCTION_CALL":
                print("Error: Gemini attempted a function call but it was malformed. Please review tool definitions and prompt clarity.")
                break 

            model_response_content = candidate.content
            function_call_to_execute = None
            text_response_from_gemini = ""
            model_turn_dict_parts = []
            ...
            if model_response_content and model_response_content.parts:
                for p_proto in model_response_content.parts:
                    if p_proto.function_call.name:
                        ...
                        model_turn_dict_parts.append(
                            {"function_call": {"name": p_proto.function_call.name, "args": dict(p_proto.function_call.args)}}
                        )
                        break 
                    elif p_proto.text:
                        text_response_from_gemini += p_proto.text
                        model_turn_dict_parts.append({"text": p_proto.text})
            
            if model_turn_dict_parts:
                conversation_history.append({"role": "model", "parts": model_turn_dict_parts})
            elif not function_call_to_execute and not text_response_from_gemini.strip() and model_response_content and model_response_content.role == "model" and not model_response_content.parts:
            ...
            if function_call_to_execute:
                ...
                args_dict = {key: val for key, val in fc.args.items()} # Gemini args are often Struct, convert to dict
                print(f"Gemini wants to call Function: {function_name}")
                print(f"With Arguments from AI: {args_dict}")
                ...
                if function_name in AVAILABLE_PYTHON_FUNCTIONS:
                    ...
                    function_result_value = "Error: Function did not produce a result." # Default
                    
                    try:
                        prepared_call_args = [] # This will be a list of arguments for the target Python function
                        
                        if function_name == "handle_quickscore_command":
                            ticker_arg = args_dict.get("ticker")
                            if ticker_arg:
                                prepared_call_args = [[str(ticker_arg)]] # Python function expects List[str]
                            else:
                                raise ValueError("Ticker not provided by AI for quickscore.")
                        
                        elif function_name == "handle_market_command":
                            action_arg = args_dict.get("action")
                            sensitivity_arg_from_ai = args_dict.get("sensitivity") # Could be float like 2.0 or int 2
                            date_arg = args_dict.get("date_str")

                            if action_arg and sensitivity_arg_from_ai is not None:
                                try:
                                    # Robustly convert AI's sensitivity to a clean integer
                                    sensitivity_int = int(float(sensitivity_arg_from_ai))
                                    if sensitivity_int not in [1, 2, 3]:
                                        raise ValueError(f"Sensitivity '{sensitivity_int}' out of range [1,2,3].")
                                except ValueError as e_sens:
                                    raise ValueError(f"Invalid sensitivity '{sensitivity_arg_from_ai}' from AI: {e_sens}") from e_sens
                                
                                current_call_args_list = [str(action_arg), str(sensitivity_int)]
                                if str(action_arg).lower() == "save":
                                    if date_arg:
                                        # Validate date format coming from AI
                                        try:
                                            datetime.strptime(str(date_arg), '%m/%d/%Y')
                                            current_call_args_list.append(str(date_arg))
                                        except ValueError:
                                            raise ValueError(f"Invalid date format '{date_arg}' from AI. Expected MM/DD/YYYY.")
                                    else:
                                        # If AI was instructed to use today's date and didn't, or if date is critical.
                                        # The modified handle_market_command can also error out if date is missing for save.
                                        # For this dispatch, we'll require it if action is 'save'.
                                        # The AI prompt now guides it to use today's date if user says "today".
                                        raise ValueError("Date not provided by AI for 'save' action in market command.")
                                prepared_call_args = [current_call_args_list] # Python function expects List[str]
                            else:
                                raise ValueError("Action or sensitivity not provided by AI for market command.")
                        
                        elif function_name == "run_breakout_analysis_singularity":
                            # This function in AVAILABLE_PYTHON_FUNCTIONS takes no args from AI
                            prepared_call_args = [] # No arguments to pass to the function call itself
                        
                        else: # Default for other functions that might take **args_dict
                            # This case needs to be carefully considered per function.
                            # If a function truly takes **args_dict, this is fine.
                            # Otherwise, specific arg prep like above is needed.
                            # For now, let's assume no other functions use this generic path.
                            # function_result_value = await python_function(**args_dict) # Example
                            raise NotImplementedError(f"Argument preparation for function '{function_name}' not defined.")

                        # Actual function call
                        if asyncio.iscoroutinefunction(python_function):
                            if not prepared_call_args and function_name == "run_breakout_analysis_singularity":
                                function_result_value = await python_function()
                            elif prepared_call_args: # For functions expecting a list of args like quickscore/market
                                function_result_value = await python_function(*prepared_call_args)
                            else: # Should not be hit if prepared_call_args is exhaustive for known functions
                                raise ValueError(f"Argument preparation failed for {function_name} before async call.")
                        else: # Synchronous functions (less common for your main handlers)
                            function_result_value = await asyncio.to_thread(python_function, *prepared_call_args)
                        
                        print(f"Result of {function_name}: {function_result_value}")

                        conversation_history.append({
                            "role": "tool",
                            "parts": [{"function_response": {
                                        "name": function_name,
                                        "response": {"content": str(function_result_value)}
                                      }}]
                        })

                    except ValueError as e_val_dispatch: # Catch arg validation/prep errors
                        error_message = f"Argument error for {function_name}: {str(e_val_dispatch)}"
                        print(error_message)
                        conversation_history.append({
                            "role": "tool",
                            "parts": [{"function_response": {"name": function_name, "response": {"error": error_message}}}]
                        })
                        break # Stop on this critical error
                    except Exception as e_exec:
                        error_message = f"Error executing Python function {function_name}: {str(e_exec)}"
                        print(error_message)
                        traceback.print_exc()
                        conversation_history.append({
                            "role": "tool",
                        ...
                        break 
                else: # Function name from AI not in our dispatcher
                    error_message = f"Python function '{function_name}' is not mapped in AVAILABLE_PYTHON_FUNCTIONS."
                    print(error_message)
                    conversation_history.append({
                        "role": "tool",
                        "parts": [{"function_response": {"name": function_name, "response": {"error": error_message}}}]
                ...
                print(text_response_from_gemini.strip())
                print("-----------------------------")
                print("AI provided a text response. Ending interaction for this prompt.")
                break 
            
            else:
                print("Gemini did not propose a function call or provide text. Ending interaction.")
                if candidate and candidate.content and not candidate.content.parts: print("(Candidate content exists but has no parts)")
                elif candidate and not candidate.content: print("(Candidate has no content)")
                break

        except Exception as e:
            print(f"An error occurred during AI interaction turn {turn + 1}: {e}")
...
# Remember to define/import and map your actual functions to AVAILABLE_PYTHON_FUNCTIONS
# async def actual_run_breakout_analysis_singularity(): ...
# async def actual_handle_quickscore_command(args: list): ...
# AVAILABLE_PYTHON_FUNCTIONS = {
#    "run_breakout_analysis_singularity": actual_run_breakout_analysis_singularity,
#    "handle_quickscore_command": actual_handle_quickscore_command,
# }

# This should be defined in the same scope as handle_ai_prompt or passed to it.
# These are your actual, rewritten, async functions.
# (Ensure this dictionary and the functions it references are correctly defined)
AVAILABLE_PYTHON_FUNCTIONS = {
    "run_breakout_analysis_singularity": run_breakout_analysis_singularity, # Or your actual one
    "handle_quickscore_command": handle_quickscore_command, # Or your actual one
    "handle_market_command": handle_market_command  # <-- MAP TO THE MODIFIED COMMAND HANDLER
}
        ...
        try:
            ...
            if command == "/invest":
                ...
                if not user_natural_prompt:
                    print("Please provide a prompt for the AI. Usage: /ai <your request>")
                elif not gemini_model: # Check if model was configured
                    print("Gemini API model not available. Please check configuration and API key.")
                else:
                    await handle_ai_prompt(user_natural_prompt) # We will create this function next
            elif command == "/help":
                display_commands()
            elif command == "/exit":
