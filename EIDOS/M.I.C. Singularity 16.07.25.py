...
async def handle_report_generation(args: List[str], ai_params: Optional[Dict] = None, is_called_by_ai: bool = False):
    ...
    # --- Step 5 & 6: Enhanced Report Generation & Saving ---
    if not is_called_by_ai: print("-> Generating enhanced AI-powered report content...")

    # Enhance tailored data with scores and percentages
    ...
    # --- MODIFICATION: Call new AI helper functions ---
    ...
    # Generate rationale for each holding and add to the report
    for holding in tailored_portfolio_data:
        print(f"   -> Generating rationale for {holding['ticker']}...")
        rationale = await get_ai_stock_rationale(holding['ticker'], investment_goals, risk_tolerance)
        score_val = holding.get('invest_score')
        score_str = f"{score_val:.2f}%" if isinstance(score_val, (int, float)) else "N/A"

        report_lines.append(f"### {holding['ticker']}")
        report_lines.append(f"- **Allocation:** ${holding['actual_money_allocation']:,.2f} ({holding['percentage_allocation']:.2f}%)")
        report_lines.append(f"- **Shares:** {holding['shares']:.2f}")
        report_lines.append(f"- **Invest Score:** {score_str}")
        report_lines.append(f"- **Rationale:** {rationale}")
        report_lines.append("-" * 40)
    
    report_lines.append(f"\n## Final Cash Position")
    report_lines.append(f"**Remaining Cash:** ${final_cash:,.2f} ({(final_cash/portfolio_value)*100:.2f}%)")
    
    report_content = "\n".join(report_lines)
    filename = f"Investment Report - {datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
    try:
