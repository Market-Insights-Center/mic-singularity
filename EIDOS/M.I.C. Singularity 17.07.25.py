...
async def handle_report_generation(args: List[str], ai_params: Optional[Dict] = None, is_called_by_ai: bool = False):
    ...
    try:
        ...
        sector_requests = [s.strip().lower() for s in sector_requests_str.split(',')] if sector_requests_str else []
    ...
    # --- Step 3: Filtered Stock List Generation ---
    if not is_called_by_ai: print("-> Generating filtered stock list...")
    top_10_local_tickers = []
    if sector_requests:
        local_tickers = []
        for sector in sector_requests:
            try:
                with open(f"report_generation_{sector}_list.txt", 'r') as f:
                    local_tickers.extend([line.strip().upper() for line in f if line.strip()])
            except FileNotFoundError:
                if not is_called_by_ai: print(f"   Note: Local list for sector '{sector}' not found.")
        
        if local_tickers:
            scores_local = await calculate_market_invest_scores_singularity(list(set(local_tickers)), ema_sensitivity, is_called_by_ai=True)
            top_10_local_tickers = [d['ticker'] for d in scores_local[:10] if d.get('score') is not None]

    num_web_stocks_to_find = 20 if not top_10_local_tickers else 10
    if not is_called_by_ai: print(f"   Screening for {num_web_stocks_to_find} unique stocks online...")
    web_stocks = await screen_stocks_for_report(sectors=sector_requests, num_stocks=num_web_stocks_to_find, existing_tickers=top_10_local_tickers, is_called_by_ai=True)
    
    all_potential_stocks_with_scores = await calculate_market_invest_scores_singularity(list(set(top_10_local_tickers + web_stocks)), ema_sensitivity, is_called_by_ai=True)
    
    # De-duplicate GOOG/GOOGL and BRK.A/BRK.B
    temp_list = []
    seen_tickers = set()
    for stock_data in all_potential_stocks_with_scores:
        ticker = stock_data['ticker']
        if ticker in ['GOOG', 'GOOGL']:
            if 'GOOG' not in seen_tickers:
                temp_list.append({'ticker': 'GOOG', 'score': stock_data.get('score')})
                seen_tickers.add('GOOG')
        elif ticker in ['BRK-A', 'BRK-B']: # yfinance uses BRK-B
            if 'BRK-B' not in seen_tickers:
                temp_list.append({'ticker': 'BRK-B', 'score': stock_data.get('score')})
                seen_tickers.add('BRK-B')
        elif ticker not in seen_tickers:
            temp_list.append(stock_data)
            seen_tickers.add(ticker)
            
    all_potential_stocks_with_scores = temp_list
    all_potential_stocks_with_scores.sort(key=lambda x: x.get('score', -1), reverse=True)
    filtered_stocks_final = [d['ticker'] for d in all_potential_stocks_with_scores[:15]]
    
    if not filtered_stocks_final:
        print("   -> No stocks found from primary search. Triggering fallback...")
        all_local_files = glob.glob("report_generation_*_list.txt")
        fallback_tickers = set()
        for file in all_local_files:
            with open(file, 'r') as f:
                fallback_tickers.update([line.strip().upper() for line in f if line.strip()])
        
        if fallback_tickers:
            print(f"   -> Analyzing {len(fallback_tickers)} stocks from all local lists...")
            fallback_scores = await calculate_market_invest_scores_singularity(list(fallback_tickers), ema_sensitivity, is_called_by_ai=True)
            fallback_scores.sort(key=lambda x: x.get('score', -1), reverse=True)
            filtered_stocks_final = [d['ticker'] for d in fallback_scores[:15]]
            print(f"   -> Fallback selected top 15 stocks across all lists: {', '.join(filtered_stocks_final)}")
        else:
            print("   -> Fallback failed: No local stock lists found.")
    ...
    if filtered_stocks_final:
        ...
        print("   Warning: No stocks passed filtering criteria (including fallback). Reallocating weight to Market Index ETFs.")
    ...
    # Fetch Industry data for each holding
    ...
    # Add industry and other details to the holding data
    ...
    # --- MODIFICATION: New table header and data structure ---
    ...
    for holding in tailored_portfolio_data:
        ...
        # Get AI rationale (can be done concurrently for speed, but sequential is simpler here)
