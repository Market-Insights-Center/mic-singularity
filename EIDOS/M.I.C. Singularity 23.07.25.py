...
async def screen_stocks_for_report(sectors: List[str], num_stocks: int, existing_tickers: List[str], is_called_by_ai: bool = False) -> List[str]:
    """
    Screens for stocks using TradingView with parameters for report generation.
    """
    if not is_called_by_ai:
        print(f"   Executing screener for report...")
    try:
        # Base query with market cap and volume constraints
        query = Query().select('name').where(
            Column('market_cap_basic') >= 5_000_000_000, # $5B Market Cap
            Column('volume') >= 500_000, # 500k Volume
            Column('close') >= 1
        )
        
        # Add sector filter if requested and not empty
        if sectors and any(s.strip() for s in sectors):
            # Capitalize sectors for the query as TradingView screener expects
            capitalized_sectors = [s.strip().capitalize() for s in sectors if s.strip()]
            query.where(Column('sector').one_of(*capitalized_sectors))

        # Order by market cap to get larger, more stable companies first
        # Fetch more than needed to allow for filtering of existing tickers
        query.order_by('market_cap_basic', ascending=False).limit(num_stocks * 5) 
        
        # Execute the screener query in a separate thread to avoid blocking
        scanner_results_tuple = await asyncio.to_thread(query.get_scanner_data, timeout=60)
        
        if scanner_results_tuple and isinstance(scanner_results_tuple, tuple) and len(scanner_results_tuple) > 0 and isinstance(scanner_results_tuple[1], pd.DataFrame):
            df = scanner_results_tuple[1]
            if not df.empty and 'name' in df.columns:
                all_found = [str(t).split(':')[-1].replace('.', '-') for t in df['name'].tolist() if pd.notna(t)]
                # Filter out tickers that are already in our list from local files
                unique_new = [t for t in all_found if t not in existing_tickers]
                # Return the number of unique new stocks requested
                return unique_new[:num_stocks]
    except Exception as e:
        if not is_called_by_ai:
            print(f"    Warning: Stock screener for report failed: {e}")
...
async def handle_report_generation(args: List[str], ai_params: Optional[Dict] = None, is_called_by_ai: bool = False):
    ...
    based on user-defined criteria, now with enhanced logic and AI-generated content.
    """
    if not is_called_by_ai:
    ...
    try:
        ...
        # This input is no longer used for filtering but can be kept for context if desired.
        sector_requests_str = ask_singularity_input("Enter any specific sectors to focus on, comma-separated (or press Enter to skip)", default_val="")
        
    except (ValueError, TypeError):
        print("Invalid input. Please start over.")
        return

    # --- Step 2: Determine Strategy and Parameters ---
    if not is_called_by_ai: print("-> Determining strategy based on market conditions...")
    ...
    if is_high_score:
        if risk_tolerance <= 2:
            strategy = {'name': 'High Score / Low Tolerance', 'amplification': 1.0, 'weights': {'Market Index': 20, 'Hedging': 20, 'Filtered Stocks': 60}, 'sub_portfolios': {'Market Index': 'SPY,DIA,QQQ', 'Hedging': 'GLD,SLV'}}
        elif risk_tolerance == 3:
            strategy = {'name': 'High Score / Medium Tolerance', 'amplification': 1.5, 'weights': {'Market Index': 15, 'Hedging': 15, 'Filtered Stocks': 70}, 'sub_portfolios': {'Market Index': 'SPY,QQQ', 'Hedging': 'GLD,SLV'}}
        else:
            strategy = {'name': 'High Score / High Tolerance', 'amplification': 2.0, 'weights': {'Market Index': 7.5, 'Hedging': 7.5, 'Breakouts': 10, 'Filtered Stocks': 75}, 'sub_portfolios': {'Market Index': 'SPY,QQQ', 'Hedging': 'GLD,SLV'}}
    else:
        if risk_tolerance <= 2:
            strategy = {'name': 'Low Score / Low Tolerance', 'amplification': 0.5, 'weights': {'Market Index': 30, 'Hedging': 30, 'Filtered Stocks': 40}, 'sub_portfolios': {'Market Index': 'SPY,DIA', 'Hedging': 'GLD,SLV'}}
        elif risk_tolerance == 3:
            strategy = {'name': 'Low Score / Medium Tolerance', 'amplification': 1.0, 'weights': {'Market Index': 25, 'Hedging': 25, 'Filtered Stocks': 50}, 'sub_portfolios': {'Market Index': 'SPY,DIA,QQQ', 'Hedging': 'GLD,SLV'}}
        else:
            strategy = {'name': 'Low Score / High Tolerance', 'amplification': 1.5, 'weights': {'Market Index': 15, 'Hedging': 15, 'Breakouts': 10, 'Filtered Stocks': 60}, 'sub_portfolios': {'Market Index': 'SPY,QQQ', 'Hedging': 'GLD,SLV'}}

    ema_sensitivity = 1 if risk_tolerance <= 2 else (2 if risk_tolerance == 3 else 3)
    if not is_called_by_ai: print(f"   Strategy Selected: {strategy['name']} | Amplification: {strategy['amplification']} | EMA Sensitivity: {ema_sensitivity}")

    # --- Step 3: Filtered Stock List Generation (NEW LOGIC) ---
    if not is_called_by_ai: print("-> Generating filtered stock list from all saved sector files...")
    
    all_local_files = glob.glob("report_generation_*_list.txt")
    all_local_tickers = set()

    if not all_local_files:
        print("   -> ERROR: No 'report_generation_*_list.txt' files found. Cannot select stocks.")
        return
        
    for file in all_local_files:
        try:
            with open(file, 'r') as f:
                all_local_tickers.update([line.strip().upper() for line in f if line.strip()])
        except Exception as e:
            print(f"   Warning: Could not read file {file}: {e}")
    
    if not all_local_tickers:
        print("   -> ERROR: The saved sector lists are empty. Cannot select stocks.")
        return

    if not is_called_by_ai: print(f"   -> Analyzing {len(all_local_tickers)} unique stocks from all local lists...")
    
    all_stocks_with_scores = await calculate_market_invest_scores_singularity(list(all_local_tickers), ema_sensitivity, is_called_by_ai=True)
    all_stocks_with_scores.sort(key=lambda x: x.get('score', -1), reverse=True)
    
    # Select the top 25 tickers with the highest invest scores
    top_25_stocks = all_stocks_with_scores[:25]
    filtered_stocks_final = [d['ticker'] for d in top_25_stocks]

    if not filtered_stocks_final:
        print("   -> ERROR: Could not determine top stocks after scoring. Aborting report generation.")
        return
    
    if not is_called_by_ai: print(f"   -> Selected top {len(filtered_stocks_final)} stocks for the report based on invest scores.")
    ...
    if not is_called_by_ai: print(f"-> Calculating Invest Scores for all {len(all_tickers_to_score)} portfolio components...")
    ...
    if not is_called_by_ai: print("-> Allocating portfolio using /invest logic...")
    invest_params = {
        "ema_sensitivity": ema_sensitivity,
        "amplification": strategy['amplification'],
    ...
    if filtered_stocks_final:
        invest_params['sub_portfolios'].append({'tickers': ",".join(filtered_stocks_final), 'weight': strategy['weights']['Filtered Stocks']})
    else:
        # This fallback is less likely to be hit now, but kept for safety
        print("   Warning: No stocks passed filtering criteria. Reallocating weight to Market Index ETFs.")
        if invest_params['sub_portfolios']:
             for sub_p in invest_params['sub_portfolios']:
                if sub_p['tickers'] == strategy['sub_portfolios']['Market Index']:
                    sub_p['weight'] += strategy['weights']['Filtered Stocks']
                    break
    ...
    if not is_called_by_ai: print("-> Enhancing data and generating AI-powered report content...")
    ...
    for holding in tailored_portfolio_data:
        ...
        final_table_data.append([
            ...
            score_str,
            f"{holding['shares']:.2f}",
            f"{holding['percentage_allocation']:.2f}%",
            holding.get('industry', 'N/A')
        ])
    ...
    report_lines = [
        ...
        f"(Generated by M.I.C. Singularity on {datetime.now().strftime('%Y-%m-%d')})",
        "\n" + "="*80 + "\n",
        "## Client Profile",
        f"**Risk Tolerance:** {risk_tolerance}/5",
        f"**Investment Goals:** {investment_goals}",
