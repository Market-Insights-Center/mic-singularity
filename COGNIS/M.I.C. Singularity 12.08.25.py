...
try:
    ...
    print("⚠️ Warning: 'PyPortfolioOpt' is not installed. The /optimize command will not be available.")
    print("Please run: pip install PyPortfolioOpt")
...
try:
    ...
    print("⚠️ Warning: 'seaborn' is not installed. The /heatmap command will not be available.")
    print("Please run: pip install seaborn")
    sns = None

try:
    import speech_recognition as sr
    print("✔ 'speech_recognition' loaded successfully.")
except ImportError:
    print("⚠️ Warning: 'speech_recognition' is not installed. Voice commands will not be available.")
...
try:
    ...
    print("⚠️ Warning: 'gTTS' is not installed. Voice commands will not be available.")
    print("Please run: pip install gTTS")
    gTTS = None

try:
    import playsound
    print("✔ 'playsound' loaded successfully.")
except ImportError:
    print("⚠️ Warning: 'playsound' is not installed. Voice commands will not be available.")
    print("Please run: pip install playsound")
    playsound = None

# --- Required for microphone access by speech_recognition ---
try:
    import pyaudio
    print("✔ 'pyaudio' loaded successfully.")
except ImportError:
    print("⚠️ Warning: 'pyaudio' is not installed. Microphone access for voice commands may fail.")
    print("If you encounter issues, please install PyAudio for your system.")
    pyaudio = None

try:
    from scipy.stats import percentileofscore
except ImportError:
    print("⚠️ Warning: 'scipy' is not installed. Volatility Rank will not be available.")
...
active_alerts = []
alert_lock = asyncio.Lock()  # To safely modify the list from different tasks
...
try:
    ...
    # --- Load API Keys ---
    GEMINI_API_KEY = config.get('API_KEYS', 'GEMINI_API_KEY', fallback="AIzaSyDYpuf4NC1SET9Z5_hQqbJ9tzpxXOPk4k0")
...
# --- Gemini API Configuration (using key from config) ---
gemini_model = None
try:
    if not GEMINI_API_KEY or GEMINI_API_KEY == "YOUR_GEMINI_API_KEY" or "AIza" not in GEMINI_API_KEY:
        print("⚠️ Warning: Gemini API key is missing or is a placeholder in config.ini. AI features will be disabled.")
    else:
...
async def load_alerts_from_csv():
    """
    Loads alerts from the CSV file into the active_alerts list at startup.
    """
    ...
    try:
        with open(ALERTS_FILE, mode='r', newline='', encoding='utf-8') as csvfile:
            reader = csv.DictReader(csvfile)
            temp_alerts = []
            for row in reader:
                try:
                    # Convert values back to their correct types
                    row['value'] = float(row['value'])
                    if 'sensitivity' in row and row['sensitivity']:
                        row['sensitivity'] = int(row['sensitivity'])
                    else:
                        # Ensure key exists even if empty for price alerts
                        row['sensitivity'] = None
                    temp_alerts.append(row)
                except (ValueError, KeyError) as e:
                    print(f"Warning: Skipping malformed alert in {ALERTS_FILE}: {row}. Error: {e}")
            
            # --- FIX IS HERE: Use await instead of asyncio.run ---
            async with alert_lock:
                active_alerts = temp_alerts

            if active_alerts:
                print(f"✅ Loaded {len(active_alerts)} alert(s) from {ALERTS_FILE}.")
    except Exception as e:
        print(f"Error loading alerts from {ALERTS_FILE}: {e}")

def save_alerts_to_csv():
    """
    Saves the current state of the active_alerts list to the CSV file, overwriting it.
    """
    try:
        with open(ALERTS_FILE, mode='w', newline='', encoding='utf-8') as csvfile:
            # Define all possible headers
            fieldnames = ['ticker', 'metric', 'operator', 'value', 'sensitivity']
...
async def get_user_preferences_tool(is_called_by_ai: bool = False) -> Dict[str, Any]:
    """An AI tool to retrieve all currently saved user preferences."""
    if not is_called_by_ai:
        return {"status": "error", "message": "This tool is for AI use only."}
    
    prefs = await asyncio.to_thread(load_user_preferences)
    if not prefs:
        return {"status": "success", "preferences": None, "message": "No user preferences have been saved yet."}
...
def make_hashable(obj):
    ...
    # Handle MapComposite and RepeatedComposite by converting them to standard Python types first
...
# --- Voice Assistant TTS Engine (pyttsx3) ---
tts_engine = None # Ensure the variable exists globally
try:
    import pyttsx3
    tts_engine = pyttsx3.init()
    
    # --- Set properties for a faster, male voice ---
    voices = tts_engine.getProperty('voices')
    male_voice = None
    
    # First, try to find a voice explicitly tagged as male
    for voice in voices:
        if hasattr(voice, 'gender') and voice.gender == 'male':
            male_voice = voice
            break
            
    # If no explicitly male voice is found, check for common male voice names as a fallback
    if not male_voice:
        for voice in voices:
            if voice.name.startswith('Microsoft David') or voice.name.startswith('Microsoft Mark'):
                male_voice = voice
                break
    
    if male_voice:
        tts_engine.setProperty('voice', male_voice.id)
        print(f"✔ Male voice '{male_voice.name}' selected for TTS.")
    else:
        print("⚠️ Warning: Male voice not found. Using the default system voice.")
    
    # Make the voice talk faster (default is ~200 WPM)
    current_rate = tts_engine.getProperty('rate')
    tts_engine.setProperty('rate', current_rate + 50) # Increase speed by 50 WPM
    
    print("✔ 'pyttsx3' TTS engine configured successfully.")

except ImportError:
    print("⚠️ Warning: 'pyttsx3' is not installed. Voice output will be disabled.")
...
def speak_text(text: str):
    """Converts a text string to speech and plays it using the pyttsx3 engine."""
    if not tts_engine:
    ...
    try:
        ...
        print("AI Response (TTS failed):", text) # Fallback

# --- (Replace the listen_for_voice and handle_voice_command functions) ---

def listen_for_voice() -> Optional[str]:
    """Listens for voice input from the microphone and transcribes it to text."""
    if not sr or not pyaudio:
        print("Voice recognition libraries not available.")
        return None
    ...
    with sr.Microphone() as source:
        ...
        # --- MODIFIED: Wake words updated in console message ---
        print("🎤 Listening for wake words ('Cognis')...")
        try:
...
async def parse_and_execute_voice_command(command_text: str):
    """Parses the transcribed text and calls the appropriate command handler."""
    global AI_CONVERSATION_HISTORY
    
    ai_formatted_command = f"/ai {command_text}"
    print(f"🤖 Processing as AI command: \"{ai_formatted_command}\"")

    is_new_session = not AI_CONVERSATION_HISTORY
    await handle_ai_prompt(command_text, is_new_session=is_new_session)

    final_response_text = "I've completed the request."
    if AI_CONVERSATION_HISTORY:
        for entry in reversed(AI_CONVERSATION_HISTORY):
            if entry.get("role") == "model" and entry.get("parts"):
                text_part = next((part.get("text", "") for part in entry["parts"] if "text" in part), "")
                if text_part:
                    final_response_text = text_part
                    break
    
    speak_text(final_response_text)

async def handle_voice_command():
    """Main loop to handle the voice assistant interaction."""
    if not all([sr, tts_engine, pyaudio]):
        print("❌ Cannot start voice assistant. One or more required libraries are missing or failed to initialize.")
        return

    # --- MODIFIED: List of wake words ---
    wake_words = ["cognis", "sandman", "dude", "dumbass", "idiot"]
    exit_phrases = ["stop listening", "stop chat", "end conversation"]
    
    # --- MODIFIED: Spoken message updated with new wake words ---
    speak_text("Voice assistant activated. Say 'Cognis' followed by your command.")
    
    while True:
        transcribed_text = listen_for_voice()

        if transcribed_text:
            text_lower = transcribed_text.lower()
            
            if any(phrase in text_lower for phrase in exit_phrases):
                speak_text("Deactivating voice assistant. Goodbye!")
                break
            
            # --- MODIFIED: Logic to detect any wake word from the list ---
            ...
            if found_wake_word:
                ...
                command_part = text_lower.split(found_wake_word, 1)[-1].strip()
                if command_part:
                    speak_text(f"Understood. Processing: {command_part}")
                    await parse_and_execute_voice_command(command_part)
                else:
                    speak_text("Yes? How can I help you?")

def plot_correlation_heatmap(correlation_matrix: pd.DataFrame, title: str, is_called_by_ai: bool = False) -> Optional[str]:
    """
    Generates and saves a seaborn heatmap for a given correlation matrix.

    Args:
        correlation_matrix: A pandas DataFrame containing the correlation data.
        title: The title for the heatmap chart.
        is_called_by_ai: Flag to suppress console output.

    Returns:
        The filename of the saved image, or None if an error occurred.
    """
    if not sns:
        if not is_called_by_ai:
            print("❌ Seaborn library is not installed. Cannot generate heatmap.")
        return None
    try:
        ...
        # Draw the heatmap. Annotations are turned off for clarity with many tickers.
...
async def get_specific_index_tickers(index_symbol: str, is_called_by_ai: bool = False) -> List[str]:
    """
    Fetches the component tickers for a specific major index from Wikipedia.

    Args:
        index_symbol: The symbol for the index (SPY, QQQ, DIA, RUT).
        is_called_by_ai: Flag to suppress console output.

    Returns:
        A list of ticker symbols for the specified index.
    """
    index_symbol = index_symbol.upper()
    if index_symbol == 'SPY':
        return await asyncio.to_thread(get_sp500_symbols_singularity, is_called_by_ai=True)
    ...
    if index_symbol not in urls:
        if not is_called_by_ai:
            print(f"   -> ERROR: Index '{index_symbol}' is not supported. Use SPY, QQQ, DIA, or RUT.")
        return []
    ...
    try:
        ...
        if col_name not in df.columns:
            if not is_called_by_ai:
                print(f"   -> ERROR: Could not find '{col_name}' column for {index_symbol}.")
            return []
...
def create_pdf_report_reportlab(filename, report_data):
    """
    Generates the investment report PDF using the ReportLab library.
    """
    ...
    t.setStyle(TableStyle([
        ...
        # --- FIX IS HERE: Corrected 'whitespoke' to 'whitesmoke' ---
...
async def save_chart_for_pdf(portfolio_data, cash_value, total_value, filename_prefix="report_pie_chart"):
    """
    Generates and saves a portfolio allocation pie chart as an image file,
    showing the top 15 holdings and grouping the rest into "Other".
    
    Returns:
        str: The filename of the saved chart image.
    """
    # Combine stock holdings and cash into a single list
    all_holdings = [{'ticker': h['ticker'], 'value': h['actual_money_allocation']} for h in portfolio_data]
    if cash_value > 1e-9: # Only add cash if it's a meaningful amount
    ...
    # --- NEW LOGIC: Group smaller holdings into 'Other' ---
    labels = []
    sizes = []
    if len(all_holdings) > 15:
        ...
        sizes = [h['value'] for h in all_holdings]
...
def safe_get(data_dict, key, default=None):
    ...
    # yfinance can return 'None' as a string or just None
...
def calculate_backtest_performance(portfolio_value_history: pd.Series) -> dict:
    """
    Calculates key performance metrics from a portfolio's value history.

    Args:
        portfolio_value_history: A pandas Series with portfolio values indexed by date.

    Returns:
        A dictionary containing total return, final value, and max drawdown.
    """
    if portfolio_value_history.empty:
        return {
            "final_value": 0,
            "total_return_pct": 0,
            "max_drawdown_pct": 0
    ...
    initial_value = portfolio_value_history.iloc[0]
    final_value = portfolio_value_history.iloc[-1]
    total_return_pct = ((final_value / initial_value) - 1) * 100

    # --- Calculate Maximum Drawdown ---
    # Create a running maximum of the portfolio value
    running_max = portfolio_value_history.cummax()
    # Calculate the drawdown series (percentage drop from the running max)
    drawdown = (portfolio_value_history - running_max) / running_max
    max_drawdown_pct = drawdown.min() * 100  # The max drawdown is the minimum value in the series
...
def plot_backtest_performance_graph(portfolio_history: pd.Series, spy_history: pd.Series, filename_prefix: str = "backtest_performance"):
    """
    Generates and saves a graph comparing portfolio performance against SPY.
    """
    try:
        plt.style.use('dark_background')
        fig, ax = plt.subplots(figsize=(14, 8))

        # Normalize both series to a starting value of 100 for comparison
        portfolio_normalized = (portfolio_history / portfolio_history.iloc[0]) * 100
        spy_normalized = (spy_history / spy_history.iloc[0]) * 100

        ax.plot(portfolio_normalized.index, portfolio_normalized, label='Your Portfolio', color='cyan', linewidth=2.5)
        ax.plot(spy_normalized.index, spy_normalized, label='SPY', color='red', linestyle='--')

        start_date_str = portfolio_history.index.min().strftime('%Y-%m-%d')
        end_date_str = portfolio_history.index.max().strftime('%Y-%m-%d')

        ax.set_title(f'Backtest Performance vs. SPY\n({start_date_str} to {end_date_str})', color='white', fontsize=16)
...
async def run_portfolio_backtest(portfolio_code: str, start_date: str, end_date: str, initial_value: float = 10000.0):
    """
    Runs a historical simulation for a saved portfolio configuration with dynamic monthly rebalancing,
    plotting the final performance on a daily scale.
    """
    print(f"-> Starting backtest for portfolio '{portfolio_code}' from {start_date} to {end_date}...")
    ...
    try:
        ...
        if portfolio_row.empty:
            ...
            return
        portfolio_config = portfolio_row.iloc[0]
    except Exception as e:
        print(f"❌ Error reading portfolio config: {e}")
        return
    ...
    for i in range(1, num_portfolios + 1):
        ...
        if tickers_str and isinstance(tickers_str, str):
            cleaned_str = tickers_str.replace('"', '').replace("'", "")
            original_tickers.update([t.strip() for t in cleaned_str.split(',') if t.strip()])
    
    if not original_tickers:
        print("❌ Error: No tickers found in the portfolio configuration.")
        return

    # 2. Fetch Historical Data
    # --- MODIFICATION: Fetch DAILY data for smoother plotting, but go back further to ensure enough weekly points for EMA ---
    fetch_start_date = (pd.to_datetime(start_date) - timedelta(weeks=75)).strftime('%Y-%m-%d')
    tickers_to_fetch = list(original_tickers) + ['SPY']
    
    print(f"-> Fetching daily historical data for {len(tickers_to_fetch)} tickers...")
    data_download = await asyncio.to_thread(
        yf.download, tickers=tickers_to_fetch, start=fetch_start_date, end=end_date, interval="1d", progress=False
    )
    
    if data_download.empty or 'Close' not in data_download.columns:
        print("❌ Error: Could not download historical data for the given period.")
        return
        
    hist_data = data_download['Close'].copy()
    hist_data.dropna(axis=1, how='all', inplace=True)

    # 3. Initialize Simulation Variables
    sim_data_daily = hist_data.loc[start_date:end_date].copy()
    buying_power = initial_value
    shares_owned = {}
    portfolio_value_history = pd.Series(index=sim_data_daily.index, dtype=float)
    last_rebalance_month = -1

    print("-> Running monthly rebalancing simulation...")
    # 4. Run the Dynamic Simulation Loop (iterating daily)
    for date, daily_prices in sim_data_daily.iterrows():
        # Check if it's time to rebalance (first day of a new month)
        if date.month != last_rebalance_month:
            print(f"   - Rebalancing for {date.strftime('%B %Y')}...")
            last_rebalance_month = date.month
            
            current_holdings_value = sum(shares_owned.get(ticker, 0) * daily_prices.get(ticker, 0) for ticker in shares_owned if pd.notna(daily_prices.get(ticker)))
            buying_power += current_holdings_value
            shares_owned = {}

            # --- MODIFICATION: Resample daily data to weekly to calculate signals ---
            historical_slice_daily = hist_data.loc[:date]
            historical_slice_weekly = historical_slice_daily.resample('W-MON').last()

            valid_tickers_this_month = [
                ticker for ticker in original_tickers 
                if ticker in historical_slice_weekly.columns and len(historical_slice_weekly[ticker].dropna()) >= 55
            ]

            if not valid_tickers_this_month:
                portfolio_value_history[date] = buying_power
                continue

            amplification = float(portfolio_config.get('amplification', 1.0))
            span_short, span_long = (8, 55)
            
            current_scores = {}
            for ticker in valid_tickers_this_month:
                # Calculate EMA on the weekly resampled data
                ema8 = historical_slice_weekly[ticker].ewm(span=span_short, adjust=False).mean()
                ema55 = historical_slice_weekly[ticker].ewm(span=span_long, adjust=False).mean()
                if not ema8.empty and not ema55.empty and not pd.isna(ema55.iloc[-1]) and ema55.iloc[-1] != 0:
                    ema_enter = (ema8.iloc[-1] - ema55.iloc[-1]) / ema55.iloc[-1]
                    current_scores[ticker] = ((ema_enter * 4) + 0.5) * 100

            weighted_scores = {}
            for i in range(1, num_portfolios + 1):
                tickers_str = portfolio_config.get(f'tickers_{i}', '')
                weight_pct = float(portfolio_config.get(f'weight_{i}', 0))
                cleaned_str = tickers_str.replace('"', '').replace("'", "")
                sub_portfolio_tickers = [t.strip() for t in cleaned_str.split(',')]
                
                total_amplified_score = 0
                amplified_scores_sub = {}
                for ticker in sub_portfolio_tickers:
                    if ticker in current_scores:
                        amplified_score = max(0, (current_scores[ticker] * amplification) - (amplification - 1) * 50)
                        amplified_scores_sub[ticker] = amplified_score
                        total_amplified_score += amplified_score
                
                for ticker, amp_score in amplified_scores_sub.items():
                    sub_alloc = (amp_score / total_amplified_score) if total_amplified_score > 0 else 0
                    final_weight = sub_alloc * (weight_pct / 100.0)
                    weighted_scores[ticker] = weighted_scores.get(ticker, 0) + final_weight

            total_weight = sum(weighted_scores.values())
            if total_weight > 0:
                target_weights = {ticker: weight / total_weight for ticker, weight in weighted_scores.items()}
                for ticker, weight in target_weights.items():
                    price = daily_prices.get(ticker)
                    if price and not pd.isna(price) and price > 0:
                        dollar_amount = buying_power * weight
                        shares_to_buy = dollar_amount / price
                        shares_owned[ticker] = shares_owned.get(ticker, 0) + shares_to_buy
            
            total_spent = sum(shares_owned.get(t,0) * daily_prices.get(t,0) for t in shares_owned if pd.notna(daily_prices.get(t)))
            buying_power -= total_spent

        # Calculate portfolio value for the current DAY
        current_holdings_value = sum(shares_owned.get(ticker, 0) * daily_prices.get(ticker, 0) for ticker in shares_owned if pd.notna(daily_prices.get(ticker)))
        portfolio_value_history[date] = buying_power + current_holdings_value

    # 5. Calculate and Display Final Performance
    portfolio_value_history.dropna(inplace=True)
    performance_metrics = calculate_backtest_performance(portfolio_value_history)
    ...
    # 6. Plot performance using daily data
    if 'SPY' in sim_data_daily.columns:
        spy_history_daily = sim_data_daily['SPY'].loc[portfolio_value_history.index] # Align SPY data
        plot_backtest_performance_graph(portfolio_value_history, spy_history_daily)
...
async def get_daily_change_for_tickers(tickers: List[str]) -> Dict[str, Dict[str, Any]]:
    """
    Fetches the live price, daily percentage change, and daily absolute change for a list of tickers.
    Includes a retry mechanism and explicitly sets auto_adjust=False for reliability.
    """
    if not tickers:
        return {}
    ...
    for attempt in range(max_retries):
        try:
            data = await asyncio.to_thread(
                ...
                interval="1d",
                progress=False,
                auto_adjust=False, # Explicitly set to get raw 'Close' price
                timeout=30 
            )
            ...
            results = {}
            # Use data['Close'] directly since auto_adjust=False ensures it's the primary price column
            close_prices = data['Close']
            if close_prices is None:
                return {ticker: {'error': 'Close price data not found in download'} for ticker in tickers}

            for ticker in tickers:
                ticker_series = close_prices
                if isinstance(close_prices, pd.DataFrame):
                    if ticker in close_prices.columns:
                        ticker_series = close_prices[ticker]
                    else:
                        results[ticker] = {'error': 'Ticker data not found in results'}
                        continue
                
                ticker_series = ticker_series.dropna()
                
                if len(ticker_series) > 1:
                    live_price = ticker_series.iloc[-1]
                    ...
                    if pd.notna(live_price) and pd.notna(prev_close) and prev_close != 0:
                        ...
                        change_abs = live_price - prev_close
                        results[ticker] = {'live_price': live_price, 'change_pct': change_pct, 'change_abs': change_abs}
                    else:
            ...
            print(f"  -> Data fetch attempt {attempt + 1}/{max_retries} failed: {e}")
            if attempt < max_retries - 1:
                await asyncio.sleep(retry_delay_seconds)
            else:
                print("  -> All fetch attempts failed. Returning error.")
                return {ticker: {'error': str(e)} for ticker in tickers}
    return {}

async def get_specific_economic_data() -> dict[str, str]:
    """
    Performs a live scrape for specific economic indicators and calculates the 
    1-year and 6-month change for the Dollar Index.
    """
    # This dictionary maps the exact text on the website to the desired output key.
    ...
    # Initialize the data dictionary with the final, desired keys
    data = {
        "Dollar (1y)": "N/A",
        "GDP Growth Rate": "N/A",
    ...
    # 1. Fetch Currency (1Y & 6M % Change of Dollar Index) from Yahoo Finance
    try:
        # Fetch 1 year of data, which is enough for both calculations
        dxy = await asyncio.to_thread(yf.Ticker("DX-Y.NYB").history, period="1y")
        if not dxy.empty and len(dxy) > 126: # Need at least ~6 months of data
            # 1-Year Calculation
            start_price_1y = dxy['Close'].iloc[0]
            end_price = dxy['Close'].iloc[-1]
            yoy_change = ((end_price - start_price_1y) / start_price_1y) * 100
            
            # 6-Month Calculation (approx. 126 trading days)
            start_price_6m = dxy['Close'].iloc[-126]
            six_month_change = ((end_price - start_price_6m) / start_price_6m) * 100

            # Combine into final string
            data["Dollar (1y)"] = f"{yoy_change:.2f}% (6M: {six_month_change:+.2f}%)"
            
    except Exception as e:
        print(f"   -> Warning: Could not fetch Dollar Index data (DX-Y.NYB): {e}")

    # 2. Scrape all other indicators from Trading Economics
    try:
        ...
        for row in soup.find_all('tr'):
            ...
            if indicator_name in indicators_to_scrape:
                ...
                output_key = indicators_to_scrape[indicator_name]
                data[output_key] = f"{actual_val}{suffix} (Prev: {prev_val}{suffix})"

    except Exception as e:
        print(f"  -> Warning: Live scrape for economic indicators failed: {type(e).__name__}")
...
Third Stage: Cognis - 02/08/25 to Present                          
...
"""
    ...
    for char_art in full_ascii_message:
        ...
        py_time.sleep(0.005) # Typing animation speed for each character
...
async def handle_invest_command(args: List[str], ai_params: Optional[Dict] = None, is_called_by_ai: bool = False, return_structured_data: bool = False):
    ...
    if ai_params: # AI Call
        ...
        # *** MODIFICATION START ***
        # If another function calls this one for data, return the structured results.
...
async def run_breakout_analysis_singularity(is_called_by_ai: bool = False) -> dict:
    ...
    if not is_called_by_ai:
        print("\n--- Breakout Analysis Results (Top 30 for CLI) ---")
        if df_data_for_csv_and_return:
            ...
            if actual_display_cols:
                 print(tabulate(pd.DataFrame(df_data_for_csv_and_return)[actual_display_cols].head(30), headers="keys", tablefmt="pretty", showindex=False))
            else:
...
async def handle_briefing_command(args: List[str], ai_params: Optional[Dict] = None, is_called_by_ai: bool = False):
    """
    Generates a daily market briefing.
    Updated to consolidate data fetching and correctly handle all market assets.
    """
    if not is_called_by_ai:
        print("\n--- Generating Daily Market Briefing ---")

    # This section for finding the watchlist remains unchanged.
    watchlist_tickers = []
    watchlist_message = None
    user_prefs = load_user_preferences()
    
    if 'favorite_tickers' in user_prefs and user_prefs['favorite_tickers']:
        watchlist_tickers = user_prefs['favorite_tickers']
        if not is_called_by_ai:
            print(f"Info: Using your saved favorite tickers for the watchlist: {', '.join(watchlist_tickers)}")
    elif not os.path.exists(USERS_FAVORITES_FILE):
        if is_called_by_ai:
    ...
    if not is_called_by_ai:
        print("➪ Briefing Step 1/4: Fetching Market, Risk, and Economic Data...")
    
    # CONSOLIDATED FETCH: Get all price-like data in one robust call.
    tasks1 = {
        "market_data": get_daily_change_for_tickers(['SPY', '^VIX', 'GLD', 'SLV', 'BTC-USD', '^TNX']),
        "risk": perform_risk_calculations_singularity(is_called_by_ai=True),
        "economic_data": get_specific_economic_data(), # Use the new function here
    }
    ...
    if not is_called_by_ai:
        print("➪ Briefing Step 2/4: Analyzing S&P 500 Movers...")
    ...
    market_results = results_dict.get('market_data', {})
    spy_data = market_results.get('SPY', {})
    vix_data = market_results.get('^VIX', {})
    gld_data = market_results.get('GLD', {})
    slv_data = market_results.get('SLV', {})
    btc_data = market_results.get('BTC-USD', {})
    tnx_data = market_results.get('^TNX', {}) 
    
    briefing_data['intro'] = {
        'spy_price': spy_data.get('live_price'), 'spy_change': spy_data.get('change_pct'),
        'vix_price': vix_data.get('live_price'), 'vix_change': vix_data.get('change_pct'),
        'gld_price': gld_data.get('live_price'), 'gld_change': gld_data.get('change_pct'),
        'slv_price': slv_data.get('live_price'), 'slv_change': slv_data.get('change_pct'),
        'btc_price': btc_data.get('live_price'), 'btc_change': btc_data.get('change_pct'),
        'treasury_yield': tnx_data.get('live_price'),
        'treasury_change': tnx_data.get('change_abs'),
    }
    
    briefing_data['economic'] = results_dict.get('economic_data', {})
    ...
    briefing_data['breakouts'] = { 'newly_added': [item['Ticker'] for item in breakout_results.get('newly_added_stocks', [])], 'removed': breakout_results.get('removed_stocks', []), 'top_3': breakout_results.get('current_breakout_stocks', [])[:3], 'top_3_performance': top_3_perf_data }
    if 'watchlist' in results_dict and results_dict.get('watchlist'):
        ...
        if valid_watchlist:
            ...
            briefing_data['watchlist'] = { 'top': valid_watchlist[:3], 'bottom': list(reversed(valid_watchlist[-3:])) }
    elif watchlist_message:
        briefing_data['watchlist'] = { 'error_message': watchlist_message }
    ...
    def format_asset(price, change, prefix="$", price_prec=2, chg_prec=2):
        if price is not None and pd.notna(price) and change is not None and pd.notna(change):
            return f"{prefix}{price:,.{price_prec}f} ({change:+,.{chg_prec}f}%)"
        return "N/A"

    spy_str = format_asset(intro.get('spy_price'), intro.get('spy_change'))
    vix_str = format_asset(intro.get('vix_price'), intro.get('vix_change'))
    gld_str = format_asset(intro.get('gld_price'), intro.get('gld_change'))
    slv_str = format_asset(intro.get('slv_price'), intro.get('slv_change'))
    btc_str = format_asset(intro.get('btc_price'), intro.get('btc_change'))
    
    yield_val = intro.get('treasury_yield')
    yield_chg = intro.get('treasury_change')
    yield_str = "N/A"
    if yield_val is not None and pd.notna(yield_val) and yield_chg is not None and pd.notna(yield_chg):
        yield_str = f"{float(yield_val):.2f}% ({float(yield_chg):+.2f})"

    print(f"SPY: {spy_str} | VIX: {vix_str} | GLD: {gld_str} | SLV: {slv_str} | BTC: {btc_str} | 10Y Yield: {yield_str}")
    ...
    print("\n--- Key Economic Data ---")
    econ_data = briefing_data.get('economic', {})
    econ_table_data = [[key, value] for key, value in econ_data.items()]
    print(tabulate(econ_table_data, headers=["Indicator", "Value"], tablefmt="pretty"))
...
async def calculate_volatility_metrics(ticker: str, period: str) -> tuple[Optional[float], Optional[float]]:
    """
    Calculates current Implied Volatility (from options) and Historical Volatility Rank.
    The rank is the percentile of the current volatility within its historical range for the given period.
    Returns (current_iv, vol_rank)
    """
    if 'percentileofscore' not in globals() or percentileofscore is None:
        return None, None # Return None if scipy is not installed
        
    try:
        stock_yf = yf.Ticker(ticker)

        # 1. Calculate Historical Volatility and its Rank
        hist_data = await asyncio.to_thread(stock_yf.history, period=period, interval="1d")
        vol_rank = None
        if not hist_data.empty and len(hist_data) > 30:
            ...
            if len(hv_series) > 1:
                ...
                vol_rank = percentileofscore(hv_series, current_hv, kind='rank')

        # 2. Get Current IV from options chain using a robust method
        current_iv = None
        try:
            exp_dates_str = await asyncio.to_thread(lambda: stock_yf.options)
            if exp_dates_str:
                # Find the expiration date closest to 30 days from now
                today = datetime.now().date()
                exp_dates = [datetime.strptime(d, '%Y-%m-%d').date() for d in exp_dates_str]
                target_date = today + timedelta(days=30)
                closest_exp_date = min(exp_dates, key=lambda d: abs(d - target_date))
                
                opt_chain = await asyncio.to_thread(stock_yf.option_chain, closest_exp_date.strftime('%Y-%m-%d'))
                
                # Get the ATM (at-the-money) straddle's average IV
                current_price = hist_data['Close'].iloc[-1]
                atm_strike = opt_chain.calls.iloc[(opt_chain.calls['strike'] - current_price).abs().argsort()[0]]['strike']
                
                atm_call_iv = opt_chain.calls[opt_chain.calls['strike'] == atm_strike]['impliedVolatility'].iloc[0]
                atm_put_iv = opt_chain.puts[opt_chain.puts['strike'] == atm_strike]['impliedVolatility'].iloc[0]
                
                if pd.notna(atm_call_iv) and pd.notna(atm_put_iv):
                    current_iv = ((atm_call_iv + atm_put_iv) / 2) * 100
                elif pd.notna(atm_call_iv):
                    current_iv = atm_call_iv * 100
                elif pd.notna(atm_put_iv):
                    current_iv = atm_put_iv * 100
        except Exception:
            # This can fail if options data doesn't exist for the ticker.
            current_iv = None
        
        return current_iv, vol_rank

    except Exception:
        # This will catch errors in the broader history fetch or HVR calculation.
        return None, None
        ...
        try:
            ...
            for ticker_a_item in tickers_list_a:
                try:
                    ...
                    iv_display = f"{current_iv:.1f}%" if current_iv is not None else "N/A"
...
def calculate_market_iv_risk(eod_csv_file_path: str = RISK_EOD_CSV_FILE, main_csv_file_path: str = RISK_CSV_FILE, is_called_by_ai: bool = False) -> Optional[float]:
    """
    Calculates Market Implied Volatility (IV) based on EOD Raw Market Invest Scores,
    with a fallback to the main data file if EOD data is insufficient.
    """
    df_source = None
    score_column_name = None
    source_file_info = ""

    # Primary source: EOD file
    if os.path.exists(eod_csv_file_path):
        try:
            df_eod = pd.read_csv(eod_csv_file_path, on_bad_lines='skip')
            if not df_eod.empty and 'Raw Market Invest Score (EOD)' in df_eod.columns and 'Date' in df_eod.columns:
                ...
                if len(eod_scores_series) >= 21:
                    df_source = df_eod
                    score_column_name = 'Raw Market Invest Score (EOD)'
                    source_file_info = f"EOD file ({eod_csv_file_path})"
                    risk_logger.info(f"Using EOD data for Market IV calculation.")
        except Exception as e:
            risk_logger.error(f"Error processing EOD file {eod_csv_file_path} for Market IV: {e}")

    # Fallback source: Main data file
    if df_source is None:
        if not is_called_by_ai:
            print("INFO: EOD data insufficient for Market IV. Attempting fallback to main history file.")
        risk_logger.info("EOD data insufficient for Market IV. Attempting fallback to main history file.")
        if os.path.exists(main_csv_file_path):
            try:
                df_main = pd.read_csv(main_csv_file_path, on_bad_lines='skip')
                if not df_main.empty and 'Raw Market Invest Score' in df_main.columns and 'Timestamp' in df_main.columns:
                    df_main['Timestamp'] = pd.to_datetime(df_main['Timestamp'], errors='coerce')
                    df_main = df_main.sort_values(by='Timestamp', ascending=True).dropna(subset=['Timestamp'])
                    main_scores_series = pd.to_numeric(df_main['Raw Market Invest Score'], errors='coerce').dropna()
                    if len(main_scores_series) >= 21:
                        df_source = df_main
                        score_column_name = 'Raw Market Invest Score'
                        source_file_info = f"main data file ({main_csv_file_path})"
                        risk_logger.info(f"Using main data file as fallback for Market IV calculation.")
            except Exception as e:
                risk_logger.error(f"Error processing main data file {main_csv_file_path} for Market IV fallback: {e}")

    if df_source is None or score_column_name is None:
        risk_logger.warning(f"No sufficient data source found for Market IV calculation in either EOD or main file.")
        return None

    try:
        scores_series = pd.to_numeric(df_source[score_column_name], errors='coerce').dropna()
        daily_changes_mis = scores_series.diff().abs().dropna()

        if len(daily_changes_mis) < 20:
            risk_logger.info(f"Insufficient daily changes ({len(daily_changes_mis)}) in {source_file_info} for Market IV (need >= 20).")
            return None

        average_daily_mis_change_magnitude = daily_changes_mis.tail(20).mean()
        if pd.isna(average_daily_mis_change_magnitude):
            risk_logger.warning(f"Average daily MIS change magnitude is NaN from {source_file_info}. Cannot calculate Market IV.")
            return None

        scaling_factor = 10000.0
        expressed_percentage_daily_change = average_daily_mis_change_magnitude / scaling_factor
        growth_factor_annualized = (1 + expressed_percentage_daily_change) ** 252
        market_iv_calculated = (growth_factor_annualized - 1) * 100

        return float(market_iv_calculated)
    except Exception as e:
        risk_logger.exception(f"Error during final Market IV calculation using source {source_file_info}:")
        return None
    
# --- Core R.I.S.K. Orchestration Function ---
def calculate_percentile_rank(current_value, historical_values):
    ...
    if current_value is None or not historical_values:
        return 0.0  # Return 0 if no history or current value
    
    # Count how many historical values are less than or equal to the current value
    lower_or_equal_count = sum(1 for val in historical_values if val <= current_value)
    
    # Calculate percentile
    percentile = (lower_or_equal_count / len(historical_values)) * 100
    return percentile
...
async def perform_risk_calculations_singularity(is_eod_save: bool = False, is_called_by_ai: bool = False):
    ...
    results_summary = {
        ...
        "raw_market_invest_score": 'N/A',
        "market_score_percentile": 'N/A',
        "market_invest_score": 'N/A',
        "market_iv_eod": 'N/A',
        "market_ivr": 'N/A',
    ...
    # Updated call to the modified function
    market_iv_val = await asyncio.to_thread(calculate_market_iv_risk, RISK_EOD_CSV_FILE, RISK_CSV_FILE, is_called_by_ai=True)

    invest_score_percentile_val = 0.0
    market_ivr_val = 0.0
    ...
    if os.path.exists(RISK_CSV_FILE):
        try:
            ...
            if not df_hist.empty:
                historical_raw_scores = pd.to_numeric(df_hist['Raw Market Invest Score'], errors='coerce').dropna().tolist()
                historical_ivs = pd.to_numeric(df_hist['Market IV'], errors='coerce').dropna().tolist()

                if uncapped_mis is not None and historical_raw_scores:
                    invest_score_percentile_val = calculate_percentile_rank(uncapped_mis, historical_raw_scores)
                
                if market_iv_val is not None and historical_ivs:
                    market_ivr_val = calculate_percentile_rank(market_iv_val, historical_ivs)
        except Exception as e:
            risk_logger.error(f"Error reading historical data for percentile calculations: {e}")
    ...
    if os.path.exists(RISK_CSV_FILE):
        try:
            ...
            if not df_hist_signal.empty and 'Raw Market Invest Score' in df_hist_signal.columns:
                df_hist_signal['Signal_Score'] = np.clip(pd.to_numeric(df_hist_signal['Raw Market Invest Score'], errors='coerce'), 0, 100)
                valid_prev_scores = df_hist_signal['Signal_Score'].dropna()
                if not valid_prev_scores.empty:
                    previous_capped_mis_from_csv = valid_prev_scores.iloc[-1]
    ...
    results_summary.update({
        ...
        "market_score_percentile": f"{invest_score_percentile_val:.1f}%",
        "market_invest_score": f"{capped_mis_signal:.2f}" if capped_mis_signal is not None else 'N/A',
        "market_iv_eod": f"{market_iv_val:.2f}" if market_iv_val is not None else 'N/A',
        "market_ivr": f"{market_ivr_val:.1f}%",
        "market_signal": risk_persistent_signal,
        "signal_date_info": signal_day_str_for_summary.strip()
    })

    timestamp_iso_utc = datetime.now(pytz.UTC).isoformat()
    main_csv_fieldnames = ['Timestamp', 'General Market Score', 'Large Market Cap Score', 'EMA Score', 'Combined Score',
                           ...
                           'Raw Market Invest Score', 'Market Score Percentile', 'Market IV', 'Market IVR']
    try:
        main_file_exists = os.path.exists(RISK_CSV_FILE)
        with open(RISK_CSV_FILE, 'a', newline='', encoding='utf-8') as csvfile_main:
            ...
            row_to_write_main = {
                ...
                'Market Score Percentile': results_summary["market_score_percentile"],
                'Market IV': results_summary["market_iv_eod"],
    ...
    if is_eod_save and uncapped_mis is not None and market_iv_val is not None:
        eod_csv_fieldnames = ['Date', 'Raw Market Invest Score (EOD)', 'Market IV']
        try:
            ...
            with open(RISK_EOD_CSV_FILE, 'a', newline='', encoding='utf-8') as eod_csvfile_out:
                ...
                eod_row_to_write = {
                    'Date': datetime.now(EST_TIMEZONE).strftime('%Y-%m-%d'),
                    'Raw Market Invest Score (EOD)': f"{uncapped_mis:.2f}",
                    'Market IV': f"{market_iv_val:.2f}"
                }
    ...
    if not is_called_by_ai:
        ...
        print(f"  Score Percentile: {results_summary['market_score_percentile']}")
        print(f"  Market IV: {results_summary['market_iv_eod']}")
...
async def generate_risk_graphs_singularity(is_called_by_ai: bool = False) -> dict:
    ...
    df_main_risk_hist['Timestamp'] = pd.to_datetime(df_main_risk_hist['Timestamp'], errors='coerce').dt.tz_localize(None)
    df_main_risk_hist = df_main_risk_hist.sort_values(by='Timestamp').dropna(subset=['Timestamp'])

    plt.style.use('dark_background')
    key_color = 'white'; grid_color = 'gray'; fig_size_tuple = (14, 7)

    # Graph 1: Market Scores
    try:
        plt.figure(figsize=fig_size_tuple)
        score_cols_to_plot = ['General Market Score', 'Large Market Cap Score', 'EMA Score', 'Combined Score']
        for col_name in score_cols_to_plot:
            if col_name in df_main_risk_hist.columns:
                plt.plot(df_main_risk_hist['Timestamp'], pd.to_numeric(df_main_risk_hist[col_name], errors='coerce'), label=col_name.replace(' Score','S').replace(' Market','M'), linewidth=1.5)
    ...
    try:
        ...
        ax1_spy.tick_params(axis='x', colors=key_color, rotation=25)
        ax2_vix = ax1_spy.twinx()
    ...
    try:
        ...
        for col_name_rec in recession_cols_to_plot:
            if col_name_rec in df_main_risk_hist.columns:
                ...
                plt.plot(df_main_risk_hist['Timestamp'], numeric_series_rec, label=col_name_rec.replace(' Chance','').replace(' Based',''), linewidth=1.5)
    ...
    # Graph 4: Percentile Ranks
    try:
        plt.figure(figsize=fig_size_tuple)
        percentile_cols = {'Market Score Percentile': 'cyan', 'Market IVR': 'fuchsia'}
        for col, color in percentile_cols.items():
            if col in df_main_risk_hist.columns:
                 numeric_data = pd.to_numeric(df_main_risk_hist[col].astype(str).str.rstrip('%'), errors='coerce')
                 plt.plot(df_main_risk_hist['Timestamp'], numeric_data, label=col, color=color, linewidth=1.5)
        
        plt.title('Historical Percentile Ranks', color=key_color, fontsize=14)
        plt.xlabel('Timestamp', color=key_color); plt.ylabel('Percentile Rank (%)', color=key_color)
        plt.legend(labelcolor=key_color, fontsize='small'); plt.grid(True, color=grid_color, linestyle=':')
        plt.tick_params(axis='x', colors=key_color, rotation=20); plt.tick_params(axis='y', colors=key_color)
        plt.ylim(0, 105); plt.tight_layout()
        filename4 = f"risk_percentile_ranks_hist_{uuid.uuid4().hex[:6]}.png"
        plt.savefig(filename4, facecolor='black'); graph_files_generated_list.append(filename4); plt.close()
    except Exception as e_g4_plot:
        if not is_called_by_ai: print(f"Error generating percentile ranks graph: {e_g4_plot}")
        
    # Graph 5: Market IV
    try:
        if 'Market IV' in df_main_risk_hist.columns:
            plt.figure(figsize=fig_size_tuple)
            numeric_iv_series = pd.to_numeric(df_main_risk_hist['Market IV'], errors='coerce')
            plt.plot(df_main_risk_hist['Timestamp'], numeric_iv_series, label='Market IV', color='yellow', linewidth=2)
            plt.title('Historical Market IV', color=key_color, fontsize=16)
            plt.xlabel('Timestamp', color=key_color); plt.ylabel('Market IV', color=key_color)
            plt.legend(labelcolor=key_color, fontsize='small'); plt.grid(True, color=grid_color, linestyle=':')
            plt.tick_params(axis='x', colors=key_color, rotation=25); plt.tick_params(axis='y', colors=key_color)
            plt.tight_layout()
            filename5 = f"risk_market_iv_hist_{uuid.uuid4().hex[:6]}.png"
            plt.savefig(filename5, facecolor='black'); graph_files_generated_list.append(filename5); plt.close()
    except Exception as e_g5_plot:
        if not is_called_by_ai: print(f"Error generating Market IV graph: {e_g5_plot}")
...
async def handle_macd_forecast_command(args: List[str], ai_params: Optional[Dict] = None, is_called_by_ai: bool = False):
    ...
    Downloads one-year historical closing prices for a list of stocks and calculates the MACD.
    Based on specific conditions related to recent MACD changes, it forecasts a future price
    and the date that price will be reached. A graph visualizing the past month's prices
    and the forecast is generated for each stock.
    """
    if not is_called_by_ai:
    ...
    if ai_params:
        ...
        # FIX: Added .strip(',') to remove unwanted commas from each ticker
        tickers = [t.strip().strip(',').upper().replace('"', '') for t in tickers_str.replace(',', ' ').split() if t.strip()]
    elif args:
        # FIX: Added .strip(',') to remove unwanted commas from each ticker
        tickers = [t.strip().strip(',').upper().replace('"', '') for t in args]
    else:
        tickers_input = input("Enter a list of stock tickers separated by spaces or commas: ")
        # FIX: Added .strip(',') to remove unwanted commas from each ticker
        tickers = [ticker.strip().strip(',').upper().replace('"', '') for ticker in tickers_input.replace(',', ' ').split() if ticker.strip()]
    ...
    for ticker in tickers:
        try:
            # Download one year of historical data in a thread
            end_date = datetime.today()
            start_date = end_date - timedelta(days=365)
            data_download = await asyncio.to_thread(yf.download, ticker, start=start_date, end=end_date, progress=False, group_by='ticker', auto_adjust=False)

            # --- FIX: Handle MultiIndex columns from yfinance ---
            if isinstance(data_download.columns, pd.MultiIndex):
                # Flatten the MultiIndex and create an explicit copy to avoid SettingWithCopyWarning
                data = data_download[ticker].copy()
            else:
                data = data_download.copy()


            # --- FIX: Robust check for valid data and 'Close' column ---
            if data.empty or 'Close' not in data.columns or data['Close'].isnull().all():
                summary = f"Forecast for {ticker}: Could not be produced. No valid data was downloaded."
                if not is_called_by_ai: print(summary); print("-" * 60)
                all_summaries.append(summary)
                continue

            # Calculate EMAs and MACD
            data['ema_25'] = data['Close'].ewm(span=25, adjust=False).mean()
            data['ema_75'] = data['Close'].ewm(span=75, adjust=False).mean()
            data['macd'] = data['ema_25'] - data['ema_75']

            if len(data['macd'].dropna()) < 4:
                summary = f"Forecast for {ticker}: Could not be produced. Not enough data for MACD analysis."
                if not is_called_by_ai: print(summary); print("-" * 60)
                all_summaries.append(summary)
                continue
            ...
            if macd_changes.isnull().any():
                summary = f"Forecast for {ticker}: Could not be produced. Insufficient data for MACD change calculation."
                if not is_called_by_ai: print(summary); print("-" * 60)
                all_summaries.append(summary)
                continue

            macd_change_1, macd_change_2, macd_change_3 = macd_changes.iloc[-1], macd_changes.iloc[-2], macd_changes.iloc[-3]
            condition1 = abs(macd_change_1) < abs(macd_change_2) < abs(macd_change_3)
            condition2 = (macd_change_1 > 0 and macd_change_2 > 0 and macd_change_3 > 0) or \
                         (macd_change_1 < 0 and macd_change_2 < 0 and macd_change_3 < 0)

            if not (condition1 and condition2):
                summary = f"Forecast for {ticker}: Could not be produced due to failed conditions."
                if not is_called_by_ai: print(summary); print("-" * 60)
                all_summaries.append(summary)
                continue
            ...
            if pd.isna(avg_macd_change) or avg_macd_change == 0:
                summary = f"Forecast for {ticker}: Could not be produced. The average MACD change is zero or invalid."
                if not is_called_by_ai: print(summary); print("-" * 60)
                all_summaries.append(summary)
                continue

            forecast_day_count = int(abs(last_macd / avg_macd_change))
            if forecast_day_count <= 0 or forecast_day_count >= 365:
                summary = f"Forecast for {ticker}: Could not be produced. Projected day count ({forecast_day_count}) is not in a reasonable range."
                if not is_called_by_ai: print(summary); print("-" * 60)
                all_summaries.append(summary)
                continue
            ...
            similar_macd_changes = data[(data['macd_change_full'] >= avg_macd_change - tolerance) & (data['macd_change_full'] <= avg_macd_change + tolerance)]

            if similar_macd_changes.empty:
                summary = f"Forecast for {ticker}: Could not be produced. Could not find similar historical instances."
                if not is_called_by_ai: print(summary); print("-" * 60)
                all_summaries.append(summary)
                continue
            ...
            if pd.isna(avg_price_change):
                summary = f"Forecast for {ticker}: Could not be produced. Could not calculate a valid average price change."
                if not is_called_by_ai: print(summary); print("-" * 60)
                all_summaries.append(summary)
                continue

            last_price_float = data['Close'].iloc[-1]
            forecasted_price_val = last_price_float * ((1 + avg_price_change) ** forecast_day_count)
            percent_change = ((forecasted_price_val - last_price_float) / last_price_float) * 100
            last_date_label = data.index[-1]
            forecast_date = pd.to_datetime(last_date_label) + pd.tseries.offsets.BusinessDay(n=forecast_day_count)
            time_away = forecast_date.date() - end_date.date()

            ticker_summary = (f"Forecast for {ticker}: Successful. "
                              f"Forecasted Price: ${forecasted_price_val:.2f} ({percent_change:+.2f}%) "
                              f"on {forecast_date.strftime('%Y-%m-%d')} ({time_away.days} days).")
            all_summaries.append(ticker_summary)

            if not is_called_by_ai:
                print(f"\nForecast for {ticker}: Successful")
                print(f"  - Forecasted Price: ${forecasted_price_val:.2f}")
                print(f"  - Percent Change: {percent_change:+.2f}%")
                print(f"  - Forecast Date: {forecast_date.strftime('%Y-%m-%d')}")
                print(f"  - Time Away: {time_away.days} days (~{time_away.days / 7.0:.1f} weeks)")
                print("-" * 60)

                # Generate graph in a thread
                await asyncio.to_thread(
                    plot_macd_forecast_graph,
                    ticker,
                    data,
                    forecast_day_count,
                    avg_price_change,
            ...
            summary = f"An error occurred for {ticker}: {e}"
            if not is_called_by_ai: print(summary); print("-" * 60)
            all_summaries.append(summary)
...
def plot_macd_forecast_graph(ticker, data, forecast_day_count, avg_price_change, forecast_date, is_called_by_ai):
    ...
    try:
        ...
        ax.plot(past_month_data.index, past_month_data['Close'], label='Past Month Prices', color='grey', marker='.')

        if forecast_day_count > 0:
            ...
            # Annotate the final forecast point
...
async def handle_favorites_command(args: List[str], is_called_by_ai: bool = False):
    """
    Allows the user to view and overwrite their saved favorite tickers list.
    This is a CLI-only command.
    """
    if is_called_by_ai:
        return "This command is for direct user interaction via the CLI."
    ...
    if current_favorites:
        print(f"Your current saved list is: {', '.join(current_favorites)}")
    else:
        print("You do not have any saved favorite tickers yet.")

    # Get new list from user
    new_list_str = input("\nEnter your new comma-separated list of tickers to overwrite the list (or press Enter to cancel): ").strip()

    if not new_list_str:
        print("Operation cancelled. Your favorites list was not changed.")
        return

    # Process and save the new list
    new_tickers_list = sorted(list(set([ticker.strip().upper() for ticker in new_list_str.split(',') if ticker.strip()])))
    
    # Use the existing preference-saving tool to ensure data is stored correctly
    save_result = await update_user_preference_tool(key='favorite_tickers', value=new_tickers_list, is_called_by_ai=True)

    if save_result.get("status") == "success":
        if new_tickers_list:
            print(f"\n✅ Success! Your favorites list has been updated to: {', '.join(new_tickers_list)}")
        else:
            print("\n✅ Success! Your favorites list has been cleared.")
    else:
...
def get_gics_map():
    """Returns a dictionary mapping GICS codes to names."""
    return {
        '10': 'Energy', '1010': 'Energy', '101010': 'Energy Equipment & Services', '10101010': 'Oil & Gas Drilling',
        '10101020': 'Oil & Gas Equipment & Services', '101020': 'Oil, Gas & Consumable Fuels', '10102010': 'Integrated Oil & Gas',
        '10102020': 'Oil & Gas Exploration & Production', '10102030': 'Oil & Gas Refining & Marketing', '10102040': 'Oil & Gas Storage & Transportation',
        '10102050': 'Coal & Consumable Fuels', '15': 'Materials', '1510': 'Materials', '151010': 'Chemicals',
        '15101010': 'Commodity Chemicals', '15101020': 'Diversified Chemicals', '15101030': 'Fertilizers & Agricultural Chemicals',
        '15101040': 'Industrial Gases', '15101050': 'Specialty Chemicals', '151020': 'Construction Materials', '15102010': 'Construction Materials',
        '151030': 'Containers & Packaging', '15103010': 'Metal, Glass & Plastic Containers', '15103020': 'Paper & Forest Products',
        '151040': 'Metals & Mining', '15104010': 'Aluminum', '15104020': 'Diversified Metals & Mining', '15104025': 'Copper',
        ...
        '201020': 'Building Products', '20102010': 'Building Products', '201030': 'Construction & Engineering', '20103010': 'Construction & Engineering',
        '201040': 'Electrical Equipment', '20104010': 'Electrical Components & Equipment', '20104020': 'Heavy Electrical Equipment',
        '201050': 'Industrial Conglomerates', '20105010': 'Industrial Conglomerates', '201060': 'Machinery', '20106010': 'Construction Machinery & Heavy Trucks',
        '20106015': 'Agricultural & Farm Machinery', '20106020': 'Industrial Machinery & Supplies & Components',
        '201070': 'Trading Companies & Distributors', '20107010': 'Trading Companies & Distributors', '2020': 'Commercial & Professional Services',
        '202010': 'Commercial Services & Supplies', '20201010': 'Commercial Printing', '20201050': 'Environmental & Facilities Services',
        '20201060': 'Office Services & Supplies', '20201070': 'Diversified Support Services', '20201080': 'Security & Alarm Services',
        '202020': 'Professional Services', '20202010': 'Human Resource & Employment Services', '20202020': 'Research & Consulting Services',
        '2030': 'Transportation', '203010': 'Air Freight & Logistics', '20301010': 'Air Freight & Logistics', '203020': 'Airlines',
        '20302010': 'Airlines', '203030': 'Marine', '20303010': 'Marine', '203040': 'Road & Rail', '20304010': 'Railroads',
        '20304020': 'Road Transportation', '203050': 'Transportation Infrastructure', '20305010': 'Airport Services',
        '20305020': 'Highways & Railtracks', '20305030': 'Marine Ports & Services',
        '25': 'Consumer Discretionary', '2510': 'Automobiles & Components', '251010': 'Automobile Components',
        '25101010': 'Automotive Parts & Equipment', '25101020': 'Tires & Rubber', '251020': 'Automobiles', '25102010': 'Automobile Manufacturers',
        '25102020': 'Motorcycle Manufacturers', '2520': 'Consumer Durables & Apparel', '252010': 'Household Durables',
        '25201010': 'Consumer Electronics', '25201020': 'Home Furnishings', '25201030': 'Homebuilding', '25201040': 'Household Appliances',
        '25201050': 'Housewares & Specialties', '252020': 'Leisure Products', '25202010': 'Leisure Products', '252030': 'Textiles, Apparel & Luxury Goods',
        ...
        '25301020': 'Hotels, Resorts & Cruise Lines', '25301030': 'Leisure Facilities', '25301040': 'Restaurants',
        '253020': 'Diversified Consumer Services', '25302010': 'Education Services', '25302020': 'Specialized Consumer Services',
        '2550': 'Retailing', '255010': 'Distributors', '25501010': 'Distributors', '255020': 'Internet & Direct Marketing Retail',
        '25502020': 'Internet & Direct Marketing Retail', '255030': 'Multiline Retail', '25503010': 'Department Stores', '25503020': 'General Merchandise Stores',
        '255040': 'Specialty Retail', '25504010': 'Apparel Retail', '25504020': 'Computer & Electronics Retail', '25504030': 'Home Improvement Retail',
        '25504040': 'Specialty Stores', '25504050': 'Automotive Retail', '25504060': 'Homefurnishing Retail',
        '30': 'Consumer Staples', '3010': 'Food & Staples Retailing', '301010': 'Food & Staples Retailing',
        '30101010': 'Drug Retail', '30101020': 'Food Distributors', '30101030': 'Food Retail', '30101040': 'Hypermarkets & Super Centers',
        '3020': 'Food, Beverage & Tobacco', '302010': 'Beverages', '30201010': 'Brewers', '30201020': 'Distillers & Vintners',
        '30201030': 'Soft Drinks & Non-alcoholic Beverages', '302020': 'Food Products', '30202010': 'Agricultural Products & Services',
        '30202030': 'Packaged Foods & Meats', '302030': 'Tobacco', '30203010': 'Tobacco',
        ...
        '35101010': 'Health Care Equipment', '35101020': 'Health Care Supplies', '351020': 'Health Care Providers & Services',
        '35102010': 'Health Care Distributors', '35102015': 'Health Care Services', '35102020': 'Health Care Facilities',
        '35102030': 'Managed Health Care', '351030': 'Health Care Technology', '35103010': 'Health Care Technology',
        '3520': 'Pharmaceuticals, Biotechnology & Life Sciences', '352010': 'Biotechnology', '35201010': 'Biotechnology',
        '352020': 'Pharmaceuticals', '35202010': 'Pharmaceuticals', '352030': 'Life Sciences Tools & Services', '35203010': 'Life Sciences Tools & Services',
        '40': 'Financials', '4010': 'Banks', '401010': 'Banks', '40101010': 'Diversified Banks', '40101015': 'Regional Banks',
        '4020': 'Financial Services', '402010': 'Financial Services', '40201020': 'Diversified Financial Services',
        '40201030': 'Multi-Sector Holdings', '40201040': 'Specialized Finance', '40201050': 'Commercial & Residential Mortgage Finance',
        '402020': 'Consumer Finance', '40202010': 'Consumer Finance', '402030': 'Capital Markets', '40203010': 'Asset Management & Custody Banks',
        '40203020': 'Investment Banking & Brokerage', '40203030': 'Diversified Capital Markets', '40203040': 'Financial Exchanges & Data',
        '4030': 'Insurance', '403010': 'Insurance', '40301010': 'Insurance Brokers', '40301020': 'Life & Health Insurance',
        '40301030': 'Multi-line Insurance', '40301040': 'Property & Casualty Insurance', '40301050': 'Reinsurance',
        '45': 'Information Technology', '4510': 'Software & Services', '451010': 'IT Services', '45101010': 'IT Consulting & Other Services',
        '45101020': 'Data Processing & Outsourced Services', '451020': 'Software', '45102010': 'Application Software',
        '45102020': 'Systems Software', '45102030': 'Home Entertainment Software',
        '451030': 'Communications Equipment', '45103010': 'Communications Equipment', '4520': 'Technology Hardware & Equipment',
        '452010': 'Technology Hardware, Storage & Peripherals', '45201020': 'Technology Hardware, Storage & Peripherals',
        '452020': 'Electronic Equipment, Instruments & Components', '45202010': 'Electronic Equipment & Instruments',
        '45202030': 'Electronic Components', '45202040': 'Electronic Manufacturing Services',
        '4530': 'Semiconductors & Semiconductor Equipment', '453010': 'Semiconductors & Semiconductor Equipment', '45301010': 'Semiconductor Materials & Equipment', '45301020': 'Semiconductors',
        '50': 'Communication Services', '5010': 'Telecommunication Services', '501010': 'Diversified Telecommunication Services',
        '50101010': 'Alternative Carriers', '50101020': 'Integrated Telecommunication Services', '501020': 'Wireless Telecommunication Services',
        '50102010': 'Wireless Telecommunication Services', '5020': 'Media & Entertainment', '502010': 'Media',
        '50201010': 'Advertising', '50201020': 'Broadcasting', '50201030': 'Cable & Satellite', '50201040': 'Publishing',
        '502020': 'Entertainment', '50202010': 'Movies & Entertainment', '50202020': 'Interactive Home Entertainment',
        ...
        '551020': 'Gas Utilities', '55102010': 'Gas Utilities', '551030': 'Multi-Utilities', '55103010': 'Multi-Utilities',
        '551040': 'Water Utilities', '55104010': 'Water Utilities', '551050': 'Independent Power and Renewable Electricity Producers',
        '55105010': 'Independent Power Producers & Energy Traders', '55105020': 'Renewable Electricity',
        '60': 'Real Estate', '6010': 'Real Estate', '601010': 'Equity Real Estate Investment Trusts (REITs)',
        '60101010': 'Diversified REITs', '60101020': 'Industrial REITs', '60101030': 'Hotel & Resort REITs', '60101040': 'Office REITs',
        '60101050': 'Health Care REITs', '60101060': 'Residential REITs', '60101070': 'Retail REITs', '60101080': 'Specialized REITs',
        '601020': 'Real Estate Management & Development', '60102010': 'Diversified Real Estate Activities',
        '60102020': 'Real Estate Operating Companies', '60102030': 'Real Estate Development', '60102040': 'Real Estate Services'
    }

def filter_stocks_by_gics(user_input_str: str, txt_path: str = 'gics_database.txt') -> set:
    """
    Filters tickers from a GICS text file based on user input of codes or names.
    The text file format should be: GICS_CODE:TICKER1,TICKER2,...
    """
    if not os.path.exists(txt_path):
        print(f"   -> ERROR: GICS ticker database '{txt_path}' not found.")
        return set()

    gics_map = get_gics_map()
    name_to_code_map = {name.lower(): code for code, name in gics_map.items()}
    
    # Read the data from the .txt file into a dictionary
    gics_data = {}
    try:
        ...
        print(f"   -> ERROR: Could not read or process GICS database '{txt_path}': {e}")
        return set()

    user_inputs = [item.strip() for item in user_input_str.split(',')]
    target_codes = set()

    for item in user_inputs:
        if item.isdigit():
            target_codes.add(item)
        elif item.lower() in name_to_code_map:
            target_codes.add(name_to_code_map[item.lower()])
        else:
            print(f"   -> Warning: Could not find a GICS code or name matching '{item}'. It will be skipped.")

    if not target_codes:
        print("   -> ERROR: No valid GICS codes or names were identified from your input.")
        return set()

    selected_tickers = set()
    # Iterate through the user's target codes
    for user_code in target_codes:
        # Iterate through the codes from the database file
        for db_code, tickers_str in gics_data.items():
            if db_code.startswith(user_code):
                tickers_from_row = [t.strip().upper() for t in tickers_str.split(',') if t.strip()]
                selected_tickers.update(tickers_from_row)
...
async def pre_screen_stocks_by_sensitivity(tickers: list, sensitivity: int) -> list:
    ...
    # Fetch data in chunks to be efficient
    chunk_size = 100
    screened_tickers = []
    ...
    for i in range(0, len(tickers), chunk_size):
        chunk = tickers[i:i+chunk_size]
        try:
            # Use yfinance download for faster bulk data fetching
            data = await asyncio.to_thread(
            ...
            # Calculate average volume over the period and get the latest market cap
            avg_vol = data['Volume'].mean()
            
            for ticker in chunk:
                try:
                    # yfinance info can be slow, but is the most reliable for market cap
                    stock_info = await asyncio.to_thread(lambda t=ticker: yf.Ticker(t).info)
                    market_cap = stock_info.get('marketCap', 0)
                    
                    # Get the average volume for the specific ticker
            ...
            # Skip chunk if download fails
            continue
            
    print(f"   -> After pre-screening, {len(screened_tickers)} stocks remain.")
    return screened_tickers

async def get_ai_stock_rationale(ticker: str, investment_goals: str, risk_tolerance: int) -> str:
    """
    Generates a brief, AI-powered rationale for including a stock in a portfolio.
    """
    global gemini_model
    if not gemini_model:
    ...
    try:
        ...
        prompt = f"""
        Given the following information about the user and a specific company:
        - User's Investment Goal: "{investment_goals}"
        - User's Risk Tolerance: {risk_tolerance}/5
        - Company: {ticker} (Sector: {sector})
        - Business Summary: "{summary}"

        Please provide a concise (2-3 sentences) rationale explaining what this company does and how its inclusion in the portfolio aligns with the user's stated goals and risk tolerance. Focus on clarity and relevance.
        """
        ...
        response = await asyncio.to_thread(
            ...
            prompt,
            generation_config=genai.types.GenerationConfig(temperature=0.7) # Add some creativity
        )
...
async def get_ai_trading_strategy(risk_tolerance: int, market_invest_score: float, strategy_name: str) -> str:
    """
    Generates a brief, reasonable trading strategy suggestion based on user inputs and market conditions.
    """
    global gemini_model
    if not gemini_model:
        return "AI model not available."
    ...
    try:
        prompt = f"""
        Based on the following inputs for an investment report:
        - User's Risk Tolerance: {risk_tolerance}/5
        - Current Market Invest Score: {market_invest_score:.2f}/100
        - Selected Strategy Name: "{strategy_name}"

        Generate a brief (3-4 sentences) 'Proposed Trading Strategy' section for the report. This should be general guidance and NOT financial advice. Cover the following points reasonably:
        1.  **Review Frequency:** Suggest how often the user might want to review this type of portfolio (e.g., quarterly, monthly).
        2.  **Rebalancing:** Mention the concept of rebalancing in simple terms.
        3.  **General Tone:** Maintain an informative but cautious tone, reminding the user of market volatility.
        """
...
async def handle_report_generation(args: List[str], ai_params: Optional[Dict] = None, is_called_by_ai: bool = False):
    """
    Orchestrates the end-to-end process of generating a custom investment report
    using ReportLab for PDF creation.
    """
    if is_called_by_ai:
        print("AI-driven report generation is not yet implemented. This is a CLI-only feature.")
        return
    ...
    # --- Step 2 & 3: Determine Strategy, Filter, and Score Stocks ---
    ...
    market_invest_score_str = risk_results.get("market_invest_score", "50.0").replace('%', '')
    ...
    # --- Step 5: Prepare Data and Generate PDF ---
    ...
    # Enhance holdings data with rationale and other info
    ...
    for holding in tailored_portfolio_data:
        ...
        holding['rationale'] = await get_ai_stock_rationale(holding['ticker'], investment_goals, risk_tolerance)
    ...
    # Package all data for the PDF function
    report_data_package = {
        ...
        "trading_strategy": await get_ai_trading_strategy(risk_tolerance, market_invest_score, strategy['name']),
        ...
        "cash_percentage": (final_cash / portfolio_value) * 100
    }

    # --- Step 6: Save the PDF file ---
    report_filename = f"Singularity_Report_RL_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
    try:
...
async def handle_heatmap_command(args: List[str], is_called_by_ai: bool = False):
    ...
    if not args:
        ...
        print("Examples: /heatmap AAPL MSFT GOOG")
        print("          /heatmap I-SPY")
        print("          /heatmap P-4")
        return
    ...
    # 1. Parse Input and Get Ticker List
    if input_arg.startswith('I-'):
        index_symbol = input_arg.split('I-')[1]
        title_prefix = f"Index_{index_symbol}"
        if not is_called_by_ai: print(f"-> Fetching tickers for index: {index_symbol}")
        ticker_list = await get_specific_index_tickers(index_symbol, is_called_by_ai=is_called_by_ai)
    
    elif input_arg.startswith('P-'):
        portfolio_code = input_arg.split('P-')[1]
        title_prefix = f"Portfolio_{portfolio_code}"
        if not is_called_by_ai: print(f"-> Fetching tickers for portfolio code: {portfolio_code}")
        if os.path.exists(PORTFOLIO_DB_FILE):
            try:
                df = pd.read_csv(PORTFOLIO_DB_FILE)
                portfolio_row = df[df['portfolio_code'].astype(str) == portfolio_code]
            ...
            print(f"-> Error: Portfolio database '{PORTFOLIO_DB_FILE}' not found.")

    else:
        ticker_list = [arg.upper() for arg in args]
        title_prefix = "_".join(ticker_list[:3])
        if len(ticker_list) > 3: title_prefix += "_and_others"
        if not is_called_by_ai: print(f"-> Using provided ticker list: {', '.join(ticker_list)}")
    ...
    # 2. Fetch Historical Data
    if not is_called_by_ai: print(f"-> Fetching 1-year historical data for {len(ticker_list)} tickers...")
    hist_data = await asyncio.to_thread(get_yf_data_singularity, ticker_list, period="1y", is_called_by_ai=True)

    if hist_data.empty:
        print("-> Error: Failed to download any valid historical data. Aborting.")
        return
    
    # 3. Calculate Returns and Correlation
    if not is_called_by_ai: print("-> Calculating daily returns and correlation matrix...")
    daily_returns = hist_data.pct_change().dropna()
    
    if daily_returns.empty:
        print("-> Error: Could not calculate daily returns. Aborting.")
        return
        
    correlation_matrix_daily = daily_returns.corr(method='pearson')

    # 4. Generate and Save Heatmap
    if not is_called_by_ai: print("-> Generating heatmap...")
    heatmap_title = f"Daily_Returns_Correlation_{title_prefix}"
    await asyncio.to_thread(plot_correlation_heatmap, correlation_matrix_daily, heatmap_title, is_called_by_ai)

    print("\n--- Heatmap Generation Complete ---")

async def get_index_tickers(is_called_by_ai: bool = False) -> set:
    """
    Fetches tickers from NASDAQ-100, DOW-100, and Russell 2000 from Wikipedia.
    Returns a set of unique and validated tickers.
    """
    if not is_called_by_ai:
        print("   -> Fetching additional index component lists...")

    ticker_set = set()
    # URLs for the indices
    urls = {
        "NASDAQ-100": 'https://en.wikipedia.org/wiki/Nasdaq-100',
        "DOW-30": 'https://en.wikipedia.org/wiki/Dow_Jones_Industrial_Average',
        "RUSSELL-2000": 'https://en.wikipedia.org/wiki/List_of_Russell_2000_stocks'
    }

    for name, url in urls.items():
        try:
            # Use requests in a separate thread to avoid blocking asyncio loop
            response = await asyncio.to_thread(requests.get, url, timeout=15)
            soup = BeautifulSoup(response.content, 'html.parser')
            
            tables = soup.find_all('table', {'class': 'wikitable'})
            
            found_tickers_for_index = False
            for table in tables:
                ...
                if 'SYMBOL' in headers:
                    ticker_col_index = headers.index('SYMBOL')
                elif 'TICKER' in headers:
                    ticker_col_index = headers.index('TICKER')
                ...
                if ticker_col_index != -1:
                    for row in table.find_all('tr')[1:]: # Skip header
                        ...
                        if len(cells) > ticker_col_index:
                            ...
                            ticker = cells[ticker_col_index].text.strip().upper().replace('.', '-')
                            # **Validation Added**: Ensure it looks like a ticker (short, alphanumeric)
                            if ticker and len(ticker) <= 5 and all(c.isalnum() or c == '-' for c in ticker):
                                ticker_set.add(ticker)
                                found_tickers_for_index = True
                    if found_tickers_for_index:
                        break
        except Exception as e:
            if not is_called_by_ai:
                print(f"   -> Warning: Could not fetch tickers for {name}. Reason: {type(e).__name__}")
                
    if not is_called_by_ai:
        print(f"   -> Fetched {len(ticker_set)} unique tickers from additional indices.")
    return ticker_set

def plot_simulation_performance_graph(user_portfolio_history: list, index_data: pd.DataFrame, simulation_period_str: str, simulation_dates: pd.Index, filename_prefix: str = "simulation_performance"):
    """
    ...
    try:
        ...
        # **FIX**: Localize the simulation dates to match the timezone of the index data (or remove timezone from both)
        # Easiest fix is to remove timezone from index_data before plotting.
        if isinstance(index_data.index, pd.DatetimeIndex):
            index_data.index = index_data.index.tz_localize(None)

        user_dates = simulation_dates[:len(user_portfolio_history)].tz_localize(None) # Also remove tz here for consistency
...
async def handle_simulation_command(args: List[str], is_called_by_ai: bool = False):
    ...
    sp500_tickers = await asyncio.to_thread(get_sp500_symbols_singularity, is_called_by_ai=True)
    ...
    while current_week < len(simulation_df):
        ...
        # **FIX**: Output sample commands at the start and every 10 weeks
    ...
    # **FIX**: Corrected Best/Worst trade logic with FIFO accounting.
    best_trade_profit = 0
    worst_trade_loss = 0
    ...
    for sell_trade in sells:
        ...
        if abs(shares_to_account_for) > sell_trade['shares'] * 0.001:
            # This indicates not enough buy history was found for this sale.
            continue

        total_sale_value = sell_trade['shares'] * sell_trade['price']
        total_profit = total_sale_value - total_cost_basis

        # Initialize worst_trade_loss if it's still at 0 to correctly capture the first loss
...
async def handle_fundamentals_command(args: list, ai_params: dict = None, is_called_by_ai: bool = False):
    """
    Fetches key fundamental data for a stock, calculates a dynamic fundamental score
    using curved equations, and displays the information in a table.
    """
    if is_called_by_ai:
    ...
    try:
        ...
        if not info or info.get('trailingPE') is None:
             message = f"Could not retrieve fundamental data for '{ticker}'. The ticker may be invalid or delisted."
             if not is_called_by_ai:
                 print(message)
             return {"error": message} if is_called_by_ai else None

        # Extract raw data using the safe_get helper
        ...
        # --- Calculate Individual Scores Using Dynamic Equations ---

        # 1. P/E Ratio Score
        pe_score = 0.0
        if pe_ratio_raw is not None:
            if pe_ratio_raw > 0:
                pe_score = 25 * np.exp(-0.00042 * pe_ratio_raw**2)
            # If pe_ratio_raw is <= 0, score remains 0

        # 2. Revenue Growth Score (convert decimal to percentage for equation)
        rev_growth_score = 0.0
        if revenue_growth_raw is not None:
            rev_growth_percent = revenue_growth_raw * 100
            rev_growth_score = 25 / (1 + np.exp(-0.11 * (rev_growth_percent - 12.5)))
        ...
        if debt_to_equity_raw is not None:
            if debt_to_equity_raw <= 0:
                ...
                de_score = 25 * np.exp(-0.00956 * debt_to_equity_raw)

        # 4. Profit Margin Score (convert decimal to percentage for equation)
        margin_score = 0.0
        if profit_margin_raw is not None:
            margin_percent = profit_margin_raw * 100
            margin_score = 25 / (1 + np.exp(-0.11 * (margin_percent - 12.5)))

        # Calculate Final Fundamental Score
        total_score = pe_score + rev_growth_score + de_score + margin_score

        # --- Prepare data for tabulation ---
        pe_ratio_str = f"{pe_ratio_raw:.2f}" if pe_ratio_raw is not None else "N/A"
        rev_growth_str = f"{revenue_growth_raw*100:.2f}%" if revenue_growth_raw is not None else "N/A"
        de_ratio_str = f"{debt_to_equity_raw:.2f}" if debt_to_equity_raw is not None else "N/A"
        margin_str = f"{profit_margin_raw*100:.2f}%" if profit_margin_raw is not None else "N/A"

        table_data = [
            ["P/E Ratio", pe_ratio_str, f"{pe_score:.2f} / 25"],
            ["Revenue Growth", rev_growth_str, f"{rev_growth_score:.2f} / 25"],
            ["Debt-to-Equity", de_ratio_str, f"{de_score:.2f} / 25"],
            ["Profit Margin", margin_str, f"{margin_score:.2f} / 25"],
            ["---", "---", "---"],
            ["Fundamental Score", "", f"{total_score:.2f} / 100"]
        ]

        if not is_called_by_ai:
            print(tabulate(table_data, headers=["Metric", "Value", "Score"], tablefmt="pretty"))
        
        # Return a dictionary for AI calls
        return {
            "ticker": ticker.upper(),
            ...
            "scores": {
                "pe_score": pe_score,
                "revenue_growth_score": rev_growth_score,
                "debt_to_equity_score": de_score,
                "profit_margin_score": margin_score,
            },
            "fundamental_score": total_score,
            "summary_table": tabulate(table_data, headers=["Metric", "Value", "Score"], tablefmt="grid")
        }
...
async def handle_optimize_command(args: list, is_called_by_ai: bool = False):
    """
    Calculates the optimal portfolio weights for maximum Sharpe ratio using PyPortfolioOpt.
    Includes interactive tailoring and performance calculation for the tailored portfolio.
    """
    if not EfficientFrontier:
        print("❌ Error: PyPortfolioOpt library is not installed. Please run 'pip install PyPortfolioOpt' to use this command.")
        return
    ...
    if not args:
        ...
        print("Example: /optimize AAPL MSFT GOOG V")
        print("         /optimize P-4")
        return
    ...
    if input_arg.startswith('P-'):
        ...
        try:
            ...
            if portfolio_row.empty:
                print(f"❌ Error: Portfolio code '{portfolio_code}' not found in the database.")
                return
    ...
    if not initial_ticker_list:
        print("❌ Error: No tickers to optimize. Please provide a valid list or portfolio code.")
        return

    print(f"-> Optimizing for {len(initial_ticker_list)} tickers from {source_description}...")

    # --- Step 2: Fetch historical data with fallback ---
    optimization_period = "3y"
    prices = pd.DataFrame()
    required_data_points_3y = 252 * 3 - 22  # Approx 3 years of trading days with a buffer

    try:
        print(f"-> Attempting to fetch 3y of historical data...")
        data_download_3y = await asyncio.to_thread(
            yf.download,
            tickers=initial_ticker_list,
            period="3y",
            interval="1d",
            progress=False
        )
        
        if not data_download_3y.empty and 'Close' in data_download_3y.columns:
            prices_3y = data_download_3y['Close']
            
            valid_counts = prices_3y.count()
            bad_tickers_3y = valid_counts[valid_counts < required_data_points_3y].index.tolist()

            if bad_tickers_3y:
                # --- MODIFICATION START ---
                print(f"⚠️ The following {len(bad_tickers_3y)} tickers have insufficient 3y data: {', '.join(bad_tickers_3y)}")
                print("   -> Attempting 1y period for all tickers to ensure consistency.")
                # --- MODIFICATION END ---
                optimization_period = "1y"
                
                data_download_1y = await asyncio.to_thread(
                    yf.download,
                    tickers=initial_ticker_list,
                    period="1y",
                    interval="1d",
                ...
                if not data_download_1y.empty:
                    prices = data_download_1y['Close']
            else:
                prices = prices_3y
        else:
            print(f"⚠️ No data returned for 3y period. Attempting 1y period for all tickers.")
            optimization_period = "1y"
            data_download_1y = await asyncio.to_thread(
                yf.download,
                tickers=initial_ticker_list,
        ...
        if prices.empty or len(prices.columns) < 2:
            print("❌ Error: Could not retrieve sufficient historical data for at least two tickers.")
            return
        
        final_tickers = prices.columns.tolist()
        if len(final_tickers) < len(initial_ticker_list):
            dropped_tickers = sorted(list(set(initial_ticker_list) - set(final_tickers)))
            print(f"⚠️ Warning: Dropped {len(dropped_tickers)} tickers due to missing data even after fallback: {', '.join(dropped_tickers)}")
        
        print(f"-> Continuing optimization with {len(final_tickers)} tickers over a {optimization_period} period...")

    except Exception as e:
        print(f"❌ Error fetching historical price data: {e}")
        return

    # --- Step 3: Interactive Tailoring Prompts ---
    tailor_portfolio = False
    total_portfolio_value = 0.0
    use_fractional_shares = False

    if not is_called_by_ai:
        tailor_input = input("Do you want to tailor this optimized portfolio to a specific value? (yes/no): ").lower().strip()
        if tailor_input == 'yes':
            tailor_portfolio = True
            try:
                value_input = input("Enter the total amount of money to allocate (e.g., 10000): ")
                total_portfolio_value = float(value_input)
                if total_portfolio_value <= 0:
                    print("❌ Error: Portfolio value must be positive. Aborting tailoring.")
                    tailor_portfolio = False
                else:
                    frac_input = input("Allow fractional shares (round to one decimal)? (yes/no): ").lower().strip()
                    use_fractional_shares = frac_input == 'yes'
            except ValueError:
                print("❌ Error: Invalid number entered for portfolio value. Aborting tailoring.")
                tailor_portfolio = False
    ...
    # --- Step 6: Get performance metrics and display results ---
    ideal_return, ideal_vol, ideal_sharpe = ef.portfolio_performance(verbose=False)

    if tailor_portfolio:
        print("\n--- Tailored Portfolio Allocation ---")
        tailored_data = []
        tailored_weights_dict = {ticker: 0.0 for ticker in final_tickers}
        total_spent = 0.0
        ...
        for ticker, weight in sorted_weights:
            if weight * 100 >= 0.01:
                ...
                ideal_dollar_amount = total_portfolio_value * weight
                ideal_shares = ideal_dollar_amount / latest_price
                
                shares_to_buy = 0.0
                if use_fractional_shares:
                    shares_to_buy = round(ideal_shares, 1)
                else:
                    shares_to_buy = float(round(ideal_shares))

                actual_dollar_allocation = shares_to_buy * latest_price
                total_spent += actual_dollar_allocation
                
                # Store the actual weight for the tailored performance calculation
                tailored_weights_dict[ticker] = actual_dollar_allocation / total_portfolio_value

                tailored_data.append([
                    ticker,
                    f"{shares_to_buy:.1f}" if use_fractional_shares else f"{int(shares_to_buy)}",
                    f"${actual_dollar_allocation:,.2f}",
                    f"{(actual_dollar_allocation / total_portfolio_value) * 100:.2f}%"
                ])
        
        remaining_cash = total_portfolio_value - total_spent

        print(tabulate(tailored_data, headers=["Ticker", "Shares to Buy", "$ Allocation", "Actual % Allocation"], tablefmt="pretty"))
        print(f"\nRemaining Cash: ${remaining_cash:,.2f}")

        # --- NEW: Calculate performance of the tailored portfolio ---
        # Create a weight array in the same order as ef.tickers
        tailored_weights_array = np.array([tailored_weights_dict[ticker] for ticker in ef.tickers])
        
        # Manually set the weights in the EfficientFrontier object
        ef.weights = tailored_weights_array
        tailored_return, tailored_vol, tailored_sharpe = ef.portfolio_performance(verbose=False)
    ...
    print("\n--- Expected Portfolio Performance (Based on Ideal Weights) ---")
    performance_data_ideal = [
        ["Expected Annual Return", f"{ideal_return * 100:.2f}%"],
        ["Annual Volatility", f"{ideal_vol * 100:.2f}%"],
        ["Sharpe Ratio", f"{ideal_sharpe:.2f}"]
    ]
    print(tabulate(performance_data_ideal, tablefmt="pretty"))

    if tailor_portfolio:
        print("\n--- Expected Portfolio Performance (Based on Tailored/Rounded Allocation) ---")
        performance_data_tailored = [
            ["Expected Annual Return", f"{tailored_return * 100:.2f}%"],
            ["Annual Volatility", f"{tailored_vol * 100:.2f}%"],
            ["Sharpe Ratio", f"{tailored_sharpe:.2f}"]
        ]
        print(tabulate(performance_data_tailored, tablefmt="pretty"))

    print("\nOptimization complete.")

async def handle_monitor_command(args: list, is_called_by_ai: bool = False):
    """
    Manages real-time monitoring alerts for stocks, now with persistent storage.
    """
    if is_called_by_ai:
        return "This command is interactive and designed for direct user use in the CLI."
    ...
    async with alert_lock:
        if action == "add":
            # (Parsing logic remains the same as the previous version)
            if len(args) < 5:
                print("Usage: /monitor add <TICKER> price <operator> <value>")
                print("       /monitor add <TICKER> invest <sensitivity 1-3> <operator> <value>")
                return
            ...
            if metric == 'price':
                ...
                try:
                    ...
                    if operator not in ['>', '<', '>=', '<=']:
                        ...
                        return
                    alert_to_add = {"ticker": ticker, "metric": "price", "operator": operator, "value": value, "sensitivity": None}
                except ValueError:
                    print("Error: The value for the alert must be a valid number.")

            elif metric == 'invest':
                if len(args) != 6:
                    print("Usage: /monitor add <TICKER> invest <sensitivity 1-3> <operator> <value>")
                    return
                try:
                    sensitivity = int(args[3])
                    operator = args[4]
                    value = float(args[5])
                    ...
                    if operator not in ['>', '<', '>=', '<=']:
                        print(f"Error: Invalid operator '{operator}'.")
                        return
                    alert_to_add = {"ticker": ticker, "metric": "invest", "sensitivity": sensitivity, "operator": operator, "value": value}
                except ValueError:
                    print("Error: Sensitivity and value must be valid numbers.")
            else:
                print(f"Error: Invalid metric '{metric}'. Use 'price' or 'invest'.")

            if alert_to_add:
                active_alerts.append(alert_to_add)
                save_alerts_to_csv() # Save the full updated list
                if metric == 'price':
                     print(f"✅ Alert added and saved: Monitor {ticker} price {alert_to_add['operator']} {alert_to_add['value']}")
                else:
                     print(f"✅ Alert added and saved: Monitor {ticker} INVEST score (sens: {alert_to_add['sensitivity']}) {alert_to_add['operator']} {alert_to_add['value']}")
            ...
            for i, alert in enumerate(active_alerts):
                ...
                table_data.append([i + 1, alert['ticker'], condition_str])
            print(tabulate(table_data, headers=["Index", "Ticker", "Condition"], tablefmt="pretty"))
            ...
            try:
                ...
                if 0 <= index_to_remove < len(active_alerts):
                    ...
                    save_alerts_to_csv()  # Save the list to the file after removing the item
...
async def handle_options_command(args: List[str], ai_params: Optional[Dict]=None, is_called_by_ai: bool = False):
    ...
    try:
        ticker = ask_singularity_input("Enter the stock ticker for options analysis").upper()
        ...
        if not date_idx_str or not date_idx_str.isdigit() or not (1 <= int(date_idx_str) <= len(exp_dates)):
            print("-> Invalid selection.")
            return
        
        selected_exp_date = exp_dates[int(date_idx_str) - 1]

        print(f"-> Fetching option chain for {selected_exp_date}...")
        opt_chain = await asyncio.to_thread(stock_yf.option_chain, selected_exp_date)
        all_strikes = []
        
        # --- MODIFICATION: Display strikes as a numbered list ---
        if not opt_chain.calls.empty:
            all_strikes = sorted(opt_chain.calls['strike'].tolist())
            ...
            for i, strike in enumerate(all_strikes):
                atm_label = " (ATM)" if strike == atm_strike else ""
                print(f"  {i+1}. ${strike:.2f}{atm_label}")
            print("-----------------------------\n")

        while True:
            print(f"--- Options Analysis Menu for {ticker} on {selected_exp_date} ---")
            print("1. Analyze Single Contract (Price & Greeks Visualization)")
            print("2. Model Strategy P/L (Profit/Loss Diagrams)")
            print("3. Get Recommended Short Strangle (IV-Based)")
            print("4. Back to Main Menu")
            
            choice = ask_singularity_input("Select an option (1-4)")
            
            # Pass the pre-fetched chain, price, and strikes list to the menu functions
            if choice == '1':
                await analyze_single_contract_menu(ticker, selected_exp_date, opt_chain, current_price, all_strikes)
            elif choice == '2':
                await model_strategy_pnl_menu(ticker, selected_exp_date, opt_chain, current_price, all_strikes)
            elif choice == '3':
...
# --- Core Data & Calculation Helper ---
async def get_option_contract_details(ticker_str: str, exp_date: str, strike: float, option_type: str, opt_chain=None, current_price: Optional[float] = None) -> Optional[Dict[str, Any]]:
    """
    Fetches all necessary data for a single option contract for BSM modeling.
    Uses pre-fetched data if provided to avoid redundant API calls.
    """
    try:
        # --- MODIFICATION: Use pre-fetched data if available ---
        if opt_chain is None or current_price is None:
            stock_yf = await asyncio.to_thread(yf.Ticker, ticker_str)
            info = await asyncio.to_thread(lambda: stock_yf.info)
            current_price = info.get('regularMarketPrice') or info.get('currentPrice')
            opt_chain = await asyncio.to_thread(stock_yf.option_chain, exp_date)
        
        dividend_yield = yf.Ticker(ticker_str).info.get('dividendYield') or 0.0
        
        if not current_price:
            print("   -> Error: Could not determine current stock price.")
            return None
        ...
        if contract.empty:
            # This error should now be rare due to numbered selection
            print(f"   -> Error: No {option_type} contract found for strike ${strike}.")
            return None

        contract_price = contract['lastPrice'].iloc[0]
        iv = contract['impliedVolatility'].iloc[0]

        return {
            "underlying_price": current_price, "strike": strike, "time_to_exp": time_to_exp,
            "iv": iv, "dividend_yield": dividend_yield, "risk_free_rate": risk_free_rate,
            "contract_price": contract_price, "flag": 'c' if option_type == 'call' else 'p'
        }
    except Exception as e:
        print(f"   -> Error fetching option contract details: {e}")
        return None

# --- Feature 1: Sub-menu and Plotting Logic (Now uses vectorized library) ---
async def analyze_single_contract_menu(ticker: str, exp_date: str, opt_chain, current_price: float, all_strikes: list):
    """Handles the user interaction for analyzing a single contract using numbered selection."""
    option_type = ask_singularity_input("Analyze a 'call' or a 'put'?").lower()
    if option_type not in ['call', 'put']:
        print("-> Invalid option type.")
        return

    if not all_strikes:
        print("-> No strikes available to analyze.")
        return
    ...
    try:
        ...
        selected_strike = all_strikes[strike_idx]
        print(f"-> Analyzing strike ${selected_strike:.2f}...")
        await plot_option_visualizations(ticker, exp_date, selected_strike, option_type, opt_chain, current_price)
    except (ValueError, TypeError):
        # --- MODIFICATION: Changed error message to match user log ---
        print("-> Invalid selection.")

async def plot_option_visualizations(ticker: str, exp_date: str, strike: float, option_type: str, opt_chain, current_price: float):
    """Generates 3D surface plots for an option's price and Delta."""
    print("   -> Fetching contract details for visualization...")
    details = await get_option_contract_details(ticker, exp_date, strike, option_type, opt_chain, current_price)
    if not details: return

    if details['iv'] <= 0 or details['time_to_exp'] <= 0:
        print(f"   -> ERROR: Invalid parameter for BSM model. IV or Time cannot be zero or negative. "
              f"(IV={details['iv']:.4f}, Time={details['time_to_exp']:.4f}). Cannot generate plots.")
        return

    try:
        # --- Calculate and Display Current Contract Details ---
        print("   -> Calculating current greeks and metrics...")
        current_df = pd.DataFrame({
            'S': [details['underlying_price']], 'K': [details['strike']], 't': [details['time_to_exp']],
            'r': [details['risk_free_rate']], 'sigma': [details['iv']], 'q': [details['dividend_yield']],
            'Flag': [details['flag']]
        })
        ...
        current_gamma = current_greeks['gamma'].iloc[0]
        current_theta = current_greeks['theta'].iloc[0]
        current_vega = current_greeks['vega'].iloc[0]
        current_rho = current_greeks['rho'].iloc[0]
        premium = details['contract_price']
        break_even = details['strike'] + premium if option_type == 'call' else details['strike'] - premium

        # --- NEW: Calculate Breakeven % and Chance of Profit ---
        breakeven_percent = ((break_even - details['underlying_price']) / details['underlying_price']) * 100
        chance_of_profit = abs(current_delta) * 100

        table_data = [
            ["Premium (Last Price)", f"${premium:,.2f}"],
            ["Break-Even Price", f"${break_even:,.2f} ({breakeven_percent:+.2f}%)"],
            ["Implied Volatility", f"{details['iv'] * 100:.2f}%"],
            ["Chance of Profit (by Delta)", f"{chance_of_profit:.2f}%"],
            ["-", "-"],
            ["Delta", f"{current_delta:.4f}"],
            ["Gamma", f"{current_gamma:.4f}"],
            ["Theta (per day)", f"{(current_theta / 365):.4f}"],
            ["Vega", f"{current_vega:.4f}"],
            ["Rho", f"{current_rho:.4f}"]
        ]
        print("\n" + tabulate(table_data, headers=[f"{ticker.upper()} ${details['strike']:.2f} {option_type.capitalize()}", "Value"], tablefmt="grid"))


        print("\n   -> Generating 3D plots for Price and Delta...")
        # Create ranges for underlying price and time
        ...
        df = pd.DataFrame({
            'S': S_grid.flatten(), 'K': details['strike'], 't': t_grid.flatten(),
            'r': details['risk_free_rate'], 'sigma': details['iv'], 'q': details['dividend_yield'],
        ...
        fig1 = plt.figure(figsize=(12, 8)); ax1 = fig1.add_subplot(111, projection='3d')
        # --- FIX IS HERE: Using suptitle and view_init to ensure all labels are visible ---
        fig1.suptitle(f'{ticker.upper()} {exp_date} ${strike:.2f} {option_type.capitalize()} Price Surface', color='white', fontsize=16)
        ax1.view_init(elev=35, azim=-70)
        ax1.plot_surface(S_grid, t_grid * 365, price_grid, cmap='viridis')
        ax1.set_xlabel('Stock Price ($)'); ax1.set_ylabel('Days to Expiration'); ax1.set_zlabel('Option Price ($)')
        filename1 = f"option_price_surface_{ticker}_{uuid.uuid4().hex[:6]}.png"
        plt.savefig(filename1, facecolor='black'); plt.close(fig1)
        print(f"   ✓ Price surface plot saved as: {filename1}")

        # Plotting Delta Surface
        fig2 = plt.figure(figsize=(12, 8)); ax2 = fig2.add_subplot(111, projection='3d')
        # --- FIX IS HERE: Using suptitle and view_init to ensure all labels are visible ---
        fig2.suptitle(f'{ticker.upper()} {exp_date} ${strike:.2f} {option_type.capitalize()} Delta Surface', color='white', fontsize=16)
        ax2.view_init(elev=35, azim=-70)
        ax2.plot_surface(S_grid, t_grid * 365, delta_grid, cmap='plasma')
        ax2.set_xlabel('Stock Price ($)'); ax2.set_ylabel('Days to Expiration'); ax2.set_zlabel('Option Delta')
        filename2 = f"option_delta_surface_{ticker}_{uuid.uuid4().hex[:6]}.png"
        plt.savefig(filename2, facecolor='black'); plt.close(fig2)
        print(f"   ✓ Delta surface plot saved as: {filename2}")
    
    except Exception as e:
        # Catch any other unexpected errors during the plotting process
        print(f"   -> An unexpected error occurred during plot generation: {e}")
        traceback.print_exc()
        raise
                
# --- Feature 2: Strategy Modeling Logic ---
async def model_strategy_pnl_menu(ticker: str, exp_date: str, opt_chain, current_price: float, all_strikes: list):
    ...
    # --- MODIFICATION: Change to numbered selection ---
    if not all_strikes:
    ...
    try:
        ...
        if strategy_choice == '1': # Long Call
            details = await get_option_contract_details(ticker, exp_date, strike, 'call', opt_chain, current_price)
            if not details: return
            ...
            details = await get_option_contract_details(ticker, exp_date, strike, 'put', opt_chain, current_price)
            if not details: return
        ...
        # Generic plotting for P/L diagrams
...
# --- Feature 3: Recommended Short Strangle ---
async def recommend_short_strangle(ticker: str, exp_date: str, opt_chain, current_price: float):
    ...
    try:
        # --- NEW ROBUST IV CALCULATION ---
        # 1. Combine calls and puts to find the most liquid strikes near the money
        combined_df = pd.concat([
            opt_chain.calls[['strike', 'impliedVolatility', 'openInterest']],
            opt_chain.puts[['strike', 'impliedVolatility', 'openInterest']]
        ])
        
        # 2. Find the 6 strikes closest to the current price
        combined_df['distance'] = abs(combined_df['strike'] - current_price)
        nearest_strikes_df = combined_df.nsmallest(12, 'distance') # Get 12 rows (6 strikes for calls/puts)
        
        # 3. Filter for contracts with open interest and average their IV
        ...
        if liquid_contracts.empty:
            ...
            # Fallback to simple ATM IV if no liquid contracts are found
            ...
            if reliable_iv <= 0.01:
                print(f"-> Fallback IV is also too low ({reliable_iv*100:.2f}%). Cannot proceed.")
                return
        else:
        ...
        if reliable_iv <= 0.01:
            print(f"-> Cannot calculate expected move with a low IV of {reliable_iv*100:.2f}%.")
            return
        ...
        # Find the closest available strikes to our targets
...
async def handle_ai_prompt(user_new_message: str, is_new_session: bool = False, original_session_request: Optional[str] = None):
    ...
    try:
        all_gemini_tools = Tool(function_declarations=[
            ...
            manage_user_favorites_tool_declaration,
    ...
    if is_new_session:
        ...
        try:
            ...
            formatted_system_prompt_text = "You are Cognis, the AI assistant. Today's date is {current_date_for_ai_prompt}.".format(current_date_for_ai_prompt=current_date_val)

        AI_CONVERSATION_HISTORY.append({"role": "user", "parts": [{"text": formatted_system_prompt_text}]})
        # *** MODIFIED GREETING HERE ***
        AI_CONVERSATION_HISTORY.append({"role": "model", "parts": [{"text": "Understood. I am Cognis, your AI assistant for the Singularity. How can I assist you?"}]})

    AI_CONVERSATION_HISTORY.append({"role": "user", "parts": [{"text": user_new_message}]})

    max_internal_turns = 100
...
def load_system_prompt(file_path="system_prompt.txt") -> str:
    default_prompt = """You are Cognis, the AI assistant for the 'Market Insights Center Singularity' script. Your name is Cognis. Refer to yourself as Cognis and encourage the user to do the same.
...
async def main_singularity():
    ...
    # --- Load persistent alerts at startup ---
    await load_alerts_from_csv()

    display_welcome_message()
    # --- Display only utility commands at startup ---
    display_utility_commands_only()

    # Launch the background alert worker
    ...
    while True:
        try:
            ...
            # --- CLI COMMAND ROUTING ---
            elif command == "/voice": await handle_voice_command()
            elif command == "/monitor": await handle_monitor_command(args, is_called_by_ai=False)
            elif command == "/options": await handle_options_command(args, is_called_by_ai=False)
            elif command == "/reportgeneration": await handle_report_generation(args, ai_params=None, is_called_by_ai=False)
            ...
            elif command == "/simulation": await handle_simulation_command(args, is_called_by_ai=False)
            elif command == "/macdforecast": await handle_macd_forecast_command(args, ai_params=None, is_called_by_ai=False)
            elif command == "/heatmap": await handle_heatmap_command(args, is_called_by_ai=False)
            elif command == "/fundamentals": await handle_fundamentals_command(args, ai_params=None, is_called_by_ai=False)
            elif command == "/optimize": await handle_optimize_command(args, is_called_by_ai=False)
            elif command == "/favorites": await handle_favorites_command(args, is_called_by_ai=False)
