...
def listen_for_voice() -> Optional[str]:
    ...
    with sr.Microphone() as source:
        ...
        print("🎤 Listening for wake words ('Cognis' or 'Sandman')...")
        try:
...
async def handle_voice_command():
    ...
    # --- MODIFIED: List of wake words ---
    wake_words = ["cognis", "sandman", "dude"]
    exit_phrases = ["stop listening", "stop chat", "end conversation"]
...
async def handle_report_generation(args: List[str], ai_params: Optional[Dict] = None, is_called_by_ai: bool = False):
    ...
    using a GICS-based CSV for stock selection and pre-screening based on sensitivity.
    """
    if is_called_by_ai:
        print("AI-driven report generation is not yet implemented. This is a CLI-only feature.")
        return

    print("\n--- M.I.C. Report Generation ---")
    ...
    # --- Step 2: Determine Strategy and Get Initial Ticker List from GICS CSV ---
    ...
    # Strategy dictionary remains the same
    strategy = {}
    ...
    # Get initial list of tickers from the GICS database based on user input
    ...
    # --- Step 3: Pre-screen stocks and finalize list ---
    ...
    all_stocks_with_scores.sort(key=lambda x: x.get('score', -1), reverse=True)
    
    # Select the top 25 tickers with the highest invest scores from the pre-screened list
    ...
    # The rest of the logic for breakouts, scoring, allocation, and report generation remains largely the same...
    # (This includes getting breakout stocks, calculating all scores, running the /invest logic, and generating text)
    
    # --- Step 3.5: Get Breakout Stocks and Calculate ALL Scores ---
    top_5_breakouts = []
    ...
    all_tickers_to_score = set(filtered_stocks_final)
    for sub_p_tickers in strategy['sub_portfolios'].values():
        all_tickers_to_score.update(sub_p_tickers.split(','))
    if top_5_breakouts:
        all_tickers_to_score.update(top_5_breakouts)
        
    print(f"-> Calculating Invest Scores for all {len(all_tickers_to_score)} portfolio components...")
    ...
    print("-> Allocating portfolio using /invest logic...")
    invest_params = {
    ...
    if 'Breakouts' in strategy['weights']:
        if top_5_breakouts: invest_params['sub_portfolios'].append({'tickers': ",".join(top_5_breakouts), 'weight': strategy['weights']['Breakouts']})
        else:
    ...
    # --- Step 5: Enhance Data & Generate Report Content ---
    print("-> Enhancing data and generating AI-powered report content...")
    print("   -> Fetching industry data for report...")
    industry_tasks = [asyncio.to_thread(business_summary_spear, h['ticker'], 1) for h in tailored_portfolio_data]
    industry_results = await asyncio.gather(*industry_tasks)
    
    for i, holding in enumerate(tailored_portfolio_data):
        _, industry, _, _ = industry_results[i]
    ...
    tailored_portfolio_data.sort(key=lambda x: x['actual_money_allocation'], reverse=True)
    ...
    print("   -> Generating AI trading strategy...")
    trading_strategy_text = await get_ai_trading_strategy(risk_tolerance, market_invest_score, strategy['name'])
    
    final_table_header = ["Ticker", "Invest Score", "Shares", "% Allocation", "Industry"]
    final_table_data = []

    print("   -> Generating AI rationale for each holding...")
    for holding in tailored_portfolio_data:
        ...
        final_table_data.append([ holding['ticker'], score_str, f"{holding['shares']:.2f}", f"{holding['percentage_allocation']:.2f}%", holding.get('industry', 'N/A') ])
        print(f"      - Rationale for {holding['ticker']}...")
        holding['rationale'] = await get_ai_stock_rationale(holding['ticker'], investment_goals, risk_tolerance)

    # --- Step 6: Final Output and Saving ---
    print("\n" + "="*80)
    print("--- Final Tailored Portfolio ---")
    print(tabulate(final_table_data, headers=final_table_header, tablefmt="pretty"))
    print(f"\nRemaining Cash: ${final_cash:,.2f} ({(final_cash/portfolio_value)*100:.2f}%)")
    print("="*80 + "\n")
    
    report_lines = [
        "INVESTMENT RECOMMENDATION", f"(Generated by M.I.C. Singularity on {datetime.now().strftime('%Y-%m-%d')})",
        "\n" + "="*80 + "\n", "## Client Profile", f"**Risk Tolerance:** {risk_tolerance}/5", f"**Investment Goals:** {investment_goals}",
        f"**Portfolio Value:** ${portfolio_value:,.2f}", "\n" + "="*80 + "\n", "## Proposed Solution", solution_text,
        "\n" + "="*80 + "\n", "## Proposed Trading Strategy", trading_strategy_text, "\n" + "="*80 + "\n", "## Holdings Analysis & Rationale",
        f"**Strategy:** {strategy['name']}\n",
    ]
    for holding in tailored_portfolio_data:
        report_lines.append(f"### {holding['ticker']} ({holding.get('industry', 'N/A')})")
        report_lines.append(f"- **Allocation:** ${holding['actual_money_allocation']:,.2f} ({holding['percentage_allocation']:.2f}%) | **Shares:** {holding['shares']:.2f}")
        report_lines.append(f"- **Rationale:** {holding.get('rationale', 'N/A')}")
        report_lines.append("-" * 40)
    
    report_lines.append("\n" + "="*80 + "\n")
    report_lines.append("## Final Portfolio Summary Table")
    report_lines.append(tabulate(final_table_data, headers=final_table_header, tablefmt="plain"))
    report_lines.append(f"\n{'CASH':<15} {'-':<15} {'-':<15} {((final_cash/portfolio_value)*100):<15.2f}% {'Cash Reserve'}")
    
    report_content = "\n".join(report_lines)
    filename = f"Singularity Report - {datetime.now().strftime('%Y-%m-%d')}.txt"
    try:
