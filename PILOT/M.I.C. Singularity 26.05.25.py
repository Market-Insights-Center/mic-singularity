...
# from tradingview_ta import TA_Handler, Interval, Exchange # Not used in /invest or /custom directly
import csv
from datetime import datetime, timedelta, time, date
import pytz
from typing import Optional, List, Dict, Any
import time as py_time # Renamed to avoid conflict with datetime.time
import traceback # Added for more detailed error information
...
# save_portfolio_to_csv, save_portfolio_data_cli
# get_sp500_symbols_cli, get_spy_symbols_cli, calculate_market_invest_scores_cli, save_market_data_cli
# run_breakout_analysis_cli, save_breakout_data_cli
...
def get_allocation_score() -> tuple[float, float, float]:
    """
    Placeholder for market_data.csv dependent score.
    For CLI, this might need a dedicated command to generate/update market_data.csv.
    Returning default values for now.
    """
    # This function is a placeholder. In a full system, it would read 'market_data.csv'
    # or similar to get actual market condition scores.
    print("Note: get_allocation_score() currently returns default values (50.0).")
    print("The 'market_data.csv' file is required for accurate scores from get_allocation_score().")
    print("You may need to implement a '/market' command or ensure this file exists and is up-to-date.")
    return 50.0, 50.0, 50.0 # Default: AvgScore, RiskGenScore, MktInvScore


async def process_custom_portfolio(
    portfolio_data_config: Dict[str, Any],
    tailor_portfolio_requested: bool,
    frac_shares_cli: bool,
    total_value_cli: Optional[float] = None,
    is_custom_command_simplified_output: bool = False
) -> tuple[List[str], List[Dict[str, Any]], float, List[Dict[str, Any]]]:
    """
    Processes custom or /invest portfolio requests for CLI.
    ...
    if tailor_portfolio_requested:
        if total_value_cli is None or safe_score(total_value_cli) <= 0:
        ...
        total_value_float = safe_score(total_value_cli)
        ...
        for entry in final_combined_portfolio_data_calc:
            ...
            if final_stock_alloc_pct > 1e-9 and live_price > 0: 
                ...
                try:
                    ...
                    if frac_shares_cli: shares = round(exact_shares, 1) 
                ...
                share_threshold = 0.1 if frac_shares_cli else 1.0 
        ...
        if is_custom_command_simplified_output: 
            ...
            for item in current_tailored_entries_calc: 
                tailored_portfolio_table_data_display.append([
                    item['ticker'], 
                    f"{item['shares']:.1f}" if frac_shares_cli and item['shares'] > 0 else f"{int(item['shares'])}", 
        ...
        if pie_chart_allocations_data:
            ...
            if portfolio_code_val: 
                ...
                if total_value_cli: chart_title_custom += f" (Value: ${safe_score(total_value_cli):,.0f})"
                ...
                if total_value_cli: chart_title_custom += f" (Value: ${safe_score(total_value_cli):,.0f})"
            elif total_value_cli: 
                 chart_title_custom = f"Tailored Portfolio Allocation (Value: ${safe_score(total_value_cli):,.0f})"
            
            generate_portfolio_pie_chart( 
                portfolio_allocations=pie_chart_allocations_data,
                chart_title=chart_title_custom,
                filename_prefix="cli_portfolio_pie" 
            )
...
async def save_portfolio_data_cli(portfolio_code_to_save: str, date_str_to_save: str):
    """
    Internal function to save combined portfolio data for a given code and date (CLI version).
    ...
    if portfolio_config_data and date_str_to_save:
        try:
            ...
            _, combined_result_for_save, _ = await process_custom_portfolio(
                ...
                frac_shares_cli=frac_shares_from_config, # Use config's frac_shares
                total_value_cli=None,                  # No total value needed for combined % save
...
def display_welcome_message():
    ...
    # ASCII Art (simple example - corrected escape sequence)
    # Using raw strings r"..." or escaping the backslash \\
    ascii_art = [
        r"    .--''''''--.    ",
        r"  .'            '.  ",
        r" /   O      O   \ ", # Corrected: no problematic escape needed or use r"..."
        r"|    \  ^^  /    |", # Corrected
        r" \     `----'     / ", # Corrected
        r"  '.            .'  ",
        r"    .--''''''--.    ",
    ...
    for line in ascii_art:
        for char_art in line:
            print(char_art, end="", flush=True)
            py_time.sleep(0.005) # Typing animation speed
        print() # Newline after each line of art
    print("\n")
...
def display_commands():
    ...
    print("/exit - Exit the Market Insights Center.")
    print("-------------------\n")

async def handle_invest_command():
    """Handles the /invest command logic for CLI."""
    print("\n--- /invest Command ---")
    try:
        ...
        portfolio_data_config_invest = {
            ...
            'risk_tolerance': '10', # Default, not asked in CLI for /invest
        ...
        await process_custom_portfolio(
            ...
            frac_shares_cli=frac_shares,
            total_value_cli=total_value,
...
async def collect_portfolio_inputs_cli(portfolio_code_cli: str) -> Optional[Dict[str, Any]]:
    """Collects all inputs for a new custom portfolio configuration via CLI."""
    print(f"\n--- Creating New Portfolio Configuration: '{portfolio_code_cli}' ---")
    inputs_cli = {'portfolio_code': portfolio_code_cli}
    ...
    try:
        ...
        inputs_cli['ema_sensitivity'] = str(ema_sens_val)
        ...
        inputs_cli['amplification'] = str(amp_val)

        num_port_str = input("Enter the number of sub-portfolios (e.g., 2): ")
        num_portfolios_cli = int(num_port_str)
        if num_portfolios_cli <= 0: raise ValueError("Number of sub-portfolios must be greater than 0.")
        inputs_cli['num_portfolios'] = str(num_portfolios_cli)
        ...
        inputs_cli['frac_shares'] = 'true' if frac_s_str == 'yes' else 'false'
        
        # Add fixed values from original collect_portfolio_inputs structure
        inputs_cli['risk_tolerance'] = '10' # Default, not typically user-set for /custom config
        inputs_cli['risk_type'] = 'stock'   # Default
        inputs_cli['remove_amplification_cap'] = 'true' # Default

        current_total_weight_cli = 0.0
        for i in range(1, num_portfolios_cli + 1):
            print(f"\n--- Sub-Portfolio {i} ---")
            tickers_input_cli = input(f"Enter tickers for Sub-Portfolio {i} (comma-separated, e.g., GOOG,AMZN): ").upper()
            if not tickers_input_cli.strip(): raise ValueError("Tickers cannot be empty for a sub-portfolio.")
            inputs_cli[f'tickers_{i}'] = tickers_input_cli

            if i == num_portfolios_cli: # Last portfolio, weight is auto-calculated
                weight_val_cli = 100.0 - current_total_weight_cli
                if weight_val_cli < -0.01 : # Allow small tolerance for floating point arithmetic
                    print(f"Error: Previous weights sum to {current_total_weight_cli}%, which exceeds 100%. Cannot set weight for the final portfolio.")
                    return None
                weight_val_cli = max(0, weight_val_cli) # Ensure weight is not negative
                print(f"Weight for Sub-Portfolio {i} automatically set to: {weight_val_cli:.2f}%")
            else: # For portfolios before the last one
                remaining_weight_cli = 100.0 - current_total_weight_cli
                weight_str_cli = input(f"Enter weight for Sub-Portfolio {i} (0-{remaining_weight_cli:.2f}%): ")
                ...
                if not (-0.01 < weight_val_cli < remaining_weight_cli + 0.01):
                    print(f"Invalid weight. Must be a positive number up to the remaining {remaining_weight_cli:.2f}%.")
                    return None
            inputs_cli[f'weight_{i}'] = f"{weight_val_cli:.2f}" # Store weight as string, formatted
            current_total_weight_cli += weight_val_cli
        
        if not math.isclose(current_total_weight_cli, 100.0, abs_tol=0.1): # Final check on total weight
            print(f"Warning: The sum of all sub-portfolio weights is {current_total_weight_cli:.2f}%, which is not 100%. The configuration will be saved, but analysis results might be affected.")
...
async def handle_custom_command(args: List[str]):
    """Handles the /custom command logic for CLI."""
    print("\n--- /custom Command ---")
    if not args:
    ...
    save_data_code_cli = args[1].strip() if len(args) > 1 else None
    ...
    if save_data_code_cli == "3725":
        if is_new_code_auto_cli: # Cannot save data for a code that was just auto-generated and not yet configured
        ...
        try:
            ...
            await save_portfolio_data_cli(portfolio_code_input, date_to_save_str)
        except ValueError:
    ...
    if file_exists_check_db and not is_new_code_auto_cli: # Try to load existing if not auto-generating
        try:
            with open(portfolio_db_file, 'r', encoding='utf-8', newline='') as file:
                reader = csv.DictReader(file)
    ...
    if portfolio_config_from_csv is None: 
        ...
        new_portfolio_config_cli = await collect_portfolio_inputs_cli(portfolio_code_input) # Collect all inputs
        
        if new_portfolio_config_cli: # If user provided all inputs successfully
            await save_portfolio_to_csv(portfolio_db_file, new_portfolio_config_cli) # Save the new config
            portfolio_config_from_csv = new_portfolio_config_cli # Use this new config for the current analysis run
        else: # User cancelled or failed input during collect_portfolio_inputs_cli
    ...
    if portfolio_config_from_csv:
        try:
            ...
            # Note: save_data_code_cli is NOT "3725" if we reach here.
            ...
            await process_custom_portfolio(
                ...
                frac_shares_cli=frac_shares_custom_run, # Based on config or override for this run
                total_value_cli=total_value_custom_run, # Value for this run, if tailoring
        ...
        except KeyError as e: # Should be less likely if collect_portfolio_inputs_cli is robust
...
async def run_breakout_analysis_cli():
...
async def save_breakout_data_cli(date_str: str):
...
async def handle_breakout_command(args: List[str]):
    """Handles the /breakout command logic for CLI."""
    ...
    if save_code_breakout == "3725":
        ...
        try:
            ...
            await save_breakout_data_cli(date_to_save_breakout)
        except ValueError:
            print("Invalid date format. Please use MM/DD/YYYY. Save operation cancelled.")
        ...
        await run_breakout_analysis_cli()

# --- Market Command Functions ---
def get_sp500_symbols_cli() -> List[str]:
    """Fetches S&P 500 symbols from Wikipedia for CLI use."""
    try:
...
def get_spy_symbols_cli() -> List[str]:
    """Returns S&P 500 symbols, as SPY tracks the S&P 500."""
    return get_sp500_symbols_cli()

async def calculate_market_invest_scores_cli(tickers: List[str], ema_sens: int) -> List[Dict[str, Any]]:
    """
...
async def save_market_data_cli(sensitivity: int, date_str: str):
    """
    ...
    spy_symbols_market = get_spy_symbols_cli()
    ...
    all_scores_data_market = await calculate_market_invest_scores_cli(spy_symbols_market, sensitivity)
...
async def handle_market_command(args: List[str]):
    """Handles the /market command logic for CLI."""
    print("\n--- /market Command ---")
    ...
    if save_code_market == "3725":
        try:
            ...
            await save_market_data_cli(sens_to_save, date_to_save_market)
        except ValueError:
        ...
        try:
            ...
            spy_symbols_market_disp = get_spy_symbols_cli()
            ...
            all_scores_data_market_disp = await calculate_market_invest_scores_cli(spy_symbols_market_disp, sens_to_display)
            ...
            # Already sorted by score in calculate_market_invest_scores_cli
...
def get_yf_data_cli(tickers: List[str], period: str = "10y", interval: str = "1d") -> pd.DataFrame:
    """ 
    Downloads historical closing price data for multiple tickers using yfinance.
    Adapted for CLI, synchronous. Modified to mitigate PerformanceWarning.
    """
    if not tickers:
        return pd.DataFrame()
    ...
    try:
        data = yf.download(tickers_list, period=period, interval=interval, progress=False, auto_adjust=False, group_by='ticker')
        if data.empty:
            return pd.DataFrame()
        ...
        if isinstance(data.columns, pd.MultiIndex):
            for ticker_name in tickers_list: 
                if ticker_name in data.columns.get_level_values(0): # Check if ticker is a top-level column name
                    try:
                        # Try to access ('TICKER', 'Close')
                        close_series = data[(ticker_name, 'Close')]
                        if not close_series.empty and not close_series.isnull().all():
                            series_dict[ticker_name] = pd.to_numeric(close_series, errors='coerce')
                    except KeyError:
                        # If ('TICKER', 'Close') fails, it might be that 'Close' is the first level
                        # and ticker_name is the second, or yf structure is different.
                        # This part might need more sophisticated handling if yf changes its output for group_by often.
                        # For now, we assume ('TICKER', 'Close') is the primary structure.
                        pass 
        elif len(tickers_list) == 1 and 'Close' in data.columns: # Single ticker, non-MultiIndex
            ticker_name = tickers_list[0]
            if not data['Close'].isnull().all():
                 series_dict[ticker_name] = pd.to_numeric(data['Close'], errors='coerce')
...
def screen_stocks_cli() -> List[str]:
    """ Screens for stocks with Market Cap > $50B and Avg Daily Vol > 1M using TradingView. CLI version."""
    ...
    try:
        ...
        # This is a blocking call, suitable for a CLI script if run directly or in a thread for async context
...
def calculate_metrics_cli(tickers_list: List[str], spy_data_10y: pd.DataFrame) -> Dict[str, Dict[str, float]]:
    """ Calculates 1y Beta, 1y Correlation, and 1y Avg Leverages relative to SPY. CLI version."""
    ...
    if spy_data_10y.empty or 'SPY' not in spy_data_10y.columns:
         print("    Error: Valid 10y SPY historical data ('SPY' column) is required for metrics calculation by calculate_metrics_cli.")
    ...
    try:
        ...
        if spy_daily_returns_full.empty:
             print("    Error: SPY daily returns are all NaN after calculation in calculate_metrics_cli.")
             return {}
    except Exception as e:
         print(f"    Error preparing SPY daily returns for metrics in calculate_metrics_cli: {e}")
         return {}
    ...
    all_tickers_data_metrics = get_yf_data_cli(tickers_to_fetch_metrics, period="10y", interval="1d")

    if all_tickers_data_metrics.empty:
        print("    Error: Failed to fetch historical data for the main list of tickers in calculate_metrics_cli.")
        return {}
...
def save_initial_metrics_cli(metrics: Dict[str, Dict[str, float]], tickers_processed: List[str]):
    """ Saves calculated initial metrics to CULTIVATE_INITIAL_METRICS_FILE. CLI version."""
...
def calculate_cultivate_formulas_cli(allocation_score: float) -> Optional[Dict[str, Any]]:
    """ Calculates Lambda, Omega, Alpha, Beta_alloc, Mu, Rho, Omega_target, Delta, Eta, Kappa. CLI version."""
...
async def select_tickers_cli(tickers_to_filter: List[str], metrics: Dict, invest_scores_all: Dict, 
                       formula_results: Dict, portfolio_value: float) -> tuple[List[str], Optional[str], Dict]: # Made async
    """ Selects final tickers based on metrics, scores, and formulas. CLI version. Now async."""
    ...
    # Call calculate_ema_invest with await since select_tickers_cli is now async
...
def build_and_process_portfolios_cli(common_stock_tickers: List[str], amplification: float, 
    ...
    """ Builds and processes Cultivate portfolios. CLI version. """
...
async def run_cultivate_analysis_cli(
    ...
    """ 
    Orchestrates Cultivate portfolio analysis for CLI. 
    Returns:
    ...
    formula_results_cult = calculate_cultivate_formulas_cli(sigma_cult) 
    ...
    if cultivate_code_str.upper() == 'A':
        ...
        tickers_to_process_cult = screen_stocks_cli() 
        ...
        tickers_to_process_cult = get_spy_symbols_cli() 
    ...
    spy_hist_data_metrics_cult = get_yf_data_cli(['SPY'], period="10y", interval="1d") 
    
    metrics_dict_cult = {}
    if not spy_hist_data_metrics_cult.empty: 
        if tickers_to_process_cult: 
            metrics_dict_cult = calculate_metrics_cli(tickers_to_process_cult, spy_hist_data_metrics_cult) 
            if not is_saving_run: save_initial_metrics_cli(metrics_dict_cult, tickers_to_process_cult) 
    ...
    final_common_stock_tickers_cult, selection_warning_cult, _ = await select_tickers_cli(
    ...
    (combined_portfolio_list_cult, tailored_portfolio_entries_cult, final_cash_value_cult, 
     ...
     initial_omega_cash_cult) = build_and_process_portfolios_cli(
...
async def save_cultivate_data_internal_cli(combined_portfolio_data: List[Dict], date_str: str, cultivate_code: str, epsilon: float):
    """Saves combined Cultivate portfolio data to CSV. CLI version."""
...
async def handle_cultivate_command(args: List[str]):
    """Handles the /cultivate command logic for CLI."""
    print("\n--- /cultivate Command ---")
    try:
        if len(args) < 3:
            print("Usage: /cultivate <Code A/B> <PortfolioValue> <FracShares yes/no> [save_code 3725]")
        ...
        if save_action_code == "3725":
            ...
            try:
                ...
                combined_data_cult_save, code_used_cult, eps_used_cult, _, err_msg_cult = await run_cultivate_analysis_cli(
                ...
                if err_msg_cult:
                    ...
                    return
                await save_cultivate_data_internal_cli(combined_data_cult_save, date_to_save_cult, code_used_cult, eps_used_cult)
            except ValueError:
                print("Invalid date format. Please use MM/DD/YYYY. Save operation cancelled.")
            ...
            await run_cultivate_analysis_cli(
                portfolio_value=portfolio_value_input,
                frac_shares=frac_shares_input_bool,
                cultivate_code_str=cult_code_input,
                is_saving_run=False
...
def ask_cli_input(prompt: str, validation_fn=None, error_msg: str = "Invalid input.", default_val=None) -> Optional[str]:
    """
    Helper function to ask for user input in CLI, with optional validation.
...
async def calculate_portfolio_beta_correlation_cli(
    ...
    hist_data_assess = get_yf_data_cli(all_tickers_for_hist_assess, period=backtest_period, interval="1d")
...
async def handle_assess_command(args: List[str]):
    """Handles the /assess command logic for CLI."""
    print("\n--- /assess Command ---")
    if not args:
    ...
    if assess_code == 'A':
        ...
        try:
            ...
            for ticker_a_item in tickers_list_a:
                try:
                    hist_a_df = get_yf_data_cli([ticker_a_item], period=selected_period_a) 
        ...
        try:
            ...
            while True:
                ticker_b_add = ask_cli_input("Enter ticker for portfolio (or type 'done' to finish, 'cash' for cash):").upper()
                if ticker_b_add == 'DONE': break
                if ticker_b_add == 'CASH':
                    cash_val_str = ask_cli_input("Enter cash amount:", validation_fn=lambda x: x.replace('.', '', 1).isdigit() and float(x) >= 0)
                    ...
                    shares_b_str = ask_cli_input(f"Enter number of shares for {ticker_b_add}:", validation_fn=lambda x: x.replace('.', '', 1).isdigit() and float(x) >= 0)
                    if shares_b_str:
            ...
            beta_corr_results_b = await calculate_portfolio_beta_correlation_cli(portfolio_holdings_b_input, total_value_b, backtest_period_b)
        ...
        try:
            ...
            _, _, final_cash_c, structured_tailored_holdings_c = await process_custom_portfolio(
                ...
                frac_shares_cli=frac_s_c, 
                total_value_cli=assess_value_c, 
            ...
            beta_corr_results_c = await calculate_portfolio_beta_correlation_cli(portfolio_holdings_for_assess_c, assess_value_c, backtest_period_c)
        ...
        try:
            ...
            # MODIFIED: Unpack 7 values from run_cultivate_analysis_cli
            # Expected: (combined_list, tailored_entries_list, final_cash_val, code_str, eps_val, frac_s_val, error_msg)
            _, tailored_holdings_cult_d, final_cash_cult_d, _, _, _, err_msg_cult_run = await run_cultivate_analysis_cli(
            ...
            beta_corr_results_d = await calculate_portfolio_beta_correlation_cli(portfolio_holdings_for_assess_d, epsilon_d, backtest_period_d)
...
if __name__ == "__main__":
    # Ensure an event loop is running if we have async functions
    try:
        asyncio.run(main_cli())
    except KeyboardInterrupt:
        print("\nApplication terminated by user.")
    except Exception as e_run: # Catch any other critical errors during startup/run
